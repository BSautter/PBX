...
Line:`,e.lineno)}),window.addEventListener(`unhandledrejection`,function(e){console.error(`Unhandled Promise Rejection:`,e.reason)}),debugLog(`Admin panel loading...`,new Date().toISOString()),(function(){var e=[`https://cdn.jsdelivr.net/npm/chart.js@4.4.0/dist/chart.umd.min.js`,`https://cdnjs.cloudflare.com/ajax/libs/Chart.js/4.4.0/chart.umd.min.js`,`https://unpkg.com/chart.js@4.4.0/dist/chart.umd.min.js`],t=0;function n(){if(t>=e.length){console.error(`All Chart.js CDN sources failed`),window.chartJsLoadFailed=!0;return}var r=document.createElement(`script`);r.src=e[t],r.onerror=function(){debugWarn(`Chart.js CDN `+(t+1)+` failed, trying next...`),t++,n()},r.onload=function(){debugLog(`Chart.js loaded successfully from CDN `+(t+1)),window.chartJsLoadFailed=!1},document.head.appendChild(r)}n()})(),window.API_BASE=(function(){var e=window.location.port;return e===`9000`||e===``||e===`80`||e===`443`?window.location.origin:window.location.protocol+`//`+(window.location.hostname||`localhost`)+`:9000`})(),window.pbxAuthHeaders=function(){var e={"Content-Type":`application/json`},t=localStorage.getItem(`pbx_token`);return t&&(e.Authorization=`Bearer `+t),e};function e(...e){window.WEBRTC_VERBOSE_LOGGING&&debugLog(`[VERBOSE]`,...e)}var t=class{constructor(t,n){this.apiUrl=t,this.extension=n,this.sessionId=null,this.callId=null,this.peerConnection=null,this.localStream=null,this.isCallActive=!1,this.remoteAudio=null,this._audioCtx=null,this._ringbackNodes=null,this._ringbackTimer=null,this._ringbackSafetyTimer=null,this._callStatusPollTimer=null,this._currentCallState=null,e(`WebRTC Phone constructor called:`,{apiUrl:this.apiUrl,extension:this.extension}),this.initializeUI()}_ensureAudioCtx(){return this._audioCtx||=new(window.AudioContext||window.webkitAudioContext),this._audioCtx.state===`suspended`&&this._audioCtx.resume(),this._audioCtx}startRingbackTone(){if(!this._ringbackNodes){this._ringbackSafetyTimer&&clearTimeout(this._ringbackSafetyTimer),this._ringbackSafetyTimer=setTimeout(()=>{debugLog(`[WebRTC Phone] Ringback safety timeout reached (60s), stopping`),this.stopRingbackTone()},6e4);try{let t=this._ensureAudioCtx(),n=t.createGain();n.gain.value=0,n.connect(t.destination);let r=t.createOscillator();r.frequency.value=440,r.connect(n),r.start();let i=t.createOscillator();i.frequency.value=480,i.connect(n),i.start(),this._ringbackNodes={osc1:r,osc2:i,gain:n};let a=2e3,o=4e3,s=!0;n.gain.value=.15,this._ringbackTimer=setInterval(()=>{s=!s,n.gain.value=s?.15:0},s?a:o),clearInterval(this._ringbackTimer);let c=()=>{this._ringbackNodes&&(this._ringbackNodes.gain.gain.value=.15,this._ringbackTimer=setTimeout(()=>{this._ringbackNodes&&(this._ringbackNodes.gain.gain.value=0,this._ringbackTimer=setTimeout(c,o))},a))};c(),e(`Ringback tone started`)}catch(e){console.error(`[WebRTC Phone] Error starting ringback tone:`,e)}}}stopRingbackTone(){if(this._ringbackSafetyTimer&&=(clearTimeout(this._ringbackSafetyTimer),null),this._ringbackTimer&&=(clearTimeout(this._ringbackTimer),null),this._ringbackNodes){try{this._ringbackNodes.osc1.stop(),this._ringbackNodes.osc2.stop(),this._ringbackNodes.gain.disconnect()}catch{}this._ringbackNodes=null,e(`Ringback tone stopped`)}}playDTMFTone(t){let n={1:[697,1209],2:[697,1336],3:[697,1477],4:[770,1209],5:[770,1336],6:[770,1477],7:[852,1209],8:[852,1336],9:[852,1477],"*":[941,1209],0:[941,1336],"#":[941,1477]}[t];if(n)try{let r=this._ensureAudioCtx(),i=r.createGain();i.gain.value=.15,i.connect(r.destination);let a=r.createOscillator();a.frequency.value=n[0],a.connect(i);let o=r.createOscillator();o.frequency.value=n[1],o.connect(i);let s=r.currentTime;a.start(s),o.start(s),a.stop(s+.15),o.stop(s+.15),i.gain.setValueAtTime(.15,s+.12),i.gain.linearRampToValueAtTime(0,s+.15),e(`DTMF tone played for digit:`,t)}catch(e){console.error(`[WebRTC Phone] Error playing DTMF tone:`,e)}}_startCallStatusPolling(){if(this._callStatusPollTimer)return;this._currentCallState=`calling`;let t=async()=>{if(!this.callId){this._stopCallStatusPolling();return}try{let t=await fetch(`${this.apiUrl}/api/webrtc/call-status?call_id=${encodeURIComponent(this.callId)}`,{headers:this.getAuthHeaders()});if(!t.ok)return;let n=await t.json();if(n.error)return;let r=n.status;if(r===this._currentCallState)return;e(`Call state changed:`,this._currentCallState,`->`,r);let i=this._currentCallState;this._currentCallState=r,r===`ringing`&&i===`calling`?(this.startRingbackTone(),this.updateStatus(`Ringing...`,`info`)):r===`connected`?(this.stopRingbackTone(),this.updateStatus(`Call connected`,`success`),this.updateUIState(`connected`)):r===`ended`&&(this.stopRingbackTone(),this._stopCallStatusPolling(),this.updateStatus(`Call ended by remote party`,`info`),this.hangup())}catch(t){e(`Call status poll error:`,t)}};this._callStatusPollTimer=setInterval(t,500),t()}_stopCallStatusPolling(){this._callStatusPollTimer&&=(clearInterval(this._callStatusPollTimer),null)}getAuthHeaders(){let e={"Content-Type":`application/json`},t=localStorage.getItem(`pbx_token`);return t&&(e.Authorization=`Bearer ${t}`),e}initializeUI(){this.callButton=document.getElementById(`webrtc-call-btn`),this.hangupButton=document.getElementById(`webrtc-hangup-btn`),this.muteButton=document.getElementById(`webrtc-mute-btn`),this.volumeSlider=document.getElementById(`webrtc-volume`),this.statusDiv=document.getElementById(`webrtc-status`),this.targetExtension=document.getElementById(`webrtc-target-ext`),this.keypadSection=document.getElementById(`webrtc-keypad-section`),this.remoteAudio=document.getElementById(`webrtc-remote-audio`),this.remoteAudio||(this.remoteAudio=document.createElement(`audio`),this.remoteAudio.id=`webrtc-remote-audio`,this.remoteAudio.autoplay=!0,this.remoteAudio.playsInline=!0,document.body.appendChild(this.remoteAudio)),this.remoteAudio.autoplay=!0,this.remoteAudio.playsInline=!0,this.remoteAudio.volume=.8,this.callButton&&this.callButton.addEventListener(`click`,()=>this.makeCall()),this.hangupButton&&this.hangupButton.addEventListener(`click`,()=>this.hangup()),this.muteButton&&this.muteButton.addEventListener(`click`,()=>this.toggleMute()),this.volumeSlider&&this.volumeSlider.addEventListener(`input`,e=>this.setVolume(e.target.value)),this.setupKeypad(),this.updateUIState(`idle`)}setupKeypad(){let t=document.querySelectorAll(`.keypad-btn`);t.forEach(e=>{e.addEventListener(`click`,e=>{let t=e.currentTarget.getAttribute(`data-digit`);t&&this.isCallActive&&(this.playDTMFTone(t),this.sendDTMF(t),e.currentTarget.classList.add(`pressed`),setTimeout(()=>{e.currentTarget.classList.remove(`pressed`)},300))})}),e(`Keypad initialized with`,t.length,`buttons`)}updateStatus(t,n=`info`){this.statusDiv&&(this.statusDiv.textContent=t,this.statusDiv.className=`webrtc-status ${n}`,debugLog(`[WebRTC Phone] ${t}`),e(`Status update:`,{message:t,type:n}))}updateUIState(e){this.callButton&&(this.callButton.disabled=e!==`idle`),this.hangupButton&&(this.hangupButton.disabled=e===`idle`),this.muteButton&&(this.muteButton.disabled=e===`idle`);let t=e===`connected`||e===`calling`;this.keypadSection&&(this.keypadSection.style.display=t?`block`:`none`),document.querySelectorAll(`.keypad-btn`).forEach(e=>{e.disabled=!t})}_checkWebRTCSupport(){if(!navigator.mediaDevices||!navigator.mediaDevices.getUserMedia){let e=`WebRTC not supported in this browser or context. Try using HTTPS or a modern browser.`;return this.updateStatus(e,`error`),console.error(`[WebRTC Phone]`,e),!1}return!0}async requestMicrophoneAccess(){try{return this.updateStatus(`Requesting microphone access...`,`info`),this._checkWebRTCSupport()?((await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:!0,noiseSuppression:!0,autoGainControl:!0},video:!1})).getTracks().forEach(e=>e.stop()),this.updateStatus(`Ready to call (microphone access granted)`,`success`),debugLog(`[WebRTC Phone] Microphone access granted`),!0):!1}catch(e){return console.error(`[WebRTC Phone] Microphone access denied:`,e),this.updateStatus(`Microphone access denied. Please allow microphone access in browser settings.`,`error`),!1}}async createSession(){try{this.updateStatus(`Creating WebRTC session...`,`info`),e(`Creating WebRTC session:`,{url:`${this.apiUrl}/api/webrtc/session`,extension:this.extension});let t=await fetch(`${this.apiUrl}/api/webrtc/session`,{method:`POST`,headers:this.getAuthHeaders(),body:JSON.stringify({extension:this.extension})});if(e(`Session creation response:`,{status:t.status,statusText:t.statusText,ok:t.ok}),!t.ok){let n=await t.text();throw e(`Session creation failed - response text:`,n),Error(`Failed to create session: ${t.statusText} - ${n}`)}let n=await t.json();if(e(`Session creation response data:`,n),!n.success)throw Error(`Session creation failed`);return this.sessionId=n.session.session_id,this.updateStatus(`Session created: ${this.sessionId}`,`success`),e(`Session created successfully:`,{sessionId:this.sessionId,iceServers:n.ice_servers}),this.peerConnection=new RTCPeerConnection(n.ice_servers),e(`RTCPeerConnection created with configuration:`,n.ice_servers),this.peerConnection.onicecandidate=t=>{e(`ICE candidate event:`,{candidate:t.candidate,url:t.url}),t.candidate&&this.sendICECandidate(t.candidate)},this.peerConnection.onconnectionstatechange=()=>{debugLog(`Connection state: ${this.peerConnection.connectionState}`),e(`Connection state changed:`,{connectionState:this.peerConnection.connectionState,iceConnectionState:this.peerConnection.iceConnectionState,iceGatheringState:this.peerConnection.iceGatheringState,signalingState:this.peerConnection.signalingState}),this.peerConnection.connectionState===`connected`?e(`WebRTC media path ready (waiting for SIP call progress)`):this.peerConnection.connectionState===`failed`?(e(`Connection FAILED - checking stats...`),this.stopRingbackTone(),this.updateStatus(`Connection failed`,`error`),this.hangup()):this.peerConnection.connectionState===`disconnected`&&(e(`Connection DISCONNECTED`),this.stopRingbackTone(),this.updateStatus(`Call disconnected`,`warning`),this.hangup())},this.peerConnection.oniceconnectionstatechange=()=>{debugLog(`ICE connection state: ${this.peerConnection.iceConnectionState}`),e(`ICE connection state changed:`,{iceConnectionState:this.peerConnection.iceConnectionState,iceGatheringState:this.peerConnection.iceGatheringState}),this.peerConnection.iceConnectionState===`failed`&&e(`ICE connection FAILED - this usually means network connectivity issues`)},this.peerConnection.ontrack=t=>{if(debugLog(`Received remote track`),e(`Remote track received:`,{track:t.track,streams:t.streams,trackKind:t.track.kind,trackId:t.track.id}),this.remoteAudio&&t.streams?.length>0){this.remoteAudio.srcObject=t.streams[0];let e=this.remoteAudio.play();e!==void 0&&e.then(()=>{debugLog(`Remote audio playback started successfully`),this.updateStatus(`Audio connected`,`success`)}).catch(e=>{debugWarn(`Audio autoplay prevented:`,e),this.updateStatus(`Audio ready (click to unmute if needed)`,`warning`)})}else debugWarn(`No remote audio element or streams available`)},!0}catch(t){return console.error(`Error creating session:`,t),e(`Error in createSession:`,{error:t,message:t.message,stack:t.stack}),this.updateStatus(`Error: ${t.message}`,`error`),!1}}async makeCall(){try{let t=this.targetExtension?.value??`1001`;if(e(`makeCall() called:`,{targetExtension:t}),!t){this.updateStatus(`Please enter target extension`,`error`);return}if(this.updateUIState(`connecting`),this.updateStatus(`Requesting microphone access...`,`info`),!this._checkWebRTCSupport()){this.updateUIState(`idle`);return}try{e(`Requesting user media...`),this.localStream=await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:!0,noiseSuppression:!0,autoGainControl:!0},video:!1}),e(`User media granted:`,{streamId:this.localStream.id,audioTracks:this.localStream.getAudioTracks().map(e=>({id:e.id,kind:e.kind,label:e.label,enabled:e.enabled,muted:e.muted,readyState:e.readyState}))}),this.updateStatus(`Microphone access granted`,`success`)}catch(t){console.error(`Microphone access denied:`,t),e(`Microphone access error:`,{error:t,name:t.name,message:t.message}),this.updateStatus(`Microphone access denied. Please allow microphone access.`,`error`),this.updateUIState(`idle`);return}if(e(`Creating WebRTC session...`),!await this.createSession()){e(`Session creation failed`),this.updateUIState(`idle`);return}e(`Adding local tracks to peer connection...`);for(let t of this.localStream.getTracks())this.peerConnection.addTrack(t,this.localStream),debugLog(`Added local track to peer connection`),e(`Track added:`,{trackId:t.id,kind:t.kind,label:t.label});this.updateStatus(`Creating call offer...`,`info`),e(`Creating RTC offer...`);let n=await this.peerConnection.createOffer();e(`Offer created:`,{type:n.type,sdpLength:n.sdp.length}),await this.peerConnection.setLocalDescription(n),e(`Local description set`),debugLog(`Waiting for ICE gathering to complete...`),await new Promise(e=>{if(this.peerConnection.iceGatheringState===`complete`){e();return}let t=()=>{this.peerConnection.iceGatheringState===`complete`&&(this.peerConnection.removeEventListener(`icegatheringstatechange`,t),e())};this.peerConnection.addEventListener(`icegatheringstatechange`,t),setTimeout(()=>{this.peerConnection.removeEventListener(`icegatheringstatechange`,t),debugLog(`ICE gathering timed out after 5 s, proceeding with available candidates`),e()},5e3)});let r=this.peerConnection.localDescription;debugLog(`SDP Offer created (ICE gathering done):`,r.sdp),e(`Full SDP Offer:`,r.sdp),e(`Sending offer to PBX:`,{url:`${this.apiUrl}/api/webrtc/offer`,sessionId:this.sessionId});let i=await fetch(`${this.apiUrl}/api/webrtc/offer`,{method:`POST`,headers:this.getAuthHeaders(),body:JSON.stringify({session_id:this.sessionId,sdp:r.sdp})});if(e(`Offer response:`,{status:i.status,statusText:i.statusText,ok:i.ok}),!i.ok){let t=await i.text();throw e(`Offer failed - response text:`,t),Error(`Failed to send offer: ${i.statusText} - ${t}`)}let a=await i.json();if(e(`Offer response data:`,a),!a.success)throw Error(`Offer was rejected by server`);if(a.answer_sdp){e(`Setting remote description from server SDP answer`);let t=new RTCSessionDescription({type:`answer`,sdp:a.answer_sdp});await this.peerConnection.setRemoteDescription(t),e(`Remote description set successfully`)}else e(`No SDP answer from server (legacy mode)`);this.updateStatus(`Calling extension ${t}...`,`info`),this.updateUIState(`calling`),e(`Initiating call to target extension:`,{url:`${this.apiUrl}/api/webrtc/call`,sessionId:this.sessionId,targetExtension:t});let o=await fetch(`${this.apiUrl}/api/webrtc/call`,{method:`POST`,headers:this.getAuthHeaders(),body:JSON.stringify({session_id:this.sessionId,target_extension:t})});e(`Call response:`,{status:o.status,statusText:o.statusText,ok:o.ok});let s=await o.json();if(e(`Call response data:`,s),s.success)this.isCallActive=!0,this.callId=s.call_id,this.updateStatus(`Calling ${t}...`,`info`),e(`Call initiated successfully:`,{callId:s.call_id}),this.startRingbackTone(),this._startCallStatusPolling();else throw e(`Call initiation failed:`,s),Error(s.error??`Failed to initiate call`)}catch(t){console.error(`Error making call:`,t),e(`Error in makeCall:`,{error:t,message:t.message,stack:t.stack}),this.updateStatus(`Call failed: ${t.message}`,`error`),this.hangup()}}async sendICECandidate(t){try{if(!this.sessionId){debugWarn(`Cannot send ICE candidate: no active session`),e(`ICE candidate send skipped - no session`);return}e(`Sending ICE candidate:`,{sessionId:this.sessionId,candidate:t.candidate,sdpMid:t.sdpMid,sdpMLineIndex:t.sdpMLineIndex}),await fetch(`${this.apiUrl}/api/webrtc/ice-candidate`,{method:`POST`,headers:this.getAuthHeaders(),body:JSON.stringify({session_id:this.sessionId,candidate:{candidate:t.candidate,sdpMid:t.sdpMid,sdpMLineIndex:t.sdpMLineIndex}})}),debugLog(`ICE candidate sent`),e(`ICE candidate sent successfully`)}catch(t){console.error(`Error sending ICE candidate:`,t),e(`Error sending ICE candidate:`,{error:t,message:t.message})}}async sendDTMF(t){if(!this.isCallActive||!this.sessionId){debugWarn(`Cannot send DTMF: No active call`);return}try{e(`Sending DTMF digit:`,t);let n=await fetch(`${this.apiUrl}/api/webrtc/dtmf`,{method:`POST`,headers:this.getAuthHeaders(),body:JSON.stringify({session_id:this.sessionId,digit:t,duration:160})});if(e(`DTMF response:`,{status:n.status,statusText:n.statusText}),n.ok){let r=await n.json();debugLog(`DTMF '${t}' sent successfully`),e(`DTMF send result:`,r);let i=this.statusDiv?.textContent??``;this.updateStatus(`Sent: ${t}`,`info`),setTimeout(()=>{this.isCallActive&&this.updateStatus(i,`success`)},500)}else console.error(`Failed to send DTMF '${t}':`,n.statusText),this.updateStatus(`Failed to send: ${t}`,`error`)}catch(t){console.error(`Error sending DTMF:`,t),e(`DTMF send error:`,{error:t,message:t.message})}}async hangup(){if(debugLog(`Hanging up call`),this.stopRingbackTone(),this._stopCallStatusPolling(),this.sessionId||this.callId)try{e(`Notifying server of hangup:`,{sessionId:this.sessionId,callId:this.callId});let t=await fetch(`${this.apiUrl}/api/webrtc/hangup`,{method:`POST`,headers:this.getAuthHeaders(),body:JSON.stringify({session_id:this.sessionId,call_id:this.callId})});t.ok?(e(`Server hangup response:`,await t.json()),debugLog(`Server notified of call termination`)):(debugWarn(`Failed to notify server of hangup:`,t.statusText),e(`Hangup notification failed:`,{status:t.status,statusText:t.statusText}))}catch(t){console.error(`Error notifying server of hangup:`,t),e(`Error in hangup notification:`,{error:t,message:t.message})}if(this.localStream){for(let e of this.localStream.getTracks())e.stop();this.localStream=null}this.peerConnection&&=(this.peerConnection.close(),null),this.sessionId=null,this.callId=null,this.isCallActive=!1,this.updateStatus(`Call ended`,`info`),this.updateUIState(`idle`)}toggleMute(){if(!this.localStream)return;let e=this.localStream.getAudioTracks()[0];e&&(e.enabled=!e.enabled,this.muteButton&&(this.muteButton.textContent=e.enabled?`🔇 Mute`:`🔊 Unmute`,this.muteButton.classList.toggle(`muted`,!e.enabled)),this.updateStatus(e.enabled?`Unmuted`:`Muted`,`info`))}setVolume(e){this.remoteAudio&&(this.remoteAudio.volume=e/100,debugLog(`Volume set to ${e}%`))}_startRingback(){if(!this._ringbackOsc)try{let e=new(window.AudioContext||window.webkitAudioContext),t=e.createGain();t.gain.value=.15;let n=e.createOscillator();n.frequency.value=440;let r=e.createOscillator();r.frequency.value=480,n.connect(t),r.connect(t),t.connect(e.destination),n.start(),r.start(),this._ringbackCtx=e,this._ringbackOsc=[n,r],this._ringbackGain=t;let i=!0;this._ringbackTimer=setInterval(()=>{i=!i,t.gain.setValueAtTime(i?.15:0,e.currentTime)},i?2e3:4e3),clearInterval(this._ringbackTimer);let a=()=>{this._ringbackCtx&&(t.gain.setValueAtTime(.15,e.currentTime),t.gain.setValueAtTime(0,e.currentTime+2),this._ringbackTimer=setTimeout(a,6e3))};a(),debugLog(`Ringback tone started`)}catch(e){debugWarn(`Could not start ringback tone:`,e)}}_stopRingback(){if(this._ringbackTimer&&=(clearTimeout(this._ringbackTimer),null),this._ringbackOsc&&=(this._ringbackOsc.forEach(e=>{try{e.stop()}catch{}}),null),this._ringbackCtx){try{this._ringbackCtx.close()}catch{}this._ringbackCtx=null}this._ringbackGain=null}_startStatusPolling(){this._statusPollTimer||(this._statusPollTimer=setInterval(()=>this._pollCallStatus(),1e3),e(`Call status polling started`))}_stopStatusPolling(){this._statusPollTimer&&(clearInterval(this._statusPollTimer),this._statusPollTimer=null,e(`Call status polling stopped`))}async _pollCallStatus(){if(this.sessionId)try{let t=await fetch(`${this.apiUrl}/api/webrtc/call-status`,{method:`POST`,headers:this.getAuthHeaders(),body:JSON.stringify({session_id:this.sessionId,call_id:this.callId})});if(!t.ok)return;let n=await t.json();e(`Call status poll:`,n),n.status===`ringing`?(this._startRingback(),this.updateStatus(`Ringing...`,`info`)):n.status===`connected`?(this._stopRingback(),this.updateStatus(`Call connected`,`success`),this.updateUIState(`connected`)):n.status===`ended`&&(this._stopRingback(),this._stopStatusPolling(),this.updateStatus(`Call ended by remote`,`info`),this.hangup())}catch{}}},n=null,r=`webrtc-admin`;async function i(){let e=window.location.origin,i=r;try{let e=localStorage.getItem(`pbx_token`),t=e?{Authorization:`Bearer ${e}`}:{},n=await(await fetch(`/api/webrtc/phone-config`,{headers:t})).json();n.success&&n.extension?(i=n.extension,debugLog(`WebRTC Phone using configured extension:`,i)):debugLog(`WebRTC Phone using default extension:`,i)}catch(e){debugWarn(`Failed to load WebRTC phone config, using default:`,e)}n&&n.isCallActive&&await n.hangup(),n=new t(e,i),debugLog(`WebRTC Phone initialized with extension:`,i),await n.requestMicrophoneAccess();let a=document.getElementById(`webrtc-status`);a&&(i===r?a.textContent=`Ready to call (Default extension)`:a.textContent=`Ready to call (Extension: ${i})`)}document.readyState===`loading`?document.addEventListener(`DOMContentLoaded`,i):i();function a(){return`
    ...
    `}var c=async e=>{try{let t=(await(await fetch(`/api/framework/click-to-dial/history/${e}`,{headers:pbxAuthHeaders()})).json()).history??[],n=document.getElementById(`click-to-dial-history`);if(t.length===0){n.innerHTML=`<p>No call history for extension ${escapeHtml(String(e))}</p>`;return}n.innerHTML=`
        ...
        `}catch(e){document.getElementById(`click-to-dial-history`).innerHTML=`<div class="error-box">Error loading history: ${escapeHtml(e.message)}</div>`}};function l(){return setTimeout(()=>{ee()},100),`
    ...
    `}var ee=async()=>{try{te((await(await fetch(`/api/framework/video-conference/rooms`,{headers:pbxAuthHeaders()})).json()).rooms??[])}catch(e){document.getElementById(`video-rooms-list`).innerHTML=`<div class="error-box">Error loading rooms: ${escapeHtml(e.message)}</div>`}};function te(e){let t=document.getElementById(`video-rooms-list`);if(e.length===0){t.innerHTML=`<p>No conference rooms created yet.</p>`;return}t.innerHTML=`
        <table class="data-table">
            <thead>
                <tr>
                    <th>Room Name</th>
...
features:
  conversational_ai:
    ...
    `}var re=async()=>{let e=document.getElementById(`ai-statistics`);e.innerHTML=`<p>Loading statistics...</p>`;try{let t=(await(await fetch(`/api/framework/conversational-ai/statistics`,{headers:pbxAuthHeaders()})).json()).statistics??{};e.innerHTML=`
    ...
    `}var u=async()=>{try{let e=(await(await fetch(`/api/framework/predictive-dialing/campaigns`,{headers:pbxAuthHeaders()})).json()).campaigns??[],t=document.getElementById(`campaigns-list`);if(e.length===0){t.innerHTML=`<p style="color: #666;">No campaigns created yet. Framework ready for campaign management.</p>`;return}t.innerHTML=`
            <table class="data-table">
                <thead>
                    <tr>
                        <th>Campaign Name</th>
    ...
    `}var de=async()=>{try{let e=(await(await fetch(`/api/framework/voice-biometrics/profiles`,{headers:pbxAuthHeaders()})).json()).profiles??[],t=document.getElementById(`biometric-profiles-list`);if(e.length===0){t.innerHTML=`<p style="color: #666;">No users enrolled yet. Framework ready for voice enrollment.</p>`;return}t.innerHTML=`
            <table class="data-table">
                <thead>
                    <tr>
                        <th>Extension</th>
    ...
    `),document.getElementById(`enroll-user-form`).onsubmit=async function(e){e.preventDefault();let t={user_id:new FormData(e.target).get(`user_id`)};try{let e=await(await fetch(`/api/framework/voice-biometrics/enroll`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify(t)})).json();e.success?(alert(`Enrollment started for ${e.user_id}.\n\nSession ID: ${e.session_id}\nRequired samples: ${e.required_samples}\n\nPlease complete voice sample recording via the phone system.`),pe(),de()):alert(`Error starting enrollment: ${e.error??`Unknown error`}`)}catch(e){alert(`Error starting enrollment: `+e.message)}}}var he=async e=>{if(confirm(`Are you sure you want to delete this voice profile?`))try{await fetch(`/api/framework/voice-biometrics/profile/${e}`,{method:`DELETE`,headers:pbxAuthHeaders()}),await de()}catch(e){alert(`Error: ${e.message}`)}};function ge(){return`
    ...
    `}function ve(){return(async()=>{try{let e=await(await fetch(`/api/framework/bi-integration/statistics`,{headers:pbxAuthHeaders()})).json();document.getElementById(`bi-stats-display`).innerHTML=`
                <div class="stats-grid">
                    <div class="stat-card">
                        <div class="stat-icon">📦</div>
...
POST /api/framework/bi-integration/test-connection</pre>
    ...
    `}var ye=async e=>{let t=document.getElementById(`bi-export-format`)?.value||`csv`,n=document.getElementById(`bi-date-range`)?.value||`last7days`,r=event.target,i=r.textContent;r.textContent=`Exporting...`,r.disabled=!0;try{let r=await(await fetch(`/api/framework/bi-integration/export`,{method:`POST`,headers:pbxAuthHeaders(),body:JSON.stringify({dataset:e,format:t,date_range:n})})).json();r.success?alert(`✅ Export successful!\n\nDataset: ${r.dataset}\nFormat: ${r.format}\nFile: ${r.file_path}\n\nThe export has been created on the server.`):alert(`❌ Export failed: ${r.error}`)}catch(e){alert(`❌ Export error: ${e.message}`)}finally{r.textContent=i,r.disabled=!1}};function be(){return(async()=>{try{let e=await(await fetch(`/api/framework/call-tagging/statistics`,{headers:pbxAuthHeaders()})).json();document.getElementById(`tagging-stats-display`).innerHTML=`
                <div class="stats-grid">
                    <div class="stat-card">
                        <div class="stat-icon">🏷️</div>
            ...
            `}catch(e){console.error(`Error loading tagging statistics:`,e)}})(),setTimeout(()=>{d(),f()},100),`
...
POST /api/framework/call-tagging/classify/{call_id}</pre>
    ...
    `}var d=async()=>{try{let e=(await(await fetch(`/api/framework/call-tagging/tags`,{headers:pbxAuthHeaders()})).json()).tags??[],t=document.getElementById(`tags-list`);if(e.length===0){t.innerHTML=`<p style="color: #666;">No tags created yet. Click "+ Create Tag" to add your first tag.</p>`;return}t.innerHTML=`
    ...
    `),document.getElementById(`create-rule-form`).onsubmit=async function(e){e.preventDefault();let t=new FormData(e.target),n={name:t.get(`name`),tag_id:t.get(`tag_id`),priority:parseInt(t.get(`priority`))||100,conditions:[{type:t.get(`condition_type`),value:t.get(`condition_value`)}]};try{let e=await(await fetch(`/api/framework/call-tagging/rule`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify(n)})).json();e.success?(alert(`Rule created successfully!`),we(),f(),xe()):alert(`Error creating rule: ${e.error??`Unknown error`}`)}catch(e){alert(`Error creating rule: `+e.message)}}}var Ee=async e=>{if(confirm(`Are you sure you want to delete this tag?`))try{await fetch(`/api/framework/call-tagging/tags/${e}`,{method:`DELETE`,headers:pbxAuthHeaders()}),await d()}catch(e){alert(`Error: ${e.message}`)}},De=async e=>{if(confirm(`Are you sure you want to delete this rule?`))try{await fetch(`/api/framework/call-tagging/rules/${e}`,{method:`DELETE`,headers:pbxAuthHeaders()}),await f()}catch(e){alert(`Error: ${e.message}`)}},Oe=async e=>{try{await fetch(`/api/framework/call-tagging/rules/${e}/toggle`,{method:`POST`,headers:pbxAuthHeaders()}),await f()}catch(e){alert(`Error: ${e.message}`)}};function ke(){return`
...
features:
  team_messaging:
    ...
    `,a.innerHTML=o;let s=a.querySelector(`#close-${n}`);s&&s.addEventListener(`click`,()=>a.remove()),document.body.appendChild(a),setTimeout(()=>{document.getElementById(n)&&(a.style.animation=`slideOut 0.3s ease-in`,setTimeout(()=>a.remove(),300))},v.displayTime),console.error(`[${t||`Error`}]`,r),i&&console.error(`Stack trace:`,i)}var it=1e4,at=null;function ot(){typeof window.loadEmergencyContacts==`function`&&window.loadEmergencyContacts(),typeof window.loadEmergencyHistory==`function`&&window.loadEmergencyHistory()}function st(){typeof window.loadFraudAlerts==`function`&&window.loadFraudAlerts()}function ct(){typeof window.loadCallbackQueue==`function`&&window.loadCallbackQueue()}function lt(e){at&&=(clearInterval(at),null);let t={dashboard:()=>window.loadDashboard?.(),analytics:()=>window.loadAnalytics?.(),calls:()=>window.loadCalls?.(),qos:()=>window.loadQoSMetrics?.(),emergency:ot,"callback-queue":ct,extensions:()=>window.loadExtensions?.(),phones:()=>window.loadRegisteredPhones?.(),atas:()=>window.loadRegisteredATAs?.(),"hot-desking":()=>window.loadHotDeskSessions?.(),voicemail:()=>window.loadVoicemailTab?.(),"fraud-detection":st,"sbc-management":()=>window.loadSBCData?.()};t[e]&&(at=setInterval(()=>{try{let n=t[e];typeof n==`function`?n():console.error(`Auto-refresh function for ${e} is not a function:`,n)}catch(t){console.error(`Error during auto-refresh of ${e}:`,t),t instanceof Error&&t.message?.includes(`401`)&&debugWarn(`Authentication error during auto-refresh - user may need to re-login`)}},it)),g.set(`autoRefreshInterval`,at)}function x(e){for(let e of document.querySelectorAll(`.tab-content`))e.classList.remove(`active`);for(let e of document.querySelectorAll(`.tab-button`))e.classList.remove(`active`);let t=document.getElementById(e);if(!t)console.error(`CRITICAL: Tab element with id '${e}' not found in DOM`),console.error(`This may indicate a UI template issue or incorrect tab name`),console.error(`Current tab name: "${e}"`);else{t.classList.add(`active`);let n=document.querySelector(`[data-tab="${e}"]`);n?n.classList.add(`active`):debugWarn(`Tab button for '${e}' not found`)}g.set(`currentTab`,e),lt(e);let n={dashboard:window.loadDashboard,analytics:window.loadAnalytics,extensions:window.loadExtensions,phones:window.loadRegisteredPhones,atas:window.loadRegisteredATAs,provisioning:window.loadProvisioning,"auto-attendant":window.loadAutoAttendantConfig,voicemail:window.loadVoicemailTab,paging:window.loadPagingData,calls:window.loadCalls,config:window.loadConfig,"features-status":window.loadFeaturesStatus,"webrtc-phone":window.loadWebRTCPhoneConfig,"license-management":window.initLicenseManagement,qos:window.loadQoSMetrics,"find-me-follow-me":window.loadFMFMExtensions,"time-routing":window.loadTimeRoutingRules,webhooks:window.loadWebhooks,"hot-desking":window.loadHotDeskSessions,"recording-retention":window.loadRetentionPolicies,"jitsi-integration":window.loadJitsiConfig,"matrix-integration":window.loadMatrixConfig,"espocrm-integration":window.loadEspoCRMConfig,"click-to-dial":window.loadClickToDialTab,"fraud-detection":window.loadFraudDetectionData,"nomadic-e911":window.loadNomadicE911Data,"callback-queue":window.loadCallbackQueue,"mobile-push":window.loadMobilePushConfig,"recording-announcements":window.loadRecordingAnnouncements,"speech-analytics":window.loadSpeechAnalyticsConfigs,compliance:window.loadComplianceData,"crm-integrations":window.loadCRMActivityLog,"opensource-integrations":window.loadOpenSourceIntegrations,"sbc-management":window.loadSBCData},r={emergency:[window.loadEmergencyContacts,window.loadEmergencyHistory],codecs:[window.loadCodecStatus,window.loadDTMFConfig],"sip-trunks":[window.loadSIPTrunks,window.loadTrunkHealth],"least-cost-routing":[window.loadLCRRates,window.loadLCRStatistics]},i=n[e];if(i)i();else{let t=r[e];if(t)for(let e of t)e?.()}}function ut(){let e=document.querySelectorAll(`.tab-button`);for(let t of e)t.addEventListener(`click`,()=>{let e=t.getAttribute(`data-tab`);e&&x(e)});let t=document.querySelector(`.sidebar`);t&&t.addEventListener(`keydown`,e=>{let t=e,n=t.target;if(!n.classList.contains(`tab-button`))return;let r=n.closest(`.sidebar-section`);if(!r)return;let i=Array.from(r.querySelectorAll(`.tab-button`)),a=i.indexOf(n),o=-1;t.key===`ArrowDown`?o=a<i.length-1?a+1:0:t.key===`ArrowUp`?o=a>0?a-1:i.length-1:t.key===`Home`?o=0:t.key===`End`&&(o=i.length-1),o>=0&&(t.preventDefault(),i[o]?.focus())}),document.addEventListener(`keydown`,e=>{if(e.key===`Escape`){let e=document.querySelector(`.modal.active`);e&&e.classList.remove(`active`)}})}async function dt(e,t=5,n=1e3){if(!Array.isArray(e))throw TypeError(`promiseFunctions must be an array`);let r=[];for(let i=0;i<e.length;i+=t){let a=e.slice(i,i+t).map(e=>typeof e==`function`?e():e),o=await Promise.allSettled(a);r.push(...o),i+t<e.length&&await new Promise(e=>setTimeout(e,n))}return r}async function ft(){let e=document.getElementById(`refresh-all-button`);if(!e||e.disabled)return;let t=e.textContent,n=e.disabled;try{e.textContent=`⏳ Refreshing All Tabs...`,e.disabled=!0,window.suppressErrorNotifications=!0,debugLog(`Refreshing all data for ALL tabs...`);let t=[];window.loadDashboard&&t.push(()=>window.loadDashboard()),window.loadADStatus&&t.push(()=>window.loadADStatus()),window.loadAnalytics&&t.push(()=>window.loadAnalytics()),window.loadExtensions&&t.push(()=>window.loadExtensions());let n=(await dt(t,5,1e3)).filter(e=>e.status===`rejected`);n.length>0&&debugLog(`${n.length} refresh operation(s) failed (expected for unavailable features):`,n.map(e=>e.reason?.message??e.reason)),y(`✅ All tabs refreshed successfully`,`success`)}catch(e){let t=e instanceof Error?e.message:String(e);console.error(`Error refreshing data:`,e),y(`Failed to refresh: ${t}`,`error`)}finally{window.suppressErrorNotifications=!1,e.textContent=t,e.disabled=n}}function pt(){let e=document.getElementById(`refresh-all-button`);e&&e.addEventListener(`click`,ft)}window.executeBatched=dt,window.refreshAllData=ft;var mt=6e4;async function ht(){try{let e=await h(`${p()}/api/status`,{headers:m()});if(!e.ok)throw Error(`HTTP ${e.status}: ${e.statusText}`);let t=await e.json();document.getElementById(`stat-extensions`).textContent=String(t.registered_extensions??0),document.getElementById(`stat-calls`).textContent=String(t.active_calls??0),document.getElementById(`stat-total-calls`).textContent=String(t.total_calls??0),document.getElementById(`stat-recordings`).textContent=String(t.active_recordings??0);let n=document.getElementById(`system-status`);n&&(n.textContent=`System: ${t.running?`Running`:`Stopped`}`,n.classList.remove(`connected`,`disconnected`),n.classList.add(`status-badge`,t.running?`connected`:`disconnected`)),_t()}catch(e){console.error(`Error loading dashboard:`,e);for(let e of[`stat-extensions`,`stat-calls`,`stat-total-calls`,`stat-recordings`]){let t=document.getElementById(e);t&&(t.textContent=`Error`)}y(`Failed to load dashboard: ${e instanceof Error?e.message:String(e)}`,`error`)}}function gt(){ht(),y(`Dashboard refreshed`,`success`)}async function _t(){try{let e=await h(`${p()}/api/integrations/ad/status`,{headers:m()});if(!e.ok)throw Error(`HTTP ${e.status}`);let t=await e.json(),n=document.getElementById(`ad-status-badge`);n&&(n.textContent=t.enabled?`Enabled`:`Disabled`,n.className=`status-badge ${t.enabled?`enabled`:`disabled`}`);let r=document.getElementById(`ad-connection-status`);r&&(r.textContent=t.connected?`✓ Connected`:`✗ Not Connected`,r.style.color=t.connected?`#10b981`:`#ef4444`);let i=e=>document.getElementById(e);i(`ad-server`)&&(i(`ad-server`).textContent=t.server??`Not configured`),i(`ad-auto-provision`)&&(i(`ad-auto-provision`).textContent=t.auto_provision?`Yes`:`No`),i(`ad-synced-users`)&&(i(`ad-synced-users`).textContent=String(t.synced_users??0));let a=i(`ad-error`);a&&(a.textContent=t.error??`None`,a.style.color=t.error?`#d32f2f`:`#10b981`);let o=i(`ad-sync-btn`);o&&(o.disabled=!(t.enabled&&t.connected))}catch(e){console.error(`Error loading AD status:`,e)}}function vt(){_t(),y(`AD status refreshed`,`success`)}async function yt(){let e=document.getElementById(`ad-sync-btn`);if(!e)return;let t=e.textContent;e.disabled=!0,e.textContent=`Syncing...`;try{let e=await h(`${p()}/api/integrations/ad/sync`,{method:`POST`,headers:m()},mt);if(!e.ok){let t=await e.json().catch(()=>({error:`HTTP ${e.status}`}));throw Error(t.error||`HTTP ${e.status}`)}let t=await e.json();t.success?(y(t.message||`Successfully synced ${t.synced_count} users`,`success`),_t()):y(t.error||`Failed to sync users`,`error`)}catch(e){console.error(`Error syncing AD users:`,e),y((e instanceof Error?e.message:String(e))===`Request timed out`?`AD sync timed out. Check server logs.`:`Error syncing AD users`,`error`)}finally{e.textContent=t,e.disabled=!1}}window.loadDashboard=ht,window.refreshDashboard=gt,window.loadADStatus=_t,window.refreshADStatus=vt,window.syncADUsers=yt;var bt=1e4;async function xt(){let e=document.getElementById(`extensions-table-body`);if(e){e.innerHTML=`<tr><td colspan="7" class="loading">Loading extensions...</td></tr>`;try{let t=await h(`${p()}/api/extensions`,{headers:m()},bt);if(!t.ok)throw Error(`HTTP error! status: ${t.status}`);let n=await t.json();if(window.currentExtensions=n,n.length===0){e.innerHTML=`<tr><td colspan="7" class="loading">No extensions found.</td></tr>`;return}let r=e=>{let t=``;return e.ad_synced&&(t+=` <span class="ad-badge" title="Synced from Active Directory">AD</span>`),e.is_admin&&(t+=` <span class="admin-badge" title="Admin Privileges">Admin</span>`),t};e.innerHTML=n.map(e=>`
        ...
        `).join(``)}catch(t){console.error(`Error loading extensions:`,t),e.innerHTML=`<tr><td colspan="7" class="loading">${(t instanceof Error?t.message:String(t))===`Request timed out`?`Request timed out. System may still be starting.`:`Error loading extensions`}</td></tr>`}}}function St(){let e=document.getElementById(`add-extension-modal`);e&&e.classList.add(`active`);let t=document.getElementById(`add-extension-form`);t&&t.reset()}function Ct(){let e=document.getElementById(`add-extension-modal`);e&&e.classList.remove(`active`)}function wt(e){let t=(window.currentExtensions??[]).find(t=>t.number===e);if(!t)return;let n=e=>document.getElementById(e);n(`edit-ext-number`)&&(n(`edit-ext-number`).value=t.number),n(`edit-ext-name`)&&(n(`edit-ext-name`).value=t.name),n(`edit-ext-email`)&&(n(`edit-ext-email`).value=t.email??``),n(`edit-ext-allow-external`)&&(n(`edit-ext-allow-external`).checked=!!t.allow_external),n(`edit-ext-is-admin`)&&(n(`edit-ext-is-admin`).checked=!!t.is_admin),n(`edit-ext-password`)&&(n(`edit-ext-password`).value=``);let r=document.getElementById(`edit-extension-modal`);r&&r.classList.add(`active`)}function Tt(){let e=document.getElementById(`edit-extension-modal`);e&&e.classList.remove(`active`)}async function Et(e){if(confirm(`Are you sure you want to delete extension ${e}?`))try{let t=p(),n=await fetch(`${t}/api/extensions/${e}`,{method:`DELETE`,headers:m()});n.ok?(y(`Extension deleted successfully`,`success`),xt()):y((await n.json()).error||`Failed to delete extension`,`error`)}catch(e){console.error(`Error deleting extension:`,e),y(`Failed to delete extension`,`error`)}}async function Dt(e){if(confirm(`Reboot phone for extension ${e}?`))try{let t=p();(await fetch(`${t}/api/phones/${e}/reboot`,{method:`POST`,headers:m()})).ok?y(`Reboot command sent to ${e}`,`success`):y(`Failed to reboot phone`,`error`)}catch(e){console.error(`Error rebooting phone:`,e),y(`Failed to reboot phone`,`error`)}}async function Ot(){if(confirm(`Reboot ALL registered phones?`))try{let e=p();(await fetch(`${e}/api/phones/reboot`,{method:`POST`,headers:m()})).ok?y(`Reboot command sent to all phones`,`success`):y(`Failed to reboot phones`,`error`)}catch(e){console.error(`Error rebooting all phones:`,e),y(`Failed to reboot phones`,`error`)}}function kt(){let e=document.getElementById(`add-extension-form`);e&&e.addEventListener(`submit`,async e=>{e.preventDefault();let t=e=>document.getElementById(e)?.value??``,n=e=>document.getElementById(e)?.checked??!1,r={number:t(`new-ext-number`),name:t(`new-ext-name`),email:t(`new-ext-email`),password:t(`new-ext-password`),voicemail_pin:t(`new-ext-voicemail-pin`),allow_external:n(`new-ext-allow-external`),is_admin:n(`new-ext-is-admin`)};try{let e=p(),t=await fetch(`${e}/api/extensions`,{method:`POST`,headers:{...m(),"Content-Type":`application/json`},body:JSON.stringify(r)}),n=await t.json();t.ok&&n.success?(y(`Extension added successfully`,`success`),Ct(),xt()):y(n.error||`Failed to add extension`,`error`)}catch(e){console.error(`Error adding extension:`,e),y(`Failed to add extension`,`error`)}});let t=document.getElementById(`edit-extension-form`);t&&t.addEventListener(`submit`,async e=>{e.preventDefault();let t=e=>document.getElementById(e)?.value??``,n=e=>document.getElementById(e)?.checked??!1,r=t(`edit-ext-number`),i=t(`edit-ext-password`),a=t(`edit-ext-voicemail-pin`),o={name:t(`edit-ext-name`),email:t(`edit-ext-email`),allow_external:n(`edit-ext-allow-external`),is_admin:n(`edit-ext-is-admin`)};i&&(o.password=i),a&&(o.voicemail_pin=a);try{let e=p(),t=await fetch(`${e}/api/extensions/${r}`,{method:`PUT`,headers:{...m(),"Content-Type":`application/json`},body:JSON.stringify(o)}),n=await t.json();t.ok&&n.success?(y(`Extension updated successfully`,`success`),Tt(),xt()):y(n.error||`Failed to update extension`,`error`)}catch(e){console.error(`Error updating extension:`,e),y(`Failed to update extension`,`error`)}})}window.loadExtensions=xt,window.showAddExtensionModal=St,window.closeAddExtensionModal=Ct,window.editExtension=wt,window.closeEditExtensionModal=Tt,window.deleteExtension=Et,window.rebootPhone=Dt,window.rebootAllPhones=Ot,document.readyState===`loading`?document.addEventListener(`DOMContentLoaded`,kt):kt();async function At(){try{let e=p(),t=await fetch(`${e}/api/extensions`,{headers:m()});if(!t.ok)throw Error(`HTTP error! status: ${t.status}`);let n=await t.json(),r=document.getElementById(`vm-extension-select`);if(!r)return;r.innerHTML=`<option value="">Select Extension</option>`;for(let e of n){let t=document.createElement(`option`);t.value=e.number,t.textContent=`${e.number} - ${e.name}`,r.appendChild(t)}}catch(e){console.error(`Error loading voicemail tab:`,e),y(`Failed to load extensions`,`error`)}}async function jt(){let e=document.getElementById(`vm-extension-select`)?.value;if(!e){for(let e of[`voicemail-pin-section`,`voicemail-messages-section`,`voicemail-box-overview`]){let t=document.getElementById(e);t&&(t.style.display=`none`)}return}for(let e of[`voicemail-pin-section`,`voicemail-messages-section`,`voicemail-box-overview`]){let t=document.getElementById(e);t&&(t.style.display=`block`)}let t=document.getElementById(`vm-current-extension`);t&&(t.textContent=e);try{let t=p(),n=await fetch(`${t}/api/voicemail/${e}`,{headers:m()});if(!n.ok)throw Error(`HTTP error! status: ${n.status}`);Mt((await n.json()).messages,e)}catch(e){console.error(`Error loading voicemail:`,e),y(`Failed to load voicemail messages`,`error`)}}function Mt(e,t){let n=document.getElementById(`voicemail-cards-view`);if(n){if(!e||e.length===0){n.innerHTML=`<div class="info-box">No voicemail messages</div>`;return}n.innerHTML=e.map(e=>{let n=new Date(e.timestamp).toLocaleString(),r=e.duration?`${e.duration}s`:`Unknown`,i=!e.listened;return`
        ...
        `}).join(``)}}async function Nt(e,t){try{let n=`${p()}/api/voicemail/${e}/${t}/audio`,r=document.getElementById(`vm-audio-player`);r&&(r.src=n,r.play()),await Ft(e,t)}catch(e){console.error(`Error playing voicemail:`,e),y(`Failed to play voicemail`,`error`)}}async function Pt(e,t){let n=p();window.open(`${n}/api/voicemail/${e}/${t}/audio?download=1`,`_blank`)}async function Ft(e,t){try{let n=p();await fetch(`${n}/api/voicemail/${e}/${t}/read`,{method:`POST`,headers:m()})}catch(e){console.error(`Error marking voicemail read:`,e)}}async function It(e,t){if(confirm(`Delete this voicemail message?`))try{let n=p();(await fetch(`${n}/api/voicemail/${e}/${t}`,{method:`DELETE`,headers:m()})).ok?(y(`Voicemail deleted`,`success`),jt()):y(`Failed to delete voicemail`,`error`)}catch(e){console.error(`Error deleting voicemail:`,e),y(`Failed to delete voicemail`,`error`)}}function Lt(){let e=document.getElementById(`vm-audio-player`);e&&(e.pause(),e.src=``);let t=document.getElementById(`voicemail-player-section`);t&&(t.style.display=`none`)}function Rt(){let e=document.getElementById(`voicemail-cards-view`),t=document.getElementById(`voicemail-table-view`),n=document.getElementById(`toggle-voicemail-view-btn`);if(e&&t){let r=e.style.display!==`none`;e.style.display=r?`none`:`block`,t.style.display=r?`block`:`none`,n&&(n.textContent=r?`Switch to Card View`:`Switch to Table View`)}}window.loadVoicemailTab=At,window.loadVoicemailForExtension=jt,window.playVoicemail=Nt,window.downloadVoicemail=Pt,window.deleteVoicemail=It,window.markVoicemailRead=Ft,window.closeVoicemailPlayer=Lt,window.toggleVoicemailView=Rt;async function zt(){let e=document.getElementById(`calls-list`);if(e){e.innerHTML=`<div class="loading">Loading calls...</div>`;try{let t=await(await h(`${p()}/api/calls`,{headers:m()})).json();if(t.length===0){e.innerHTML=`<div class="loading">No active calls</div>`;return}e.innerHTML=t.map(e=>`
            <div class="call-item"><strong>Call:</strong> ${_(String(e))}</div>
        `).join(``)}catch(t){console.error(`Error loading calls:`,t),e.innerHTML=`<div class="loading">Error loading calls</div>`}}}async function Bt(){try{let e=await h(`${p()}/api/config/codecs`,{headers:m()});if(!e.ok)throw Error(`Failed to load codecs (HTTP ${e.status})`);let t=await e.json(),n=document.getElementById(`codec-status`);n&&(t.codecs&&t.codecs.length>0?n.innerHTML=t.codecs.map(e=>`<div class="codec-item">${_(e.name)} - <span class="status-${e.enabled?`enabled`:`disabled`}">${e.enabled?`Enabled`:`Disabled`}</span></div>`).join(``):n.innerHTML=`<div class="info-box">No codecs configured</div>`)}catch(e){console.error(`Error loading codec status:`,e);let t=document.getElementById(`codec-status`);t&&(t.innerHTML=`<div class="error-box">Failed to load codec configuration</div>`)}}async function Vt(){try{let e=await h(`${p()}/api/config/dtmf`,{headers:m()});if(!e.ok)throw Error(`HTTP ${e.status}`);let t=await e.json(),n=document.getElementById(`dtmf-mode`);n&&(n.value=t.mode??`rfc2833`);let r=document.getElementById(`dtmf-detection-threshold`);r&&(r.value=String(t.threshold??-30))}catch(e){console.error(`Error loading DTMF config:`,e)}}async function Ht(){try{let e=p(),t={mode:document.getElementById(`dtmf-mode`)?.value??`rfc2833`,threshold:parseInt(document.getElementById(`dtmf-detection-threshold`)?.value??`-30`)};(await fetch(`${e}/api/config/dtmf`,{method:`POST`,headers:{...m(),"Content-Type":`application/json`},body:JSON.stringify(t)})).ok?y(`DTMF configuration saved`,`success`):y(`Failed to save DTMF configuration`,`error`)}catch(e){console.error(`Error saving DTMF config:`,e),y(`Failed to save DTMF configuration`,`error`)}}async function Ut(e){e&&e.preventDefault();try{let e=p(),t=document.getElementById(`codec-config-form`);if(!t)return;let n=new FormData(t),r=Object.fromEntries(n.entries());(await fetch(`${e}/api/config/codecs`,{method:`POST`,headers:{...m(),"Content-Type":`application/json`},body:JSON.stringify(r)})).ok?y(`Codec configuration saved`,`success`):y(`Failed to save codec configuration`,`error`)}catch(e){console.error(`Error saving codec config:`,e),y(`Failed to save codec configuration`,`error`)}}window.loadCalls=zt,window.loadCodecStatus=Bt,window.loadDTMFConfig=Vt,window.saveDTMFConfig=Ht,window.saveCodecConfig=Ut;var Wt=`Configuration saved successfully. Restart may be required for some changes.`;async function Gt(){try{let e=await h(`${p()}/api/config/full`,{headers:m()});if(!e.ok)throw Error(`HTTP ${e.status}`);let t=await e.json();if(t.features)for(let e of[`call-recording`,`call-transfer`,`call-hold`,`conference`,`voicemail`,`call-parking`,`call-queues`,`presence`,`music-on-hold`,`auto-attendant`]){let n=document.getElementById(`feature-${e}`),r=e.replace(/-/g,`_`);n&&(n.checked=t.features[r]??!1)}if(t.voicemail){let e=e=>document.getElementById(e);e(`voicemail-max-duration`)&&(e(`voicemail-max-duration`).value=String(t.voicemail.max_duration??120))}}catch(e){console.error(`Error loading config:`,e),y(`Failed to load configuration`,`error`)}}async function Kt(){try{let e=await h(`${p()}/api/config/features`,{headers:m()});if(!e.ok)throw Error(`HTTP ${e.status}`);let t=await e.json();qt(`core-features-table`,t.core),qt(`advanced-features-table`,t.advanced),qt(`integration-features-table`,t.integrations)}catch(e){console.error(`Error loading features status:`,e),y(`Failed to load feature status`,`error`)}}function qt(e,t){let n=document.getElementById(e);if(n){if(!t||Object.keys(t).length===0){n.innerHTML=`<tr><td colspan="3" style="text-align:center;">No features found</td></tr>`;return}n.innerHTML=Object.entries(t).map(([e,t])=>{let n=e.replace(/_/g,` `).replace(/\b\w/g,e=>e.toUpperCase()),r=t.enabled?`<span class="badge" style="background:#10b981;">Enabled</span>`:`<span class="badge" style="background:#6b7280;">Disabled</span>`;return`<tr>
            <td><strong>${_(n)}</strong></td>
            <td>${r}</td>
            <td>${_(t.description)}</td>
        </tr>`}).join(``)}}async function Jt(e){try{let t=p(),n=document.getElementById(`${e}-form`);if(!n)return;let r=new FormData(n),i=Object.fromEntries(r.entries()),a=await fetch(`${t}/api/config/section`,{method:`PUT`,headers:{...m(),"Content-Type":`application/json`},body:JSON.stringify({section:e,data:i})});a.ok?y(Wt,`success`):y((await a.json()).error||`Failed to save configuration`,`error`)}catch(t){console.error(`Error saving ${e} config:`,t),y(`Failed to save ${e} configuration`,`error`)}}async function Yt(){try{let e=await h(`${p()}/api/ssl/status`,{headers:m()});if(!e.ok)throw Error(`HTTP ${e.status}`);let t=await e.json(),n=document.getElementById(`ssl-status-info`);if(n&&(n.textContent=t.enabled?`Enabled`:`Disabled`,n.className=`status-badge ${t.enabled?`enabled`:`disabled`}`),t.certificate){let e=document.getElementById(`cert-details-section`);e&&(e.style.display=`block`);let n=document.getElementById(`cert-subject`),r=document.getElementById(`cert-issuer`),i=document.getElementById(`cert-valid-from`),a=document.getElementById(`cert-valid-until`);n&&(n.value=t.certificate.subject||`N/A`),r&&(r.value=t.certificate.issuer||`N/A`),i&&(i.value=t.certificate.expires||`N/A`),a&&(a.value=t.certificate.expires||`N/A`)}}catch(e){console.error(`Error loading SSL status:`,e)}}async function Xt(){try{let e=p(),t=await fetch(`${e}/api/ssl/generate-certificate`,{method:`POST`,headers:{...m(),"Content-Type":`application/json`},body:JSON.stringify({})});t.ok?(y(`SSL certificate generated successfully. Server restart required.`,`success`),Yt()):y((await t.json()).error||`Failed to generate SSL certificate`,`error`)}catch(e){console.error(`Error generating SSL certificate:`,e),y(`Failed to generate SSL certificate`,`error`)}}async function Zt(){await Yt(),y(`SSL status refreshed`,`success`)}var Qt=[`features-config`,`voicemail-config`,`email-config`,`recording-config`,`security-config`,`advanced-features`,`conference-config`,`ssl-config`];function $t(){for(let e of Qt){let t=document.getElementById(`${e}-form`);t&&t.addEventListener(`submit`,async t=>{t.preventDefault(),await Jt(e)})}}window.loadConfig=Gt,window.loadFeaturesStatus=Kt,window.saveConfigSection=Jt,window.loadSSLStatus=Yt,window.generateSSLCertificate=Xt,window.refreshSSLStatus=Zt,document.readyState===`loading`?document.addEventListener(`DOMContentLoaded`,$t):$t();var en=[],S={};async function tn(){await Promise.all([nn(),rn(),C(),an(),on(),sn()])}async function nn(){try{let e=p(),t=await fetch(`${e}/api/provisioning/vendors`,{headers:m()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json();en=n.vendors||[],S=n.models||{},ln(),un()}catch(e){console.error(`Error loading vendors:`,e);let t=document.getElementById(`supported-vendors-list`);t&&(t.innerHTML=`<p>Failed to load supported vendors.</p>`)}}async function rn(){try{let e=p(),t=await fetch(`${e}/api/extensions`,{headers:m()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json(),r=document.getElementById(`device-extension`);if(!r)return;r.innerHTML=`<option value="">Select Extension</option>`;for(let e of n){let t=document.createElement(`option`);t.value=e.number,t.textContent=`${e.number} - ${e.name}`,r.appendChild(t)}}catch(e){console.error(`Error loading extensions for provisioning:`,e)}}async function C(){try{let e=p(),t=await fetch(`${e}/api/provisioning/devices`,{headers:m()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json(),r=document.getElementById(`provisioning-devices-table-body`);if(!r)return;let i=Array.isArray(n)?n:n.devices||[];if(i.length===0){r.innerHTML=`<tr><td colspan="8">No provisioned devices</td></tr>`;return}r.innerHTML=i.map(e=>`
            <tr>
        ...
        `).join(``)}catch(e){console.error(`Error loading devices:`,e)}}async function an(){try{let e=p(),t=await fetch(`${e}/api/provisioning/templates`,{headers:m()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json(),r=document.getElementById(`templates-table-body`);if(!r)return;let i=n.templates||[];if(i.length===0){r.innerHTML=`<tr><td colspan="5">No templates configured</td></tr>`;return}r.innerHTML=i.map(e=>{let t=e.name||`${e.vendor||`unknown`}_${e.model||`unknown`}`;return`
            ...
            </tr>`}).join(``)}catch(e){console.error(`Error loading templates:`,e)}}async function on(){try{let e=p(),t=await fetch(`${e}/api/provisioning/settings`,{headers:m()});if(!t.ok)return;let n=await t.json(),r=e=>document.getElementById(e);r(`provisioning-enabled`)&&(r(`provisioning-enabled`).checked=n.enabled??!1),r(`provisioning-url-format`)&&(r(`provisioning-url-format`).value=n.url_format??``),fn()}catch(e){console.error(`Error loading provisioning settings:`,e)}}async function sn(){try{let e=p(),t=await fetch(`${e}/api/provisioning/phonebook-settings`,{headers:m()});if(!t.ok)return;let n=await t.json(),r=e=>document.getElementById(e);r(`ldap-phonebook-enabled`)&&(r(`ldap-phonebook-enabled`).checked=n.ldap_enabled??!1),r(`remote-phonebook-enabled`)&&(r(`remote-phonebook-enabled`).checked=n.remote_enabled??!1),mn(),hn()}catch(e){console.error(`Error loading phonebook settings:`,e)}}async function cn(e){if(confirm(`Delete device ${e}?`))try{let t=p();(await fetch(`${t}/api/provisioning/devices/${e}`,{method:`DELETE`,headers:m()})).ok?(y(`Device deleted`,`success`),C()):y(`Failed to delete device`,`error`)}catch(e){console.error(`Error deleting device:`,e),y(`Failed to delete device`,`error`)}}function ln(){let e=document.getElementById(`device-vendor`);if(e){e.innerHTML=`<option value="">Select Vendor</option>`;for(let t of en){let n=document.createElement(`option`);n.value=t,n.textContent=t,e.appendChild(n)}}}function un(){let e=document.getElementById(`supported-vendors-list`);if(e){if(en.length===0){e.innerHTML=`<p>No supported vendors found.</p>`;return}e.innerHTML=en.map(e=>{let t=S[e]||S[e.toLowerCase()]||[],n=t.length>0?t.map(e=>`<span class="badge">${_(e.toUpperCase())}</span>`).join(` `):`<em>No models</em>`;return`<div style="margin-bottom: 8px;">
            <strong>${_(e.charAt(0).toUpperCase()+e.slice(1))}</strong>: ${n}
        </div>`}).join(``)}}function dn(){let e=document.getElementById(`device-vendor`),t=document.getElementById(`device-model`);if(!e||!t)return;let n=e.value;if(t.innerHTML=``,!n){t.innerHTML=`<option value="">Select Vendor First</option>`;return}let r=S[n]||S[n.toLowerCase()]||[];if(r.length===0){t.innerHTML=`<option value="">No models available</option>`;return}t.innerHTML=`<option value="">Select Model</option>`;for(let e of r){let n=document.createElement(`option`);n.value=e,n.textContent=e,t.appendChild(n)}}function fn(){let e=document.getElementById(`provisioning-enabled`),t=document.getElementById(`provisioning-settings`);e&&t&&(t.style.display=e.checked?`block`:`none`)}async function pn(){try{let e=e=>document.getElementById(e)?.value??``,t={enabled:(e=>document.getElementById(e)?.checked??!1)(`provisioning-enabled`),server_ip:e(`provisioning-server-ip`),port:parseInt(e(`provisioning-port`),10)||9e3,custom_templates_dir:e(`provisioning-custom-dir`)},n=p();(await fetch(`${n}/api/provisioning/settings`,{method:`PUT`,headers:{...m(),"Content-Type":`application/json`},body:JSON.stringify(t)})).ok?y(`Provisioning settings saved`,`success`):y(`Failed to save provisioning settings`,`error`)}catch(e){console.error(`Error saving provisioning settings:`,e),y(`Failed to save provisioning settings`,`error`)}}function mn(){let e=document.getElementById(`ldap-phonebook-enabled`),t=document.getElementById(`ldap-phonebook-settings`);e&&t&&(t.style.display=e.checked?`block`:`none`)}function hn(){let e=document.getElementById(`remote-phonebook-enabled`),t=document.getElementById(`remote-phonebook-settings`);e&&t&&(t.style.display=e.checked?`block`:`none`)}async function gn(){try{let e=e=>document.getElementById(e)?.value??``,t=e=>document.getElementById(e)?.checked??!1,n={ldap_enabled:t(`ldap-phonebook-enabled`),ldap_server:e(`ldap-phonebook-server`),ldap_port:parseInt(e(`ldap-phonebook-port`),10)||636,ldap_base_dn:e(`ldap-phonebook-base`),ldap_bind_user:e(`ldap-phonebook-user`),ldap_bind_password:e(`ldap-phonebook-password`),ldap_use_tls:t(`ldap-phonebook-tls`),ldap_display_name:e(`ldap-phonebook-display-name`),remote_enabled:t(`remote-phonebook-enabled`),remote_refresh_interval:parseInt(e(`remote-phonebook-refresh`),10)||60},r=p();(await fetch(`${r}/api/provisioning/phonebook-settings`,{method:`PUT`,headers:{...m(),"Content-Type":`application/json`},body:JSON.stringify(n)})).ok?y(`Phone book settings saved`,`success`):y(`Failed to save phone book settings`,`error`)}catch(e){console.error(`Error saving phonebook settings:`,e),y(`Failed to save phone book settings`,`error`)}}async function _n(){try{let e=p();(await fetch(`${e}/api/provisioning/reload-templates`,{method:`POST`,headers:m()})).ok?(y(`Templates reloaded from disk`,`success`),an()):y(`Failed to reload templates`,`error`)}catch(e){console.error(`Error reloading templates:`,e),y(`Failed to reload templates`,`error`)}}function vn(){let e=document.getElementById(`add-device-form`);e&&e.reset();let t=document.getElementById(`device-model`);t&&(t.innerHTML=`<option value="">Select Vendor First</option>`)}function yn(e){y(`Viewing template: ${e}`,`info`)}function bn(){let e=document.getElementById(`add-device-form`);e&&e.addEventListener(`submit`,async e=>{e.preventDefault();let t=e=>document.getElementById(e)?.value??``,n={mac_address:t(`device-mac`),extension_number:t(`device-extension`),vendor:t(`device-vendor`),model:t(`device-model`)};if(!n.mac_address||!n.extension_number||!n.vendor||!n.model){y(`Please fill in all required fields`,`error`);return}try{let e=p(),t=await fetch(`${e}/api/provisioning/devices`,{method:`POST`,headers:{...m(),"Content-Type":`application/json`},body:JSON.stringify(n)}),r=await t.json();t.ok&&r.success?(y(r.message||`Device added successfully`,`success`),vn(),C()):y(r.error||`Failed to add device`,`error`)}catch(e){console.error(`Error adding device:`,e),y(`Failed to add device`,`error`)}})}window.loadProvisioning=tn,window.loadSupportedVendors=nn,window.loadProvisioningDevices=C,window.loadProvisioningTemplates=an,window.loadProvisioningSettings=on,window.loadPhonebookSettings=sn,window.deleteDevice=cn,window.viewTemplate=yn,window.updateModelOptions=dn,window.toggleProvisioningEnabled=fn,window.saveProvisioningSettings=pn,window.toggleLdapPhonebookSettings=mn,window.toggleRemotePhonebookSettings=hn,window.savePhonebookSettings=gn,window.reloadTemplates=_n,window.resetAddDeviceForm=vn,document.readyState===`loading`?document.addEventListener(`DOMContentLoaded`,bn):bn();async function xn(){let e=document.getElementById(`registered-phones-table-body`);if(e)try{let t=await h(`${p()}/api/registered-phones`,{headers:m()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json();if(n.length===0){e.innerHTML=`<tr><td colspan="5">No registered phones</td></tr>`;return}e.innerHTML=n.map(e=>`
            <tr>
        ...
        `).join(``)}catch(t){console.error(`Error loading registered phones:`,t),e.innerHTML=`<tr><td colspan="5">Error loading phones</td></tr>`}}async function Sn(){let e=document.getElementById(`registered-atas-table-body`);if(e)try{let t=await h(`${p()}/api/registered-phones/atas`,{headers:m()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json();if(n.length===0){e.innerHTML=`<tr><td colspan="6">No registered ATAs</td></tr>`;return}e.innerHTML=n.map(e=>{let t=[e.vendor,e.model].filter(Boolean).join(` `);return`
            <tr>
                <td>${_(e.extension_number||``)}</td>
                <td>${_(e.ip_address||``)}</td>
                <td>${_(e.mac_address||``)}</td>
        ...
        `}).join(``)}catch(t){console.error(`Error loading ATAs:`,t),e.innerHTML=`<tr><td colspan="6">Error loading ATAs</td></tr>`}}window.loadRegisteredPhones=xn,window.loadRegisteredATAs=Sn;async function Cn(){try{let e=p(),t=await fetch(`${e}/api/fraud-detection/alerts`,{headers:m()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json(),r=document.getElementById(`fraud-alerts-list`);if(!r)return;let i=n.alerts??[];if(i.length===0){r.innerHTML=`<div class="info-box">No fraud alerts</div>`;return}r.innerHTML=i.map(e=>`
        ...
        `).join(``)}catch(e){console.error(`Error loading fraud alerts:`,e)}}async function wn(){try{let e=p(),t=await fetch(`${e}/api/callback-queue/list`,{headers:m()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json(),r=document.getElementById(`callback-list`);if(!r)return;let i=n.queue??[];if(i.length===0){r.innerHTML=`<tr><td colspan="5">No callbacks in queue</td></tr>`;return}r.innerHTML=i.map(e=>`
            <tr>
                <td>${_(e.caller||``)}</td>
                <td>${_(e.number||``)}</td>
                <td>${_(e.status||``)}</td>
        ...
        `).join(``)}catch(e){console.error(`Error loading callback queue:`,e)}}async function Tn(e){try{let t=p();(await fetch(`${t}/api/callback-queue/start`,{method:`POST`,headers:{...m(),"Content-Type":`application/json`},body:JSON.stringify({callback_id:e,agent_id:`admin`})})).ok?(y(`Callback initiated`,`success`),wn()):y(`Failed to start callback`,`error`)}catch(e){console.error(`Error starting callback:`,e),y(`Failed to start callback`,`error`)}}async function En(e){try{let t=p();(await fetch(`${t}/api/callback-queue/cancel`,{method:`POST`,headers:{...m(),"Content-Type":`application/json`},body:JSON.stringify({callback_id:e})})).ok?(y(`Callback cancelled`,`success`),wn()):y(`Failed to cancel callback`,`error`)}catch(e){console.error(`Error cancelling callback:`,e),y(`Failed to cancel callback`,`error`)}}async function Dn(){try{let e=p(),t=await fetch(`${e}/api/mobile-push/devices`,{headers:m()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json(),r=document.getElementById(`mobile-devices-list`);if(!r)return;let i=n.devices??[];if(i.length===0){r.innerHTML=`<div class="info-box">No registered devices</div>`;return}r.innerHTML=i.map(e=>`
        ...
        `).join(``)}catch(e){console.error(`Error loading mobile push devices:`,e)}}async function On(){try{let e=p(),t=await fetch(`${e}/api/framework/speech-analytics/configs`,{headers:m()});if(!t.ok)return;let n=await t.json(),r=document.getElementById(`speech-analytics-configs-table`);if(r){let e=n.configs??[];e.length===0?r.innerHTML=`<div class="info-box">No speech analytics configurations</div>`:r.innerHTML=`
                    <table class="table">
                        <thead>
                            <tr>
                                <th>Configuration</th>
                ...
                `}}catch(e){console.error(`Error loading speech analytics:`,e)}}window.loadFraudAlerts=Cn,window.loadCallbackQueue=wn,window.startCallback=Tn,window.cancelCallback=En,window.loadMobilePushDevices=Dn,window.loadSpeechAnalyticsConfigs=On;async function kn(){try{let e=p(),t=await fetch(`${e}/api/emergency/contacts`,{headers:m()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json(),r=document.getElementById(`emergency-contacts-table`);if(!r)return;let i=n.contacts??[];if(i.length===0){r.innerHTML=`<tr><td colspan="5">No emergency contacts</td></tr>`;return}r.innerHTML=i.map(e=>`
            <tr>
                <td>${_(e.name||``)}</td>
                <td>${_(e.phone||``)}</td>
                <td>${_(e.role||``)}</td>
        ...
        `).join(``)}catch(e){console.error(`Error loading emergency contacts:`,e)}}function An(e){return`<span class="status-badge ${{high:`danger`,medium:`warning`,low:`info`}[e??``]??`info`}">${e??`normal`}</span>`}async function jn(){try{let e=p(),t=await fetch(`${e}/api/emergency/history`,{headers:m()});if(!t.ok)return;let n=await t.json(),r=document.getElementById(`emergency-history-table`);if(!r)return;let i=n.history??[];r.innerHTML=i.length===0?`<div class="info-box">No emergency history</div>`:i.map(e=>`
                <div class="history-item">
                    <strong>${new Date(e.timestamp).toLocaleString()}</strong> - ${_(e.description||``)}
                </div>
            `).join(``)}catch(e){console.error(`Error loading emergency history:`,e)}}async function Mn(e){if(confirm(`Delete this emergency contact?`))try{let t=p();(await fetch(`${t}/api/emergency/contacts/${e}`,{method:`DELETE`,headers:m()})).ok&&(y(`Emergency contact deleted`,`success`),kn())}catch(e){console.error(`Error deleting contact:`,e),y(`Failed to delete contact`,`error`)}}async function Nn(){try{let e=p(),t=await fetch(`${e}/api/framework/nomadic-e911/sites`,{headers:m()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json(),r=document.getElementById(`e911-sites-table`);if(!r)return;let i=n.sites??[];r.innerHTML=i.length===0?`<div class="info-box">No E911 sites configured</div>`:i.map(e=>`
                <div class="site-item">
            ...
            `).join(``)}catch(e){console.error(`Error loading E911 sites:`,e);let t=document.getElementById(`e911-sites-table`);t&&(t.innerHTML=`<div class="error-box">Failed to load E911 sites</div>`)}}async function Pn(){try{let e=p(),t=await fetch(`${e}/api/framework/nomadic-e911/locations`,{headers:m()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json(),r=document.getElementById(`extension-locations-table`);if(r){let e=n.locations??[];r.innerHTML=e.length===0?`<div class="info-box">No locations assigned</div>`:e.map(e=>`
                    <div class="location-item">
                        Extension ${_(e.extension)} - ${_(e.site_name||`Unassigned`)}
                    </div>
                `).join(``)}}catch(e){console.error(`Error loading extension locations:`,e)}}function Fn(){let e=document.getElementById(`add-emergency-contact-modal`);e&&e.classList.add(`active`)}function In(){let e=document.getElementById(`add-emergency-contact-modal`);e&&e.classList.remove(`active`)}function Ln(){let e=document.getElementById(`trigger-emergency-modal`);e&&e.classList.add(`active`)}function Rn(){let e=document.getElementById(`trigger-emergency-modal`);e&&e.classList.remove(`active`)}async function zn(e){e.preventDefault();let t=e=>document.getElementById(e)?.value??``,n=e=>document.getElementById(e)?.checked??!1,r=[];n(`method-call`)&&r.push(`call`),n(`method-page`)&&r.push(`page`),n(`method-email`)&&r.push(`email`),n(`method-sms`)&&r.push(`sms`);let i={name:t(`emergency-contact-name`),extension:t(`emergency-contact-extension`),phone:t(`emergency-contact-phone`),email:t(`emergency-contact-email`),priority:t(`emergency-contact-priority`),notification_methods:r};try{let e=p();(await fetch(`${e}/api/emergency/contacts`,{method:`POST`,headers:{...m(),"Content-Type":`application/json`},body:JSON.stringify(i)})).ok?(y(`Emergency contact added`,`success`),In(),kn()):y(`Failed to add emergency contact`,`error`)}catch(e){console.error(`Error adding emergency contact:`,e),y(`Failed to add emergency contact`,`error`)}}async function Bn(e){e.preventDefault();let t=e=>document.getElementById(e)?.value??``,n={type:t(`trigger-type`),details:t(`trigger-details`),info:t(`trigger-info`)};try{let e=p();(await fetch(`${e}/api/emergency/trigger`,{method:`POST`,headers:{...m(),"Content-Type":`application/json`},body:JSON.stringify(n)})).ok?(y(`Emergency notification sent`,`success`),Rn(),jn()):y(`Failed to trigger emergency`,`error`)}catch(e){console.error(`Error triggering emergency:`,e),y(`Failed to trigger emergency`,`error`)}}async function Vn(){try{let e=p();(await fetch(`${e}/api/emergency/test`,{method:`GET`,headers:m()})).ok?y(`Test notification sent successfully`,`success`):y(`Failed to send test notification`,`error`)}catch(e){console.error(`Error testing emergency notification:`,e),y(`Failed to send test notification`,`error`)}}window.loadEmergencyContacts=kn,window.loadEmergencyHistory=jn,window.deleteEmergencyContact=Mn,window.loadE911Sites=Nn,window.loadExtensionLocations=Pn,window.showAddEmergencyContactModal=Fn,window.closeAddEmergencyContactModal=In,window.showTriggerEmergencyModal=Ln,window.closeTriggerEmergencyModal=Rn,window.addEmergencyContact=zn,window.triggerEmergency=Bn,window.testEmergencyNotification=Vn;async function Hn(){try{let e=p(),t=await fetch(`${e}/api/phone-book`,{headers:m()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json(),r=document.getElementById(`phone-book-body`);if(!r)return;let i=n.entries??[];if(i.length===0){r.innerHTML=`<tr><td colspan="5">No phone book entries</td></tr>`;return}r.innerHTML=i.map(e=>`
            <tr>
                <td>${_(e.name||``)}</td>
                <td>${_(e.number||``)}</td>
                <td>${_(e.email||``)}</td>
        ...
        `).join(``)}catch(e){console.error(`Error loading phone book:`,e)}}async function Un(e){if(confirm(`Delete this phone book entry?`))try{let t=p();(await fetch(`${t}/api/phone-book/${e}`,{method:`DELETE`,headers:m()})).ok&&(y(`Phone book entry deleted`,`success`),Hn())}catch(e){console.error(`Error deleting entry:`,e),y(`Failed to delete entry`,`error`)}}window.loadPhoneBook=Hn,window.deletePhoneBookEntry=Un;async function Wn(){await Promise.all([w(),Gn(),Kn()])}async function w(){try{let e=p(),t=await fetch(`${e}/api/paging/zones`,{headers:m()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json(),r=document.getElementById(`paging-zones-table-body`);if(!r)return;let i=n.zones??[];if(i.length===0){r.innerHTML=`<tr><td colspan="4">No paging zones</td></tr>`;return}r.innerHTML=i.map(e=>`
            <tr>
                <td>${_(e.name||``)}</td>
                <td>${_(e.number||``)}</td>
        ...
        `).join(``)}catch(e){console.error(`Error loading paging zones:`,e)}}async function Gn(){try{let e=p(),t=await fetch(`${e}/api/paging/devices`,{headers:m()});if(!t.ok)return;let n=await t.json(),r=document.getElementById(`paging-devices-table-body`);if(r){let e=n.devices??[];r.innerHTML=e.length===0?`<div class="info-box">No paging devices</div>`:e.map(e=>`<div class="device-item">${_(e.name||e.id)}</div>`).join(``)}}catch(e){console.error(`Error loading paging devices:`,e)}}async function Kn(){try{let e=p(),t=await fetch(`${e}/api/paging/active`,{headers:m()});if(!t.ok)return;let n=await t.json(),r=document.getElementById(`active-pages-table-body`);if(r){let e=n.pages??[];r.innerHTML=e.length===0?`<div class="info-box">No active pages</div>`:e.map(e=>`<div class="page-item">${_(e.zone)} - ${_(e.initiator)}</div>`).join(``)}}catch(e){console.error(`Error loading active pages:`,e)}}async function qn(e){if(confirm(`Delete this paging zone?`))try{let t=p();(await fetch(`${t}/api/paging/zones/${e}`,{method:`DELETE`,headers:m()})).ok&&(y(`Paging zone deleted`,`success`),w())}catch(e){console.error(`Error deleting paging zone:`,e),y(`Failed to delete zone`,`error`)}}async function Jn(){let e=prompt(`Zone Extension (e.g., 701):`);if(!e)return;let t=prompt(`Zone Name (e.g., "Warehouse"):`);if(!t)return;let n={extension:e,name:t,description:prompt(`Description (optional):`)??``,device_id:prompt(`Device ID (optional):`)??``};try{let e=p(),r=await(await fetch(`${e}/api/paging/zones`,{method:`POST`,headers:{...m(),"Content-Type":`application/json`},body:JSON.stringify(n)})).json();r.success?(y(`Zone ${t} added successfully`,`success`),w()):y(r.message??`Failed to add zone`,`error`)}catch(e){console.error(`Error adding zone ${t}:`,e),y(`Error adding zone ${t}`,`error`)}}async function Yn(){let e=prompt(`Device ID (e.g., "dac-1"):`);if(!e)return;let t=prompt(`Device Name (e.g., "Main PA System"):`);if(!t)return;let n={device_id:e,name:t,type:prompt(`Device Type (e.g., "sip_gateway"):`)??`sip_gateway`,sip_address:prompt(`SIP Address (e.g., "paging@192.168.1.10:5060"):`)??``};try{let e=p(),r=await(await fetch(`${e}/api/paging/devices`,{method:`POST`,headers:{...m(),"Content-Type":`application/json`},body:JSON.stringify(n)})).json();r.success?(y(`Device ${t} added successfully`,`success`),Gn()):y(r.message??`Failed to add device`,`error`)}catch(e){console.error(`Error adding device ${t}:`,e),y(`Error adding device ${t}`,`error`)}}async function Xn(e){if(confirm(`Delete paging device ${e}?`))try{let t=p(),n=await(await fetch(`${t}/api/paging/devices/${e}`,{method:`DELETE`,headers:m()})).json();n.success?(y(`Device ${e} deleted`,`success`),Gn()):y(n.message??`Failed to delete device`,`error`)}catch(e){console.error(`Error deleting device:`,e),y(`Error deleting device`,`error`)}}window.loadPagingData=Wn,window.loadPagingZones=w,window.loadPagingDevices=Gn,window.loadActivePages=Kn,window.deletePagingZone=qn,window.showAddZoneModal=Jn,window.showAddDeviceModal=Yn,window.deletePagingDevice=Xn;async function T(){let e=document.getElementById(`license-status-container`);if(e){e.innerHTML=`<div class="loading">Loading license information...</div>`;try{let t=await h(`${p()}/api/license/status`,{headers:m()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json();if(n.success&&n.license){let t=n.license,r=t.status===`active`||t.valid?`badge-success`:`badge-danger`,i=t.status||(t.valid?`Active`:`Invalid`);e.innerHTML=`
            ...
            `}else e.innerHTML=`<div class="info-box">No license installed. System is running in open-source mode with all features available.</div>`}catch(t){console.error(`Error loading license status:`,t),e.innerHTML=`<div class="error-message">Failed to load license status</div>`}}}async function E(){let e=document.getElementById(`license-features-container`);if(e){e.innerHTML=`<div class="loading">Loading features...</div>`;try{let t=await h(`${p()}/api/license/features`,{headers:m()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json();if(!n.licensing_enabled){e.innerHTML=`<div class="info-box">Licensing disabled &mdash; all features are available (open-source mode).</div>`;return}let r=n.features??[],i=n.limits??{},a=`<p style="margin-bottom: 10px;"><strong>License Type:</strong> ${(n.license_type||`unknown`).toUpperCase()}</p>`;if(Object.keys(i).length>0){a+=`<h4 style="margin: 15px 0 8px;">Limits</h4><div class="ad-status-grid">`;for(let[e,t]of Object.entries(i))a+=`<div class="ad-status-item"><strong>${e.replace(/_/g,` `)}</strong><span>${t??`Unlimited`}</span></div>`;a+=`</div>`}if(r.length>0){a+=`<h4 style="margin: 15px 0 8px;">Included Features</h4><div style="display: flex; flex-wrap: wrap; gap: 8px;">`;for(let e of r)a+=`<span class="badge badge-success" style="padding: 4px 10px;">${e.replace(/_/g,` `)}</span>`;a+=`</div>`}e.innerHTML=a}catch(t){console.error(`Error loading license features:`,t),e.innerHTML=`<div class="error-message">Failed to load license features</div>`}}}async function Zn(e){e&&e.preventDefault();let t=document.getElementById(`generate-license-result`),n=document.getElementById(`license-type`)?.value,r=document.getElementById(`issued-to`)?.value?.trim(),i=document.getElementById(`expiration-days`)?.value,a=document.getElementById(`max-extensions`)?.value,o=document.getElementById(`max-concurrent-calls`)?.value;if(!n||!r){y(`License type and organization/person are required`,`error`);return}let s={type:n,issued_to:r};i&&(s.expiration_days=parseInt(i,10)),a&&(s.max_extensions=parseInt(a,10)),o&&(s.max_concurrent_calls=parseInt(o,10));try{let e=await(await h(`${p()}/api/license/generate`,{method:`POST`,headers:m(),body:JSON.stringify(s)})).json();if(e.success&&e.license){y(`License generated successfully`,`success`);let n=JSON.stringify(e.license,null,2);t&&(t.innerHTML=`
                ...
                `)}else y(e.error||`Failed to generate license`,`error`),t&&(t.innerHTML=`<div class="error-message">${e.error||`Failed to generate license`}</div>`)}catch(e){console.error(`Error generating license:`,e),y(`Failed to generate license`,`error`),t&&(t.innerHTML=`<div class="error-message">Failed to generate license</div>`)}}async function Qn(){let e=document.getElementById(`generated-license-json`);if(e)try{await er(JSON.parse(e.textContent||`{}`),!1)}catch(e){console.error(`Error auto-installing license:`,e),y(`Failed to install generated license`,`error`)}}async function $n(e){e&&e.preventDefault();let t=document.getElementById(`license-data`),n=document.getElementById(`enforce-licensing`),r=document.getElementById(`install-license-result`);if(!t||!t.value.trim()){y(`Please enter license data (JSON)`,`error`);return}let i;try{i=JSON.parse(t.value.trim())}catch{y(`Invalid JSON format. Please check the license data.`,`error`);return}let a=n?.checked??!1;try{await er(i,a),t.value=``,n&&(n.checked=!1)}catch(e){console.error(`Error installing license:`,e),y(`Failed to install license`,`error`),r&&(r.innerHTML=`<div class="error-message">Failed to install license</div>`)}}async function er(e,t){let n=document.getElementById(`install-license-result`),r=await(await h(`${p()}/api/license/install`,{method:`POST`,headers:m(),body:JSON.stringify({license_data:e,enforce_licensing:t})})).json();r.success?(y(r.message||`License installed successfully`,`success`),n&&(n.innerHTML=`<div class="info-box" style="border-left-color: #4caf50;">${r.message||`License installed successfully`}</div>`),T(),E()):(y(r.error||`Failed to install license`,`error`),n&&(n.innerHTML=`<div class="error-message">${r.error||`Failed to install license`}</div>`))}async function tr(e){let t=document.getElementById(`licensing-toggle-result`),n=e?`enable`:`disable`;if(confirm(`Are you sure you want to ${n} licensing?`))try{let r=await(await h(`${p()}/api/license/toggle`,{method:`POST`,headers:m(),body:JSON.stringify({enabled:e})})).json();r.success?(y(r.message||`Licensing ${n}d successfully`,`success`),t&&(t.innerHTML=`<div class="info-box" style="border-left-color: #4caf50;">${r.message||`Licensing ${n}d successfully`}</div>`),T(),E()):(y(r.error||`Failed to ${n} licensing`,`error`),t&&(t.innerHTML=`<div class="error-message">${r.error||`Failed to ${n} licensing`}</div>`))}catch(e){console.error(`Error toggling licensing:`,e),y(`Failed to ${n} licensing`,`error`),t&&(t.innerHTML=`<div class="error-message">Failed to ${n} licensing</div>`)}}async function nr(){if(!confirm(`Are you sure you want to revoke the current license? This action cannot be undone.`))return;let e=document.getElementById(`revoke-license-result`);try{let t=await(await h(`${p()}/api/license/revoke`,{method:`POST`,headers:m()})).json();t.success?(y(`License revoked successfully`,`success`),e&&(e.innerHTML=`<div class="info-box" style="border-left-color: #4caf50;">License revoked successfully</div>`),T(),E()):(y(t.error||`Failed to revoke license`,`error`),e&&(e.innerHTML=`<div class="error-message">${t.error||`Failed to revoke license`}</div>`))}catch(t){console.error(`Error revoking license:`,t),y(`Failed to revoke license`,`error`),e&&(e.innerHTML=`<div class="error-message">Failed to revoke license</div>`)}}async function rr(){if(!confirm(`Are you sure you want to remove the license lock? This will allow licensing to be disabled.`))return;let e=document.getElementById(`remove-lock-result`);try{let t=await(await h(`${p()}/api/license/remove_lock`,{method:`POST`,headers:m()})).json();t.success?(y(t.message||`License lock removed`,`success`),e&&(e.innerHTML=`<div class="info-box" style="border-left-color: #4caf50;">${t.message||`License lock removed`}</div>`)):(y(t.error||`Failed to remove license lock`,`error`),e&&(e.innerHTML=`<div class="error-message">${t.error||`Failed to remove license lock`}</div>`))}catch(t){console.error(`Error removing license lock:`,t),y(`Failed to remove license lock`,`error`),e&&(e.innerHTML=`<div class="error-message">Failed to remove license lock</div>`)}}function ir(){T(),E()}window.loadLicenseStatus=T,window.loadLicenseFeatures=E,window.generateLicense=Zn,window.autoInstallGeneratedLicense=Qn,window.installLicense=$n,window.toggleLicensing=tr,window.revokeLicense=nr,window.removeLicenseLock=rr,window.initLicenseManagement=ir;var D={};function ar(){return typeof Chart<`u`}async function or(){try{let e=document.getElementById(`analytics-period`)?.value??`7`,t=p(),n=await fetch(`${t}/api/analytics/overview?days=${e}`,{headers:m()});if(!n.ok)throw Error(`HTTP ${n.status}`);let r=await n.json();sr(r),cr(r.top_callers??[]),ar()&&(r.daily_trends&&lr(r.daily_trends),r.hourly_distribution&&ur(r.hourly_distribution),r.disposition&&dr(r.disposition))}catch(e){console.error(`Error loading analytics:`,e)}}function sr(e){let t=e=>document.getElementById(e),n=t(`analytics-total-calls`),r=t(`analytics-avg-duration`),i=t(`analytics-answer-rate`),a=t(`analytics-answered-calls`);n&&(n.textContent=String(e.total_calls??0)),r&&(r.textContent=`${e.avg_duration??0}s`),i&&(i.textContent=`${e.answer_rate??0}%`),a&&(a.textContent=String(e.answered_calls??0))}function cr(e){let t=document.getElementById(`top-callers-table`);if(t){if(e.length===0){t.innerHTML=`<tr><td colspan="4" class="loading">No call data available</td></tr>`;return}t.innerHTML=e.map(e=>`<tr>
            <td>${_(String(e.extension??`Unknown`))}</td>
            <td>${e.calls??0}</td>
            <td>${((e.total_duration??0)/60).toFixed(1)}</td>
            <td>${(e.avg_duration??0).toFixed(1)}</td>
        </tr>`).join(``)}}function lr(e){let t=document.getElementById(`daily-trends-chart`)?.getContext(`2d`);t&&(D.dailyTrends&&D.dailyTrends.destroy(),D.dailyTrends=new Chart(t,{type:`line`,data:{labels:e.labels||[],datasets:[{label:`Calls`,data:e.data||[],borderColor:`#3b82f6`,tension:.3,fill:!1}]},options:{responsive:!0,maintainAspectRatio:!1}}))}function ur(e){let t=document.getElementById(`hourly-distribution-chart`)?.getContext(`2d`);t&&(D.hourlyDist&&D.hourlyDist.destroy(),D.hourlyDist=new Chart(t,{type:`bar`,data:{labels:e.labels||[],datasets:[{label:`Calls by Hour`,data:e.data||[],backgroundColor:`#60a5fa`}]},options:{responsive:!0,maintainAspectRatio:!1}}))}function dr(e){let t=document.getElementById(`disposition-chart`)?.getContext(`2d`);t&&(D.disposition&&D.disposition.destroy(),D.disposition=new Chart(t,{type:`doughnut`,data:{labels:e.labels||[],datasets:[{data:e.data||[],backgroundColor:[`#10b981`,`#ef4444`,`#f59e0b`,`#6b7280`]}]},options:{responsive:!0,maintainAspectRatio:!1}}))}async function fr(){try{let e=p(),t=await fetch(`${e}/api/qos/metrics`,{headers:m()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json(),r=n.active_calls??0,i=n.metrics??[],a=0,o=0;if(i.length>0){let e=i.map(e=>e.mos_score??0).filter(e=>e>0);a=e.length>0?e.reduce((e,t)=>e+t,0)/e.length:0,o=e.filter(e=>e<3.5).length}let s=document.getElementById(`qos-avg-mos`),c=document.getElementById(`qos-active-calls`),l=document.getElementById(`qos-total-calls`),ee=document.getElementById(`qos-calls-with-issues`);s&&(s.textContent=a>0?a.toFixed(2):`N/A`),c&&(c.textContent=String(r)),l&&(l.textContent=String(i.length)),ee&&(ee.textContent=String(o))}catch(e){console.error(`Error loading QoS metrics:`,e)}}async function pr(){try{let e=p();if((await fetch(`${e}/api/qos/clear-alerts`,{method:`POST`,headers:m()})).ok){let e=document.getElementById(`qos-alerts-container`);e&&(e.innerHTML=`<div class="info-box">No quality alerts</div>`),y(`QoS alerts cleared`,`success`)}else y(`Failed to clear QoS alerts`,`error`)}catch(e){console.error(`Error clearing QoS alerts:`,e),y(`Failed to clear QoS alerts`,`error`)}}async function mr(e){e&&e.preventDefault();try{let e=e=>document.getElementById(e)?.value??``,t={mos_min:parseFloat(e(`qos-threshold-mos`))||3.5,jitter_max:parseInt(e(`qos-threshold-jitter`),10)||50,packet_loss_max:parseFloat(e(`qos-threshold-loss`))||2,latency_max:parseInt(e(`qos-threshold-latency`),10)||300},n=p();(await fetch(`${n}/api/qos/thresholds`,{method:`POST`,headers:{...m(),"Content-Type":`application/json`},body:JSON.stringify(t)})).ok?y(`QoS thresholds saved`,`success`):y(`Failed to save QoS thresholds`,`error`)}catch(e){console.error(`Error saving QoS thresholds:`,e),y(`Failed to save QoS thresholds`,`error`)}}window.loadAnalytics=or,window.loadQoSMetrics=fr,window.clearQoSAlerts=pr,window.saveQoSThresholds=mr;function hr(e){return{registered:`<span class="badge" style="background: #10b981;">Registered</span>`,unregistered:`<span class="badge" style="background: #6b7280;">Unregistered</span>`,failed:`<span class="badge" style="background: #ef4444;">Failed</span>`,disabled:`<span class="badge" style="background: #9ca3af;">Disabled</span>`,degraded:`<span class="badge" style="background: #f59e0b;">Degraded</span>`}[e]||e}function gr(e){return{healthy:`<span class="badge" style="background: #10b981;">Healthy</span>`,warning:`<span class="badge" style="background: #f59e0b;">Warning</span>`,critical:`<span class="badge" style="background: #f59e0b;">Critical</span>`,down:`<span class="badge" style="background: #ef4444;">Down</span>`}[e]||e}async function O(){try{let e=await h(`${p()}/api/sip-trunks`,{headers:m()});if(!e.ok)throw Error(`HTTP ${e.status}: ${e.statusText}`);let t=await e.json();if(t.trunks){let e=document.getElementById(`trunk-total`);e&&(e.textContent=String(t.count||0));let n=t.trunks.filter(e=>e.health_status===`healthy`).length,r=t.trunks.filter(e=>e.status===`registered`).length,i=t.trunks.reduce((e,t)=>e+t.channels_available,0),a=document.getElementById(`trunk-healthy`);a&&(a.textContent=String(n));let o=document.getElementById(`trunk-registered`);o&&(o.textContent=String(r));let s=document.getElementById(`trunk-total-channels`);s&&(s.textContent=String(i));let c=document.getElementById(`trunks-list`);if(!c)return;t.trunks.length===0?c.innerHTML=`<tr><td colspan="8" style="text-align: center;">No SIP trunks configured</td></tr>`:c.innerHTML=t.trunks.map(e=>{let t=hr(e.status),n=gr(e.health_status),r=(e.success_rate*100).toFixed(1);return`
                        <tr>
            ...
            `).join(``),y(`Health metrics loaded`,`success`)}}catch(e){console.error(`Error loading trunk health:`,e),y(`Error loading trunk health: ${e instanceof Error?e.message:String(e)}`,`error`)}}function vr(){let e=document.getElementById(`add-trunk-modal`);e&&(e.style.display=`block`)}function yr(){let e=document.getElementById(`add-trunk-modal`);e&&(e.style.display=`none`);let t=document.getElementById(`add-trunk-form`);t&&t.reset()}async function br(e){e.preventDefault();let t=Array.from(document.querySelectorAll(`input[name="trunk-codecs"]:checked`)).map(e=>e.value),n={trunk_id:document.getElementById(`trunk-id`).value,name:document.getElementById(`trunk-name`).value,host:document.getElementById(`trunk-host`).value,port:parseInt(document.getElementById(`trunk-port`).value),username:document.getElementById(`trunk-username`).value,password:document.getElementById(`trunk-password`).value,priority:parseInt(document.getElementById(`trunk-priority`).value),max_channels:parseInt(document.getElementById(`trunk-channels`).value),codec_preferences:t.length>0?t:[`G.711`,`G.729`]};try{let e=await(await h(`${p()}/api/sip-trunks`,{method:`POST`,headers:{...m(),"Content-Type":`application/json`},body:JSON.stringify(n)})).json();e.success?(y(`Trunk ${n.name} added successfully`,`success`),yr(),O()):y(e.error||`Error adding trunk`,`error`)}catch(e){console.error(`Error adding trunk:`,e),y(`Error adding trunk`,`error`)}}async function xr(e,t){if(confirm(`Are you sure you want to delete trunk "${t}"?`))try{let n=await(await h(`${p()}/api/sip-trunks/${e}`,{method:`DELETE`,headers:m()})).json();n.success?(y(`Trunk ${t} deleted`,`success`),O()):y(n.error||`Error deleting trunk`,`error`)}catch(e){console.error(`Error deleting trunk:`,e),y(`Error deleting trunk`,`error`)}}async function Sr(e){y(`Testing trunk...`,`info`);try{let t=await(await h(`${p()}/api/sip-trunks/test`,{method:`POST`,headers:{...m(),"Content-Type":`application/json`},body:JSON.stringify({trunk_id:e})})).json();if(t.success){let e=t.health_status??`unknown`;y(`Trunk test complete: ${e}`,e===`healthy`?`success`:`warning`),O(),_r()}else y(t.error||`Error testing trunk`,`error`)}catch(e){console.error(`Error testing trunk:`,e),y(`Error testing trunk`,`error`)}}async function k(){try{let e=await h(`${p()}/api/lcr/rates`,{headers:m()});if(!e.ok){window.suppressErrorNotifications?debugLog(`LCR rates endpoint returned error:`,e.status,`(feature may not be enabled)`):(console.error(`Error loading LCR rates:`,e.status),y(`Error loading LCR rates`,`error`));return}let t=await e.json();if(t.rates!==void 0){let e=document.getElementById(`lcr-total-rates`);e&&(e.textContent=String(t.count||0));let n=document.getElementById(`lcr-time-rates`);n&&(n.textContent=String(t.time_rates?t.time_rates.length:0));let r=document.getElementById(`lcr-rates-list`);r&&(t.rates.length===0?r.innerHTML=`<tr><td colspan="7" style="text-align: center;">No rates configured</td></tr>`:r.innerHTML=t.rates.map(e=>`
                        <tr>
                    ...
                    `).join(``))}catch(e){if(window.suppressErrorNotifications){let t=e instanceof Error?e.message:String(e);debugLog(`Error loading LCR statistics (expected if LCR not enabled):`,t)}else console.error(`Error loading LCR statistics:`,e)}}function wr(){document.body.insertAdjacentHTML(`beforeend`,`
    ...
    `)}function Tr(){let e=document.getElementById(`lcr-rate-modal`);e&&e.remove()}function Er(e){if(!e||e.trim().length===0)return{valid:!1,error:`Pattern cannot be empty`};try{return new RegExp(e),{valid:!0}}catch(e){return{valid:!1,error:`Invalid regex: ${e instanceof Error?e.message:`Invalid regex pattern`}`}}}async function Dr(e){e.preventDefault();let t=document.getElementById(`lcr-pattern`).value,n=Er(t);if(!n.valid){y(n.error||`Invalid LCR pattern`,`error`);return}let r={trunk_id:document.getElementById(`lcr-trunk-id`).value,pattern:t,description:document.getElementById(`lcr-description`).value,rate_per_minute:parseFloat(document.getElementById(`lcr-rate-per-minute`).value),connection_fee:parseFloat(document.getElementById(`lcr-connection-fee`).value),minimum_seconds:parseInt(document.getElementById(`lcr-minimum-seconds`).value),billing_increment:parseInt(document.getElementById(`lcr-billing-increment`).value)};try{let e=await(await h(`${p()}/api/lcr/rate`,{method:`POST`,headers:{...m(),"Content-Type":`application/json`},body:JSON.stringify(r)})).json();e.success?(y(`LCR rate added successfully`,`success`),Tr(),k()):y(e.error||`Error adding LCR rate`,`error`)}catch(e){console.error(`Error adding LCR rate:`,e),y(`Error adding LCR rate: ${e instanceof Error?e.message:`Unknown error`}`,`error`)}}function Or(){document.body.insertAdjacentHTML(`beforeend`,`
    ...
    `)}function kr(){let e=document.getElementById(`lcr-time-rate-modal`);e&&e.remove()}async function Ar(e){e.preventDefault();let t=Array.from(document.querySelectorAll(`input[name="time-days"]:checked`)).map(e=>parseInt(e.value)),n={name:document.getElementById(`time-rate-name`).value,start_hour:parseInt(document.getElementById(`time-rate-start-hour`).value),start_minute:parseInt(document.getElementById(`time-rate-start-minute`).value),end_hour:parseInt(document.getElementById(`time-rate-end-hour`).value),end_minute:parseInt(document.getElementById(`time-rate-end-minute`).value),days:t,multiplier:parseFloat(document.getElementById(`time-rate-multiplier`).value)};try{let e=await(await h(`${p()}/api/lcr/time-rate`,{method:`POST`,headers:{...m(),"Content-Type":`application/json`},body:JSON.stringify(n)})).json();e.success?(y(`Time-based rate added successfully`,`success`),kr(),k()):y(e.error||`Error adding time-based rate`,`error`)}catch(e){console.error(`Error adding time-based rate:`,e),y(`Error adding time-based rate`,`error`)}}async function jr(){if(confirm(`Are you sure you want to clear all LCR rates? This cannot be undone.`))try{let e=await(await h(`${p()}/api/lcr/clear-rates`,{method:`POST`,headers:{...m(),"Content-Type":`application/json`},body:JSON.stringify({})})).json();e.success?(y(`All LCR rates cleared`,`success`),k()):y(e.error||`Error clearing LCR rates`,`error`)}catch(e){console.error(`Error clearing LCR rates:`,e),y(`Error clearing LCR rates`,`error`)}}window.loadSIPTrunks=O,window.loadTrunkHealth=_r,window.showAddTrunkModal=vr,window.closeAddTrunkModal=yr,window.addSIPTrunk=br,window.deleteTrunk=xr,window.testTrunk=Sr,window.loadLCRRates=k,window.loadLCRStatistics=Cr,window.showAddLCRRateModal=wr,window.closeLCRRateModal=Tr,window.addLCRRate=Dr,window.showAddTimeRateModal=Or,window.closeTimeRateModal=kr,window.addTimeRate=Ar,window.clearLCRRates=jr;var Mr=0;async function Nr(){try{let e=await h(`${p()}/api/fmfm/extensions`,{headers:m()});if(!e.ok)throw Error(`HTTP ${e.status}: ${e.statusText}`);let t=await e.json();if(t.extensions){let e=document.getElementById(`fmfm-total-extensions`);e&&(e.textContent=String(t.count||0));let n=t.extensions.filter(e=>e.mode===`sequential`).length,r=t.extensions.filter(e=>e.mode===`simultaneous`).length,i=t.extensions.filter(e=>e.enabled!==!1).length,a=document.getElementById(`fmfm-sequential`);a&&(a.textContent=String(n));let o=document.getElementById(`fmfm-simultaneous`);o&&(o.textContent=String(r));let s=document.getElementById(`fmfm-active-count`);s&&(s.textContent=String(i));let c=document.getElementById(`fmfm-list`);if(!c)return;t.extensions.length===0?c.innerHTML=`<tr><td colspan="6" style="text-align: center;">No Find Me/Follow Me configurations</td></tr>`:c.innerHTML=t.extensions.map(e=>{let t=e.enabled!==!1,n=e.mode===`sequential`?`<span class="badge" style="background: #3b82f6;">Sequential</span>`:`<span class="badge" style="background: #10b981;">Simultaneous</span>`,r=t?`<span class="badge" style="background: #10b981;">Active</span>`:`<span class="badge" style="background: #6b7280;">Disabled</span>`,i=e.destinations||[],a=i.map(e=>`${_(e.number)}${e.ring_time?` (${e.ring_time}s)`:``}`).join(`, `),o=e.updated_at?new Date(e.updated_at).toLocaleString():`N/A`;return`
                        <tr>
                    ...
                    `}).join(``)}}catch(e){console.error(`Error loading FMFM extensions:`,e),y(`Error loading FMFM configurations`,`error`)}}function Pr(){let e=document.getElementById(`add-fmfm-modal`);e&&(e.style.display=`block`);let t=document.getElementById(`fmfm-extension`);t&&(t.value=``,t.readOnly=!1);let n=document.getElementById(`fmfm-mode`);n&&(n.value=`sequential`);let r=document.getElementById(`fmfm-enabled`);r&&(r.checked=!0);let i=document.getElementById(`fmfm-no-answer`);i&&(i.value=``);let a=document.getElementById(`fmfm-destinations-list`);a&&(a.innerHTML=``),A()}function Fr(){let e=document.getElementById(`add-fmfm-modal`);e&&(e.style.display=`none`);let t=document.getElementById(`add-fmfm-form`);t&&t.reset()}function A(){let e=document.getElementById(`fmfm-destinations-list`);if(!e)return;let t=`fmfm-dest-${Mr++}`,n=document.createElement(`div`);n.id=t,n.style.cssText=`display: flex; gap: 10px; margin-bottom: 10px; align-items: center;`,n.innerHTML=`
    ...
    `,e.appendChild(n)}async function Ir(e){e.preventDefault();let t=document.getElementById(`fmfm-extension`).value,n=document.getElementById(`fmfm-mode`).value,r=document.getElementById(`fmfm-enabled`).checked,i=document.getElementById(`fmfm-no-answer`).value,a=Array.from(document.querySelectorAll(`.fmfm-dest-number`)),o=Array.from(document.querySelectorAll(`.fmfm-dest-ringtime`)),s=a.map((e,t)=>({number:e.value,ring_time:parseInt(o[t]?.value??`20`)||20})).filter(e=>e.number);if(s.length===0){y(`At least one destination is required`,`error`);return}let c={extension:t,mode:n,enabled:r,destinations:s};i&&(c.no_answer_destination=i);try{let e=await(await h(`${p()}/api/fmfm/config`,{method:`POST`,headers:m(),body:JSON.stringify(c)})).json();e.success?(y(`FMFM configured for extension ${t}`,`success`),Fr(),Nr()):y(e.error||`Error configuring FMFM`,`error`)}catch(e){console.error(`Error saving FMFM config:`,e),y(`Error saving FMFM configuration`,`error`)}}function Lr(e){Pr();let t=document.getElementById(`fmfm-extension`);t&&(t.value=e.extension,t.readOnly=!0);let n=document.getElementById(`fmfm-mode`);n&&(n.value=e.mode);let r=document.getElementById(`fmfm-enabled`);r&&(r.checked=e.enabled!==!1);let i=document.getElementById(`fmfm-no-answer`);i&&(i.value=e.no_answer_destination||``);let a=document.getElementById(`fmfm-destinations-list`);if(a)if(a.innerHTML=``,e.destinations&&e.destinations.length>0)for(let t of e.destinations){A();let e=a.children,n=e[e.length-1],r=n.querySelector(`.fmfm-dest-number`);r&&(r.value=t.number);let i=n.querySelector(`.fmfm-dest-ringtime`);i&&(i.value=String(t.ring_time??20))}else A()}async function Rr(e){if(confirm(`Are you sure you want to delete FMFM configuration for extension ${e}?`))try{let t=await(await h(`${p()}/api/fmfm/config/${e}`,{method:`DELETE`,headers:m()})).json();t.success?(y(`FMFM configuration deleted for ${e}`,`success`),Nr()):y(t.error||`Error deleting FMFM configuration`,`error`)}catch(e){console.error(`Error deleting FMFM config:`,e),y(`Error deleting FMFM configuration`,`error`)}}function zr(e){let t=[];if(e.days_of_week){let n=[`Mon`,`Tue`,`Wed`,`Thu`,`Fri`,`Sat`,`Sun`],r=e.days_of_week.map(e=>n[e]).join(`, `);t.push(r)}return e.start_time&&e.end_time&&t.push(`${e.start_time}-${e.end_time}`),e.holidays===!0?t.push(`Holidays`):e.holidays===!1&&t.push(`Non-holidays`),t.length>0?t.join(` | `):`Always`}async function Br(){try{let e=await h(`${p()}/api/time-routing/rules`,{headers:m()});if(!e.ok)throw Error(`HTTP ${e.status}: ${e.statusText}`);let t=await e.json();if(t.rules){let e=document.getElementById(`time-routing-total`);e&&(e.textContent=String(t.count||0));let n=t.rules.filter(e=>e.enabled!==!1).length,r=t.rules.filter(e=>e.name&&(e.name.toLowerCase().includes(`business`)||e.name.toLowerCase().includes(`hours`))).length,i=t.rules.filter(e=>e.name&&(e.name.toLowerCase().includes(`after`)||e.name.toLowerCase().includes(`closed`))).length,a=document.getElementById(`time-routing-active`);a&&(a.textContent=String(n));let o=document.getElementById(`time-routing-business`);o&&(o.textContent=String(r));let s=document.getElementById(`time-routing-after`);s&&(s.textContent=String(i));let c=document.getElementById(`time-routing-list`);if(!c)return;t.rules.length===0?c.innerHTML=`<tr><td colspan="7" style="text-align: center;">No time-based routing rules</td></tr>`:c.innerHTML=t.rules.map(e=>{let t=e.enabled===!1?`<span class="badge" style="background: #6b7280;">Disabled</span>`:`<span class="badge" style="background: #10b981;">Active</span>`,n=zr(e.time_conditions||{});return`
                    ...
                    `}).join(``)}}catch(e){console.error(`Error loading time routing rules:`,e),y(`Error loading time routing rules`,`error`)}}function Vr(){let e=document.getElementById(`add-time-rule-modal`);e&&(e.style.display=`block`)}function Hr(){let e=document.getElementById(`add-time-rule-modal`);e&&(e.style.display=`none`);let t=document.getElementById(`add-time-rule-form`);t&&t.reset()}async function Ur(e){e.preventDefault();let t=document.getElementById(`time-rule-name`).value,n=document.getElementById(`time-rule-destination`).value,r=document.getElementById(`time-rule-route-to`).value,i=document.getElementById(`time-rule-start`).value,a=document.getElementById(`time-rule-end`).value,o=parseInt(document.getElementById(`time-rule-priority`).value),s=document.getElementById(`time-rule-enabled`).checked,c=Array.from(document.querySelectorAll(`input[name="time-rule-days"]:checked`)).map(e=>parseInt(e.value));if(c.length===0){y(`Please select at least one day of the week`,`error`);return}let l={name:t,destination:n,route_to:r,priority:o,enabled:s,time_conditions:{days_of_week:c,start_time:i,end_time:a}};try{let e=await(await h(`${p()}/api/time-routing/rule`,{method:`POST`,headers:m(),body:JSON.stringify(l)})).json();e.success?(y(`Time routing rule "${t}" added successfully`,`success`),Hr(),Br()):y(e.error||`Error adding time routing rule`,`error`)}catch(e){console.error(`Error saving time routing rule:`,e),y(`Error saving time routing rule`,`error`)}}async function Wr(e,t){if(confirm(`Are you sure you want to delete time routing rule "${t}"?`))try{let n=await(await h(`${p()}/api/time-routing/rule/${e}`,{method:`DELETE`,headers:m()})).json();n.success?(y(`Time routing rule "${t}" deleted`,`success`),Br()):y(n.error||`Error deleting time routing rule`,`error`)}catch(e){console.error(`Error deleting time routing rule:`,e),y(`Error deleting time routing rule`,`error`)}}async function Gr(){try{let e=await h(`${p()}/api/webhooks`,{headers:m()});if(!e.ok)throw Error(`HTTP ${e.status}: ${e.statusText}`);let t=await e.json();if(t.subscriptions){let e=document.getElementById(`webhooks-list`);if(!e)return;t.subscriptions.length===0?e.innerHTML=`<tr><td colspan="5" style="text-align: center;">No webhooks configured</td></tr>`:e.innerHTML=t.subscriptions.map(e=>{let t=e.enabled===!1?`<span class="badge" style="background: #6b7280;">Disabled</span>`:`<span class="badge" style="background: #10b981;">Active</span>`,n=(e.event_types||[]).join(`, `),r=e.secret?`Yes`:`No`;return`
                        <tr>
                            <td>
                    ...
                    `}).join(``)}}catch(e){console.error(`Error loading webhooks:`,e),y(`Error loading webhooks`,`error`)}}function Kr(){let e=document.getElementById(`add-webhook-modal`);e&&(e.style.display=`block`)}function qr(){let e=document.getElementById(`add-webhook-modal`);e&&(e.style.display=`none`);let t=document.getElementById(`add-webhook-form`);t&&t.reset()}async function Jr(e){e.preventDefault();let t=document.getElementById(`webhook-url`).value,n=document.getElementById(`webhook-secret`).value,r=document.getElementById(`webhook-enabled`).checked,i=Array.from(document.querySelectorAll(`input[name="webhook-events"]:checked`)).map(e=>e.value);if(i.length===0){y(`Please select at least one event type`,`error`);return}let a={url:t,event_types:i,enabled:r};n&&(a.secret=n);try{let e=await(await h(`${p()}/api/webhooks`,{method:`POST`,headers:m(),body:JSON.stringify(a)})).json();e.success?(y(`Webhook added successfully`,`success`),qr(),Gr()):y(e.error||`Error adding webhook`,`error`)}catch(e){console.error(`Error adding webhook:`,e),y(`Error adding webhook`,`error`)}}async function Yr(e){if(!confirm(`Are you sure you want to delete webhook for ${e}?`))return;let t=encodeURIComponent(e);try{let e=await(await h(`${p()}/api/webhooks/${t}`,{method:`DELETE`,headers:m()})).json();e.success?(y(`Webhook deleted`,`success`),Gr()):y(e.error||`Error deleting webhook`,`error`)}catch(e){console.error(`Error deleting webhook:`,e),y(`Error deleting webhook`,`error`)}}function Xr(e){let t=new Date().getTime()-e.getTime(),n=Math.floor(t/(1e3*60*60)),r=Math.floor(t%(1e3*60*60)/(1e3*60));return n>0?`${n}h ${r}m`:`${r}m`}async function Zr(){try{let e=await h(`${p()}/api/hot-desk/sessions`,{headers:m()});if(!e.ok)throw Error(`HTTP ${e.status}: ${e.statusText}`);let t=await e.json();if(t.sessions){let e=t.sessions.filter(e=>e.active!==!1),n=document.getElementById(`hotdesk-active`);n&&(n.textContent=String(e.length));let r=document.getElementById(`hotdesk-total`);r&&(r.textContent=String(t.sessions.length));let i=document.getElementById(`hotdesk-sessions-list`);if(!i)return;e.length===0?i.innerHTML=`<tr><td colspan="6" style="text-align: center;">No active hot desk sessions</td></tr>`:i.innerHTML=e.map(e=>{let t=e.login_time?new Date(e.login_time).toLocaleString():`N/A`,n=e.login_time?Xr(new Date(e.login_time)):`N/A`;return`
                        <tr>
                            <td><strong>${_(e.extension)}</strong></td>
                    ...
                    `}).join(``)}}catch(e){console.error(`Error loading callback queue:`,e),y(`Error loading callback queue`,`error`)}}function ii(){let e=document.createElement(`div`);e.className=`modal`,e.id=`request-callback-modal`,e.innerHTML=`
        <div class="modal-content">
    ...
    `,document.body.appendChild(e),e.style.display=`block`}function ai(){let e=document.getElementById(`request-callback-modal`);e&&e.remove()}async function oi(e){e.preventDefault();let t=document.getElementById(`callback-queue-id`).value,n=document.getElementById(`callback-caller-number`).value,r=document.getElementById(`callback-caller-name`).value,i=document.getElementById(`callback-preferred-time`).value,a={queue_id:t,caller_number:n};r&&(a.caller_name=r),i&&(a.preferred_time=new Date(i).toISOString());try{let e=await(await h(`${p()}/api/callback-queue/request`,{method:`POST`,headers:m(),body:JSON.stringify(a)})).json();e.success?(y(`Callback requested successfully`,`success`),ai(),j()):y(e.error||`Error requesting callback`,`error`)}catch(e){console.error(`Error requesting callback:`,e),y(`Error requesting callback`,`error`)}}async function si(e){let t=prompt(`Enter your agent ID/extension:`);if(t)try{let n=await(await h(`${p()}/api/callback-queue/start`,{method:`POST`,headers:m(),body:JSON.stringify({callback_id:e,agent_id:t})})).json();n.success?(y(`Started callback to ${n.caller_number??`caller`}`,`success`),j()):y(n.error||`Error starting callback`,`error`)}catch(e){console.error(`Error starting callback:`,e),y(`Error starting callback`,`error`)}}async function ci(e,t){let n=``;t||(n=prompt(`Enter reason for failure (optional):`)||``);try{let r=await(await h(`${p()}/api/callback-queue/complete`,{method:`POST`,headers:m(),body:JSON.stringify({callback_id:e,success:t,notes:n})})).json();r.success?(y(t?`Callback completed`:`Callback will be retried`,`success`),j()):y(r.error||`Error completing callback`,`error`)}catch(e){console.error(`Error completing callback:`,e),y(`Error completing callback`,`error`)}}async function li(e){if(confirm(`Are you sure you want to cancel this callback request?`))try{let t=await(await h(`${p()}/api/callback-queue/cancel`,{method:`POST`,headers:m(),body:JSON.stringify({callback_id:e})})).json();t.success?(y(`Callback cancelled`,`success`),j()):y(t.error||`Error cancelling callback`,`error`)}catch(e){console.error(`Error cancelling callback:`,e),y(`Error cancelling callback`,`error`)}}window.loadFMFMExtensions=Nr,window.showAddFMFMModal=Pr,window.closeAddFMFMModal=Fr,window.addFMFMDestinationRow=A,window.saveFMFMConfig=Ir,window.editFMFMConfig=Lr,window.deleteFMFMConfig=Rr,window.getScheduleDescription=zr,window.showAddTimeRuleModal=Vr,window.closeAddTimeRuleModal=Hr,window.loadTimeRoutingRules=Br,window.saveTimeRoutingRule=Ur,window.deleteTimeRoutingRule=Wr,window.showAddWebhookModal=Kr,window.closeAddWebhookModal=qr,window.loadWebhooks=Gr,window.addWebhook=Jr,window.deleteWebhook=Yr,window.loadHotDeskSessions=Zr,window.logoutHotDesk=Qr,window.getDuration=Xr,window.loadRetentionPolicies=$r,window.showAddRetentionPolicyModal=ei,window.closeAddRetentionPolicyModal=ti,window.addRetentionPolicy=ni,window.deleteRetentionPolicy=ri,window.loadCallbackQueue=j,window.showRequestCallbackModal=ii,window.closeRequestCallbackModal=ai,window.requestCallback=oi,window.startCallback=si,window.completeCallback=ci,window.cancelCallback=li;async function ui(){try{let e=p(),[t,n]=await Promise.all([h(`${e}/api/fraud-detection/alerts?hours=24`,{headers:m()}),h(`${e}/api/fraud-detection/statistics`,{headers:m()})]),[r,i]=await Promise.all([t.json(),n.json()]);if(i){let e=e=>document.getElementById(e);e(`fraud-total-alerts`)&&(e(`fraud-total-alerts`).textContent=String(i.total_alerts??0)),e(`fraud-high-risk`)&&(e(`fraud-high-risk`).textContent=String(i.high_risk_alerts??0)),e(`fraud-blocked-patterns`)&&(e(`fraud-blocked-patterns`).textContent=String(i.blocked_patterns_count??0)),e(`fraud-extensions-flagged`)&&(e(`fraud-extensions-flagged`).textContent=String(i.extensions_flagged??0))}if(r?.alerts){let e=document.getElementById(`fraud-alerts-list`);e&&(r.alerts.length===0?e.innerHTML=`<tr><td colspan="5" style="text-align: center;">No fraud alerts detected</td></tr>`:e.innerHTML=r.alerts.map(e=>{let t=new Date(e.timestamp).toLocaleString(),n=e.fraud_score>.8?`#ef4444`:e.fraud_score>.5?`#f59e0b`:`#10b981`,r=(e.fraud_score*100).toFixed(0),i=(e.alert_types??[]).join(`, `);return`
                            <tr>
                    ...
                    `).join(``))}}catch(e){console.error(`Error loading fraud detection data:`,e),y(`Error loading fraud detection data`,`error`)}}function di(){let e=document.getElementById(`add-blocked-pattern-modal`);e&&(e.style.display=`block`)}function fi(){let e=document.getElementById(`add-blocked-pattern-modal`);e&&(e.style.display=`none`);let t=document.getElementById(`add-blocked-pattern-form`);t&&t.reset()}async function pi(e){e.preventDefault();let t=document.getElementById(`blocked-pattern`),n=document.getElementById(`blocked-reason`),r=t?.value??``,i=n?.value??``;try{new RegExp(r)}catch(e){y(`Invalid regex pattern: ${e instanceof Error?e.message:String(e)}`,`error`);return}let a={pattern:r,reason:i};try{let e=await(await h(`${p()}/api/fraud-detection/blocked-pattern`,{method:`POST`,headers:m(),body:JSON.stringify(a)})).json();e.success?(y(`Blocked pattern added successfully`,`success`),fi(),ui()):y(e.error??`Error adding blocked pattern`,`error`)}catch(e){console.error(`Error adding blocked pattern:`,e),y(`Error adding blocked pattern`,`error`)}}async function mi(e,t){if(confirm(`Are you sure you want to unblock pattern "${t}"?`))try{let t=await(await h(`${p()}/api/fraud-detection/blocked-pattern/${e}`,{method:`DELETE`,headers:m()})).json();t.success?(y(`Blocked pattern removed`,`success`),ui()):y(t.error??`Error removing blocked pattern`,`error`)}catch(e){console.error(`Error removing blocked pattern:`,e),y(`Error removing blocked pattern`,`error`)}}async function M(){try{let e=p(),[t,n]=await Promise.all([h(`${e}/api/callback-queue/list`,{headers:m()}),h(`${e}/api/callback-queue/statistics`,{headers:m()})]),[r,i]=await Promise.all([t.json(),n.json()]);if(i){let e=e=>document.getElementById(e);e(`callback-total`)&&(e(`callback-total`).textContent=String(i.total_callbacks??0));let t=i.status_breakdown??{};e(`callback-scheduled`)&&(e(`callback-scheduled`).textContent=String(t.scheduled??0)),e(`callback-in-progress`)&&(e(`callback-in-progress`).textContent=String(t.in_progress??0)),e(`callback-completed`)&&(e(`callback-completed`).textContent=String(t.completed??0)),e(`callback-failed`)&&(e(`callback-failed`).textContent=String(t.failed??0))}if(r?.callbacks){let e=document.getElementById(`callback-list`);e&&(r.callbacks.length===0?e.innerHTML=`<tr><td colspan="8" style="text-align: center;">No callbacks in queue</td></tr>`:e.innerHTML=r.callbacks.map(e=>{let t=new Date(e.requested_at).toLocaleString(),n=new Date(e.callback_time).toLocaleString(),r=``;switch(e.status){case`scheduled`:r=`badge-info`;break;case`in_progress`:r=`badge-warning`;break;case`completed`:r=`badge-success`;break;case`failed`:r=`badge-danger`;break;case`cancelled`:r=`badge-secondary`;break;default:r=`badge-info`}return`
                            <tr>
                                <td><code>${_(e.callback_id)}</code></td>
                                <td>${_(e.queue_id)}</td>
                                <td>
                        ...
                        `}).join(``))}}catch(e){console.error(`Error loading callback queue:`,e),y(`Error loading callback queue`,`error`)}}function hi(){let e=document.createElement(`div`);e.className=`modal`,e.id=`request-callback-modal`,e.innerHTML=`
        <div class="modal-content">
            <span class="close" onclick="closeRequestCallbackModal()">&times;</span>
            <h2>Request Callback</h2>
            <form id="request-callback-form" onsubmit="requestCallback(event)">
    ...
    `,document.body.appendChild(e),e.style.display=`block`}function gi(){let e=document.getElementById(`request-callback-modal`);e&&e.remove()}async function _i(e){e.preventDefault();let t=document.getElementById(`callback-queue-id`)?.value??``,n=document.getElementById(`callback-caller-number`)?.value??``,r=document.getElementById(`callback-caller-name`)?.value??``,i=document.getElementById(`callback-preferred-time`)?.value??``,a={queue_id:t,caller_number:n};r&&(a.caller_name=r),i&&(a.preferred_time=new Date(i).toISOString());try{let e=await(await h(`${p()}/api/callback-queue/request`,{method:`POST`,headers:m(),body:JSON.stringify(a)})).json();e.success?(y(`Callback requested successfully`,`success`),gi(),M()):y(e.error??`Error requesting callback`,`error`)}catch(e){console.error(`Error requesting callback:`,e),y(`Error requesting callback`,`error`)}}async function vi(e){let t=prompt(`Enter your agent ID/extension:`);if(t)try{let n=await(await h(`${p()}/api/callback-queue/start`,{method:`POST`,headers:m(),body:JSON.stringify({callback_id:e,agent_id:t})})).json();n.success?(y(`Started callback to ${n.caller_number??e}`,`success`),M()):y(n.error??`Error starting callback`,`error`)}catch(e){console.error(`Error starting callback:`,e),y(`Error starting callback`,`error`)}}async function yi(e,t){let n=``;t||(n=prompt(`Enter reason for failure (optional):`)??``);try{let r=await(await h(`${p()}/api/callback-queue/complete`,{method:`POST`,headers:m(),body:JSON.stringify({callback_id:e,success:t,notes:n})})).json();r.success?(y(t?`Callback completed`:`Callback will be retried`,`success`),M()):y(r.error??`Error completing callback`,`error`)}catch(e){console.error(`Error completing callback:`,e),y(`Error completing callback`,`error`)}}async function bi(e){if(confirm(`Are you sure you want to cancel this callback request?`))try{let t=await(await h(`${p()}/api/callback-queue/cancel`,{method:`POST`,headers:m(),body:JSON.stringify({callback_id:e})})).json();t.success?(y(`Callback cancelled`,`success`),M()):y(t.error??`Error cancelling callback`,`error`)}catch(e){console.error(`Error cancelling callback:`,e),y(`Error cancelling callback`,`error`)}}async function N(){try{let e=p(),[t,n,r]=await Promise.all([h(`${e}/api/mobile-push/devices`,{headers:m()}),h(`${e}/api/mobile-push/statistics`,{headers:m()}),h(`${e}/api/mobile-push/history`,{headers:m()})]),[i,a,o]=await Promise.all([t.json(),n.json(),r.json()]);if(a){let e=e=>document.getElementById(e);e(`push-total-devices`)&&(e(`push-total-devices`).textContent=String(a.total_devices??0)),e(`push-total-users`)&&(e(`push-total-users`).textContent=String(a.total_users??0));let t=a.platforms??{};e(`push-ios-devices`)&&(e(`push-ios-devices`).textContent=String(t.ios??0)),e(`push-android-devices`)&&(e(`push-android-devices`).textContent=String(t.android??0)),e(`push-recent-notifications`)&&(e(`push-recent-notifications`).textContent=String(a.recent_notifications??0))}if(i?.devices){let e=document.getElementById(`mobile-devices-list`);e&&(i.devices.length===0?e.innerHTML=`<tr><td colspan="5" style="text-align: center;">No devices registered</td></tr>`:e.innerHTML=i.devices.map(e=>{let t=new Date(e.registered_at).toLocaleString(),n=new Date(e.last_seen).toLocaleString(),r=``;return r=e.platform===`ios`?`<span class="badge badge-info">iOS</span>`:e.platform===`android`?`<span class="badge badge-success">Android</span>`:`<span class="badge badge-secondary">${_(e.platform)}</span>`,`
                            <tr>
                        ...
                        `}).join(``))}if(o?.history){let e=document.getElementById(`push-history-list`);e&&(o.history.length===0?e.innerHTML=`<tr><td colspan="5" style="text-align: center;">No notifications sent</td></tr>`:e.innerHTML=o.history.slice(0,50).map(e=>{let t=new Date(e.sent_at).toLocaleString(),n=e.success_count??0,r=e.failure_count??0;return`
                            ...
                            `}).join(``))}}catch(e){console.error(`Error loading mobile push data:`,e),y(`Error loading mobile push data`,`error`)}}function xi(){let e=document.createElement(`div`);e.className=`modal`,e.id=`register-device-modal`,e.innerHTML=`
        <div class="modal-content">
    ...
    `,document.body.appendChild(e),e.style.display=`block`}function Si(){let e=document.getElementById(`register-device-modal`);e&&e.remove()}async function Ci(e){e.preventDefault();let t={user_id:document.getElementById(`device-user-id`)?.value??``,device_token:document.getElementById(`device-token`)?.value.trim()??``,platform:document.getElementById(`device-platform`)?.value??``};try{let e=await(await h(`${p()}/api/mobile-push/register`,{method:`POST`,headers:m(),body:JSON.stringify(t)})).json();e.success?(y(`Device registered successfully`,`success`),Si(),N()):y(e.error??`Error registering device`,`error`)}catch(e){console.error(`Error registering device:`,e),y(`Error registering device`,`error`)}}function wi(){let e=document.createElement(`div`);e.className=`modal`,e.id=`test-notification-modal`,e.innerHTML=`
        <div class="modal-content">
    ...
    `,document.body.appendChild(e),e.style.display=`block`}function Ti(){let e=document.getElementById(`test-notification-modal`);e&&e.remove()}function Ei(e){e.preventDefault(),Di(document.getElementById(`test-user-id`)?.value??``),Ti()}async function Di(e){try{let t=await(await h(`${p()}/api/mobile-push/test`,{method:`POST`,headers:m(),body:JSON.stringify({user_id:e})})).json();t.success||t.stub_mode?(t.stub_mode?y(`Test notification logged (Firebase not configured)`,`warning`):y(`Test notification sent: ${t.success_count??0} succeeded, ${t.failure_count??0} failed`,`success`),N()):y(t.error??`Error sending test notification`,`error`)}catch(e){console.error(`Error sending test notification:`,e),y(`Error sending test notification`,`error`)}}async function Oi(){try{let e=p(),[t,n]=await Promise.all([h(`${e}/api/recording-announcements/statistics`,{headers:m()}),h(`${e}/api/recording-announcements/config`,{headers:m()})]),[r,i]=await Promise.all([t.json(),n.json()]);if(r){let e=e=>document.getElementById(e);e(`announcements-enabled`)&&(e(`announcements-enabled`).textContent=r.enabled?`Enabled`:`Disabled`),e(`announcements-played`)&&(e(`announcements-played`).textContent=String(r.announcements_played??0)),e(`consent-accepted`)&&(e(`consent-accepted`).textContent=String(r.consent_accepted??0)),e(`consent-declined`)&&(e(`consent-declined`).textContent=String(r.consent_declined??0)),e(`announcement-type`)&&(e(`announcement-type`).textContent=r.announcement_type??`N/A`),e(`require-consent`)&&(e(`require-consent`).textContent=r.require_consent?`Yes`:`No`)}if(i){let e=e=>document.getElementById(e);e(`audio-file-path`)&&(e(`audio-file-path`).textContent=i.audio_path??`N/A`),e(`announcement-text`)&&(e(`announcement-text`).textContent=i.announcement_text??`N/A`)}}catch(e){console.error(`Error loading recording announcements data:`,e),y(`Error loading recording announcements data`,`error`)}}async function ki(){try{let e=await(await h(`${p()}/api/framework/speech-analytics/configs`,{headers:m()})).json(),t=document.getElementById(`speech-analytics-configs-table`);if(!t)return;if(!e.configs||e.configs.length===0){t.innerHTML=`<tr><td colspan="5" class="loading">No extension-specific configurations. Using system defaults.</td></tr>`;return}t.innerHTML=e.configs.map(e=>`
            <tr>
                <td>${_(e.extension)}</td>
        ...
        `).join(``)}catch(e){console.error(`Error loading speech analytics configs:`,e),y(`Error loading speech analytics configurations`,`error`)}}async function Ai(){try{let e=await h(`${p()}/api/framework/integrations/activity-log`,{headers:m()});if(!e.ok){console.error(`Error loading CRM activity log:`,e.status),y(`Error loading CRM activity log`,`error`);return}let t=await e.json(),n=document.getElementById(`crm-activity-log-table`);if(!n)return;if(!t.activities||t.activities.length===0){n.innerHTML=`<tr><td colspan="5" class="loading">No integration activity yet</td></tr>`;return}n.innerHTML=t.activities.map(e=>{let t=e.status===`success`?`success`:`error`,n=e.status===`success`?`OK`:`FAIL`;return`
            ...
            `}).join(``)}catch(e){console.error(`Error loading CRM activity log:`,e),y(`Error loading CRM activity log`,`error`)}}async function ji(){if(confirm(`Clear old activity log entries? This will remove entries older than 30 days.`))try{let e=await(await h(`${p()}/api/framework/integrations/activity-log/clear`,{method:`POST`,headers:m()})).json();e.success?(y(`Cleared ${e.deleted_count??0} old entries`,`success`),Ai()):y(e.error??`Error clearing activity log`,`error`)}catch(e){console.error(`Error clearing CRM activity log:`,e),y(`Error clearing activity log`,`error`)}}async function Mi(){try{let e=p(),[t,n,r]=await Promise.all([h(`${e}/api/framework/compliance/soc2/controls`,{headers:m()}),h(`${e}/api/framework/compliance/gdpr/consents`,{headers:m()}),h(`${e}/api/framework/compliance/pci/audit-log`,{headers:m()})]),[i,a,o]=await Promise.all([t.ok?t.json():null,n.ok?n.json():null,r.ok?r.json():null]),s=document.getElementById(`compliance-soc2-count`);if(s&&i){let e=i.controls??[];s.textContent=String(e.length)}let c=document.getElementById(`compliance-gdpr-count`);if(c&&a){let e=a.consents??[];c.textContent=String(e.length)}let l=document.getElementById(`compliance-pci-count`);if(l&&o){let e=o.entries??o.audit_log??[];l.textContent=String(e.length)}}catch(e){console.error(`Error loading compliance data:`,e)}}window.loadFraudAlerts=ui,window.showAddBlockedPatternModal=di,window.closeAddBlockedPatternModal=fi,window.addBlockedPattern=pi,window.deleteBlockedPattern=mi,window.loadCallbackQueue=M,window.showRequestCallbackModal=hi,window.closeRequestCallbackModal=gi,window.requestCallback=_i,window.startCallback=vi,window.completeCallback=yi,window.cancelCallback=bi,window.loadMobilePushDevices=N,window.showRegisterDeviceModal=xi,window.closeRegisterDeviceModal=Si,window.registerDevice=Ci,window.showTestNotificationModal=wi,window.closeTestNotificationModal=Ti,window.sendTestNotificationForm=Ei,window.sendTestNotification=Di,window.loadRecordingAnnouncementsStats=Oi,window.loadSpeechAnalyticsConfigs=ki,window.loadCRMActivityLog=Ai,window.clearCRMActivityLog=ji,window.loadFraudDetectionData=ui,window.loadMobilePushConfig=N,window.loadRecordingAnnouncements=Oi,window.loadComplianceData=Mi;var Ni={completed:`success`,failed:`error`,cancelled:`warning`,busy:`warning`,"no-answer":`warning`};async function Pi(){try{let e=await h(`${p()}/api/extensions`,{headers:m()});if(!e.ok)throw Error(`HTTP ${e.status}`);return(await e.json()).extensions??[]}catch(e){return console.error(`Error loading extensions for click-to-dial:`,e),window.currentExtensions??[]}}async function Fi(){try{let e=await(await h(`${p()}/api/framework/click-to-dial/configs`,{headers:m()})).json();if(e.error){console.error(`Error loading click-to-dial configs:`,e.error);return}let t=await Pi(),n=document.getElementById(`ctd-extension-select`),r=document.getElementById(`ctd-history-extension`);if(n&&t.length>0){n.innerHTML=`<option value="">Select Extension</option>`;for(let e of t){let t=document.createElement(`option`);t.value=e.number,t.textContent=`${e.number} - ${e.name}`,n.appendChild(t)}}else n&&(n.innerHTML=`<option value="">No extensions available</option>`);if(r&&t.length>0){r.innerHTML=`<option value="">All Extensions</option>`;for(let e of t){let t=document.createElement(`option`);t.value=e.number,t.textContent=`${e.number} - ${e.name}`,r.appendChild(t)}}else r&&(r.innerHTML=`<option value="">No extensions available</option>`);let i=document.getElementById(`ctd-configs-table`);if(!i)return;if(!e.configs||e.configs.length===0){i.innerHTML=`<tr><td colspan="6" style="text-align: center;">No configurations found. Configure extensions above.</td></tr>`;return}i.innerHTML=e.configs.map(e=>`
            <tr>
                <td>${_(e.extension)}</td>
        ...
        `).join(``)}catch(e){console.error(`Error loading click-to-dial configs:`,e),e instanceof Error&&b(e,`Loading click-to-dial configurations`)}}function Ii(e){let t=document.getElementById(`ctd-config-section`),n=document.getElementById(`ctd-no-extension`);t&&n&&(t.style.display=e?`block`:`none`,n.style.display=e?`none`:`block`)}async function Li(){let e=document.getElementById(`ctd-extension-select`)?.value;if(!e){Ii(!1);return}try{let t=await(await h(`${p()}/api/framework/click-to-dial/config/${e}`,{headers:m()})).json();if(t.error){console.error(`Error loading config:`,t.error);let n=document.getElementById(`ctd-current-extension`);n&&(n.textContent=e);let r=document.getElementById(`ctd-enabled`);r&&(r.checked=!0);let i=document.getElementById(`ctd-caller-id`);i&&(i.value=``);let a=document.getElementById(`ctd-auto-answer`);a&&(a.checked=!1);let o=document.getElementById(`ctd-browser-notification`);o&&(o.checked=!0)}else{let n=document.getElementById(`ctd-current-extension`);n&&(n.textContent=e);let r=document.getElementById(`ctd-enabled`);r&&(r.checked=t.config.enabled);let i=document.getElementById(`ctd-caller-id`);i&&(i.value=t.config.default_caller_id??``);let a=document.getElementById(`ctd-auto-answer`);a&&(a.checked=t.config.auto_answer);let o=document.getElementById(`ctd-browser-notification`);o&&(o.checked=t.config.browser_notification)}Ii(!0)}catch(e){console.error(`Error loading click-to-dial config:`,e),e instanceof Error&&b(e,`Loading click-to-dial configuration`)}}async function Ri(e){e.preventDefault();let t=document.getElementById(`ctd-current-extension`)?.textContent;if(!t){y(`No extension selected`,`error`);return}let n={enabled:document.getElementById(`ctd-enabled`)?.checked??!1,default_caller_id:document.getElementById(`ctd-caller-id`)?.value.trim()||null,auto_answer:document.getElementById(`ctd-auto-answer`)?.checked??!1,browser_notification:document.getElementById(`ctd-browser-notification`)?.checked??!1};try{let e=await(await h(`${p()}/api/framework/click-to-dial/config/${t}`,{method:`POST`,headers:m(),body:JSON.stringify(n)})).json();e.error?y(`Error: ${e.error}`,`error`):(y(`Configuration saved successfully`,`success`),Fi())}catch(e){console.error(`Error saving config:`,e),e instanceof Error&&b(e,`Saving click-to-dial configuration`),y(`Error saving configuration`,`error`)}}async function zi(e){let t=document.getElementById(`ctd-extension-select`);if(t){t.value=e,await Li();let n=document.getElementById(`ctd-config-section`);n&&n.scrollIntoView({behavior:`smooth`})}}async function Bi(){let e=document.getElementById(`ctd-extension-select`)?.value,t=document.getElementById(`ctd-phone-number`),n=t?.value.trim();if(!e){y(`Please select an extension`,`error`);return}if(!n){y(`Please enter a phone number`,`error`);return}try{let r=await(await h(`${p()}/api/framework/click-to-dial/call/${e}`,{method:`POST`,headers:m(),body:JSON.stringify({destination:n})})).json();r.error?y(`Error: ${r.error}`,`error`):(y(`Call initiated from extension ${e} to ${n}`,`success`),t&&(t.value=``),setTimeout(()=>Vi(),1e3))}catch(e){console.error(`Error initiating call:`,e),e instanceof Error&&b(e,`Initiating click-to-dial call`),y(`Error initiating call`,`error`)}}async function Vi(){let e=document.getElementById(`ctd-history-extension`)?.value,t=document.getElementById(`ctd-history-table`);if(t){if(!e){t.innerHTML=`<tr><td colspan="5" style="text-align: center;">Select an extension to view history</td></tr>`;return}try{let n=await(await h(`${p()}/api/framework/click-to-dial/history/${e}`,{headers:m()})).json();if(n.error){t.innerHTML=`<tr><td colspan="5" style="text-align: center;">Error: ${_(n.error)}</td></tr>`;return}if(!n.history||n.history.length===0){t.innerHTML=`<tr><td colspan="5" style="text-align: center;">No call history found</td></tr>`;return}t.innerHTML=n.history.map(e=>{let t=new Date(e.timestamp).toLocaleString(),n=e.duration?`${e.duration}s`:`-`,r=Ni[e.status]??`warning`;return`
                <tr>
                    <td>${_(t)}</td>
                    <td>${_(e.extension)}</td>
                    <td>${_(e.destination)}</td>
            ...
            `}).join(``)}catch(e){console.error(`Error loading history:`,e),e instanceof Error&&b(e,`Loading click-to-dial history`),t&&(t.innerHTML=`<tr><td colspan="5" style="text-align: center;">Error loading history</td></tr>`)}}}async function Hi(){try{let e=await(await h(`${p()}/api/webrtc/phone-config`,{headers:m()})).json();if(e.success){let t=document.getElementById(`webrtc-phone-extension`);t&&(t.value=e.extension??(typeof DEFAULT_WEBRTC_EXTENSION<`u`?DEFAULT_WEBRTC_EXTENSION:``)),typeof initWebRTCPhone==`function`&&initWebRTCPhone()}else console.error(`Failed to load WebRTC phone config:`,e.error)}catch(e){console.error(`Error loading WebRTC phone config:`,e)}}async function Ui(e){e.preventDefault();let t=document.getElementById(`webrtc-phone-extension`)?.value.trim()??``;if(!t){y(`Please enter an extension`,`error`);return}try{let e=await(await h(`${p()}/api/webrtc/phone-config`,{method:`POST`,headers:m(),body:JSON.stringify({extension:t})})).json();e.success?(y(`Phone extension saved successfully! Reloading phone...`,`success`),typeof initWebRTCPhone==`function`&&initWebRTCPhone()):y(`Error: ${e.error??`Failed to save phone extension`}`,`error`)}catch(e){console.error(`Error saving WebRTC phone config:`,e),y(`Error: ${e instanceof Error?e.message:String(e)}`,`error`)}}window.loadClickToDialConfigs=Fi,window.toggleClickToDialConfigSections=Ii,window.loadClickToDialConfig=Li,window.saveClickToDialConfig=Ri,window.editClickToDialConfig=zi,window.initiateClickToDial=Bi,window.loadClickToDialHistory=Vi,window.loadWebRTCPhoneConfig=Hi,window.saveWebRTCPhoneConfig=Ui,window.loadClickToDialTab=Fi;async function P(){try{let e=await(await h(`${p()}/api/framework/nomadic-e911/sites`,{headers:m()})).json(),t=document.getElementById(`e911-sites-table`);if(!t)return;if(!e.sites||e.sites.length===0){t.innerHTML=`<tr><td colspan="5" class="loading">No E911 sites configured</td></tr>`;return}t.innerHTML=e.sites.map(e=>`
            <tr>
                <td>${_(e.site_name)}</td>
        ...
        `).join(``)}catch(e){console.error(`Error loading extension locations:`,e),y(`Error loading extension locations`,`error`)}}async function Gi(){let e=document.getElementById(`location-history-extension`)?.value??``,t=e?`/api/framework/nomadic-e911/history/${e}`:`/api/framework/nomadic-e911/history`;try{let e=await(await h(`${p()}${t}`,{headers:m()})).json(),n=document.getElementById(`location-history-table`);if(!n)return;if(!e.history||e.history.length===0){n.innerHTML=`<tr><td colspan="5" class="loading">No location history available</td></tr>`;return}n.innerHTML=e.history.map(e=>`
            <tr>
                <td>${_(new Date(e.timestamp).toLocaleString())}</td>
                <td>${_(e.extension)}</td>
        ...
        `).join(``)}catch(e){console.error(`Error loading location history:`,e),y(`Error loading location history`,`error`)}}function F(){let e=document.getElementById(`e911-site-modal`);e&&e.remove()}function Ki(e){return`
    ...
    `}async function qi(e){let t=new FormData(e),n=t.get(`site_id`),r={};for(let[e,n]of t.entries())e!==`site_id`&&String(n).trim()&&(r[e]=String(n).trim());try{let e=p(),t=await(await h(n?`${e}/api/framework/nomadic-e911/sites/${n}`:`${e}/api/framework/nomadic-e911/create-site`,{method:n?`PUT`:`POST`,headers:m(),body:JSON.stringify(r)})).json();t.success?(y(`E911 site saved successfully`,`success`),F(),await P()):y(t.error??`Error saving E911 site`,`error`)}catch(e){console.error(`Error saving E911 site:`,e),y(`Error saving E911 site`,`error`)}}function Ji(){F(),document.body.insertAdjacentHTML(`beforeend`,Ki());let e=document.getElementById(`e911-site-form`);e.onsubmit=t=>{t.preventDefault(),qi(e)}}function Yi(e){h(`${p()}/api/framework/nomadic-e911/sites`,{headers:m()}).then(async t=>{let n=(await t.json()).sites?.find(t=>t.id===e);if(!n){y(`Site not found`,`error`);return}F(),document.body.insertAdjacentHTML(`beforeend`,Ki(n));let r=document.getElementById(`e911-site-form`);r.onsubmit=e=>{e.preventDefault(),qi(r)}}).catch(()=>{y(`Error loading site details`,`error`)})}async function Xi(e){if(confirm(`Delete E911 site ${e}?`))try{let t=await(await h(`${p()}/api/framework/nomadic-e911/sites/${e}`,{method:`DELETE`,headers:m()})).json();t.success?(y(`E911 site deleted`,`success`),await P()):y(t.error??`Error deleting site`,`error`)}catch(e){console.error(`Error deleting E911 site:`,e),y(`Error deleting E911 site`,`error`)}}function I(){let e=document.getElementById(`location-update-modal`);e&&e.remove()}function Zi(e){return`
    ...
    `}async function Qi(e){let t=new FormData(e),n=String(t.get(`extension`)??``).trim();if(!n){y(`Extension is required`,`error`);return}let r={};for(let[e,n]of t.entries())e!==`extension`&&String(n).trim()&&(r[e]=String(n).trim());try{let e=await(await h(`${p()}/api/framework/nomadic-e911/update-location/${encodeURIComponent(n)}`,{method:`POST`,headers:m(),body:JSON.stringify(r)})).json();e.success?(y(`Location updated for extension ${n}`,`success`),I(),await Wi()):y(e.error??`Error updating location`,`error`)}catch(e){console.error(`Error updating location:`,e),y(`Error updating location`,`error`)}}function $i(){I(),document.body.insertAdjacentHTML(`beforeend`,Zi());let e=document.getElementById(`location-update-form`);e.onsubmit=t=>{t.preventDefault(),Qi(e)}}function ea(e){I(),document.body.insertAdjacentHTML(`beforeend`,Zi(e));let t=document.getElementById(`location-update-form`);t.onsubmit=e=>{e.preventDefault(),Qi(t)}}function L(){let e=document.getElementById(`speech-config-modal`);e&&e.remove()}function ta(e,t){return`
    ...
    `}async function na(e){let t=new FormData(e),n=String(t.get(`extension`)??``).trim();if(!n){y(`Extension is required`,`error`);return}let r={enabled:t.get(`enabled`)===`on`,transcription_enabled:t.get(`transcription_enabled`)===`on`,sentiment_enabled:t.get(`sentiment_enabled`)===`on`,summarization_enabled:t.get(`summarization_enabled`)===`on`,keywords:String(t.get(`keywords`)??``).trim(),alert_threshold:parseFloat(String(t.get(`alert_threshold`)??`0.7`))};try{let e=await(await h(`${p()}/api/framework/speech-analytics/config/${encodeURIComponent(n)}`,{method:`POST`,headers:m(),body:JSON.stringify(r)})).json();e.success?(y(`Speech analytics config saved`,`success`),L()):y(e.error??`Error saving config`,`error`)}catch(e){console.error(`Error saving speech analytics config:`,e),y(`Error saving speech analytics config`,`error`)}}function ra(){L(),document.body.insertAdjacentHTML(`beforeend`,ta());let e=document.getElementById(`speech-config-form`);e.onsubmit=t=>{t.preventDefault(),na(e)}}function ia(e){L(),document.body.insertAdjacentHTML(`beforeend`,ta(e));let t=document.getElementById(`speech-config-form`);t.onsubmit=e=>{e.preventDefault(),na(t)}}async function aa(e){if(confirm(`Delete speech analytics config for extension ${e}?`))try{let t=await(await h(`${p()}/api/framework/speech-analytics/config/${encodeURIComponent(e)}`,{method:`DELETE`,headers:m()})).json();t.success?y(`Speech analytics config deleted`,`success`):y(t.error??`Error deleting config`,`error`)}catch(e){console.error(`Error deleting speech analytics config:`,e),y(`Error deleting speech analytics config`,`error`)}}window.loadE911Sites=P,window.loadExtensionLocations=Wi,window.loadLocationHistory=Gi,window.showAddE911SiteModal=Ji,window.editE911Site=Yi,window.deleteE911Site=Xi,window.removeE911SiteModal=F,window.showUpdateLocationModal=$i,window.updateExtensionLocation=ea,window.removeLocationModal=I,window.showAddSpeechAnalyticsConfigModal=ra,window.editSpeechAnalyticsConfig=ia,window.deleteSpeechAnalyticsConfig=aa,window.removeSpeechConfigModal=L,window.loadNomadicE911Data=function(){P(),Wi()};async function oa(){await Promise.all([R(),sa(),ca(),la(),ua()])}async function R(){try{let e=await h(`${p()}/api/framework/sbc/statistics`,{headers:m()},1e4);if(!e.ok)throw Error(`HTTP ${e.status}`);let t=await e.json();z(`sbc-active-sessions`,String(t.active_sessions)),z(`sbc-total-sessions`,String(t.total_sessions)),z(`sbc-blocked-requests`,String(t.blocked_requests)),z(`sbc-relayed-media`,String(t.relayed_media_mb)),z(`sbc-bandwidth-util`,`${t.bandwidth_utilization_pct}%`),z(`sbc-rate-violations`,String(t.rate_limit_violations)),z(`sbc-cac-rejections`,String(t.cac_rejections)),z(`sbc-port-pool`,`${t.relay_port_pool_size}/${t.relay_port_pool_total}`)}catch(e){console.error(`Error loading SBC statistics:`,e),y(`Failed to load SBC statistics: ${e instanceof Error?e.message:String(e)}`,`error`)}}async function sa(){try{let e=await h(`${p()}/api/framework/sbc/config`,{headers:m()},1e4);if(!e.ok)throw Error(`HTTP ${e.status}`);let t=await e.json();B(`sbc-cfg-enabled`,String(t.enabled)),B(`sbc-cfg-topology-hiding`,String(t.topology_hiding)),B(`sbc-cfg-media-relay`,String(t.media_relay)),B(`sbc-cfg-stun`,String(t.stun_enabled)),V(`sbc-cfg-public-ip`,t.public_ip),V(`sbc-cfg-max-calls`,String(t.max_calls)),V(`sbc-cfg-max-bandwidth`,String(t.max_bandwidth)),V(`sbc-cfg-rate-limit`,String(t.rate_limit))}catch(e){console.error(`Error loading SBC config:`,e)}}async function ca(){try{let e=await h(`${p()}/api/framework/sbc/relays`,{headers:m()},1e4);if(!e.ok)throw Error(`HTTP ${e.status}`);let t=await e.json(),n=document.getElementById(`sbc-relays-list`);if(!n)return;let r=Object.values(t.relays??{});if(r.length===0){n.innerHTML=`<tr><td colspan="7" style="text-align: center;">No active relays</td></tr>`;return}n.innerHTML=r.map(e=>`
      <tr>
    ...
    `).join(``)}catch(e){console.error(`Error loading SBC relays:`,e);let t=document.getElementById(`sbc-relays-list`);t&&(t.innerHTML=`<tr><td colspan="7" style="text-align: center;">Error loading relays</td></tr>`)}}async function la(){try{let e=await h(`${p()}/api/framework/sbc/blacklist`,{headers:m()},1e4);if(!e.ok)throw Error(`HTTP ${e.status}`);va(`sbc-blacklist-table`,(await e.json()).blacklist,`removeSBCBlacklist`)}catch(e){console.error(`Error loading SBC blacklist:`,e)}}async function ua(){try{let e=await h(`${p()}/api/framework/sbc/whitelist`,{headers:m()},1e4);if(!e.ok)throw Error(`HTTP ${e.status}`);va(`sbc-whitelist-table`,(await e.json()).whitelist,`removeSBCWhitelist`)}catch(e){console.error(`Error loading SBC whitelist:`,e)}}async function da(){try{let e=p(),t={enabled:H(`sbc-cfg-enabled`)===`true`,topology_hiding:H(`sbc-cfg-topology-hiding`)===`true`,media_relay:H(`sbc-cfg-media-relay`)===`true`,stun_enabled:H(`sbc-cfg-stun`)===`true`,public_ip:U(`sbc-cfg-public-ip`),max_calls:parseInt(U(`sbc-cfg-max-calls`)||`1000`,10),max_bandwidth:parseInt(U(`sbc-cfg-max-bandwidth`)||`100000`,10),rate_limit:parseInt(U(`sbc-cfg-rate-limit`)||`100`,10)},n=await h(`${e}/api/framework/sbc/config`,{method:`PUT`,headers:{...m(),"Content-Type":`application/json`},body:JSON.stringify(t)},1e4);if(!n.ok)throw Error(`HTTP ${n.status}`);y(`Warden SBC configuration saved`,`success`),await R()}catch(e){console.error(`Error saving SBC config:`,e),y(`Failed to save SBC configuration`,`error`)}}async function fa(e){if(confirm(`Terminate relay for call ${e}?`))try{let t=await h(`${p()}/api/framework/sbc/relay/${encodeURIComponent(e)}`,{method:`DELETE`,headers:m()},1e4);if(!t.ok)throw Error(`HTTP ${t.status}`);y(`Relay terminated`,`success`),await ca(),await R()}catch(e){console.error(`Error terminating relay:`,e),y(`Failed to terminate relay`,`error`)}}async function pa(){let e=document.getElementById(`sbc-blacklist-ip`),t=e?.value?.trim();if(!t){y(`Enter an IP address`,`warning`);return}try{let n=await h(`${p()}/api/framework/sbc/blacklist`,{method:`POST`,headers:{...m(),"Content-Type":`application/json`},body:JSON.stringify({ip:t})},1e4);if(!n.ok)throw Error(`HTTP ${n.status}`);e&&(e.value=``),y(`${t} added to blacklist`,`success`),await la(),await R()}catch(e){console.error(`Error adding to blacklist:`,e),y(`Failed to add to blacklist`,`error`)}}async function ma(e){try{let t=await h(`${p()}/api/framework/sbc/blacklist/${encodeURIComponent(e)}`,{method:`DELETE`,headers:m()},1e4);if(!t.ok)throw Error(`HTTP ${t.status}`);y(`${e} removed from blacklist`,`success`),await la(),await R()}catch(e){console.error(`Error removing from blacklist:`,e),y(`Failed to remove from blacklist`,`error`)}}async function ha(){let e=document.getElementById(`sbc-whitelist-ip`),t=e?.value?.trim();if(!t){y(`Enter an IP address`,`warning`);return}try{let n=await h(`${p()}/api/framework/sbc/whitelist`,{method:`POST`,headers:{...m(),"Content-Type":`application/json`},body:JSON.stringify({ip:t})},1e4);if(!n.ok)throw Error(`HTTP ${n.status}`);e&&(e.value=``),y(`${t} added to whitelist`,`success`),await ua(),await R()}catch(e){console.error(`Error adding to whitelist:`,e),y(`Failed to add to whitelist`,`error`)}}async function ga(e){try{let t=await h(`${p()}/api/framework/sbc/whitelist/${encodeURIComponent(e)}`,{method:`DELETE`,headers:m()},1e4);if(!t.ok)throw Error(`HTTP ${t.status}`);y(`${e} removed from whitelist`,`success`),await ua(),await R()}catch(e){console.error(`Error removing from whitelist:`,e),y(`Failed to remove from whitelist`,`error`)}}async function _a(){let e=U(`sbc-nat-local-ip`),t=U(`sbc-nat-public-ip`);if(!e||!t){y(`Enter both local and public IP addresses`,`warning`);return}try{let n=await h(`${p()}/api/framework/sbc/nat-detect`,{method:`POST`,headers:{...m(),"Content-Type":`application/json`},body:JSON.stringify({local_ip:e,public_ip:t})},15e3);if(!n.ok)throw Error(`HTTP ${n.status}`);let r=await n.json(),i=document.getElementById(`sbc-nat-result`),a=document.getElementById(`sbc-nat-type`);i&&(i.style.display=`block`),a&&(a.textContent=r.nat_type),y(`NAT type detected: ${r.nat_type}`,`success`)}catch(e){console.error(`Error detecting NAT:`,e),y(`NAT detection failed`,`error`)}}function z(e,t){let n=document.getElementById(e);n&&(n.textContent=t)}function B(e,t){let n=document.getElementById(e);n&&(n.value=t)}function V(e,t){let n=document.getElementById(e);n&&(n.value=t)}function H(e){return document.getElementById(e)?.value??``}function U(e){return document.getElementById(e)?.value?.trim()??``}function va(e,t,n){let r=document.getElementById(e);if(r){if(t.length===0){r.innerHTML=`<tr><td colspan="2" style="text-align: center;">No entries</td></tr>`;return}r.innerHTML=t.map(e=>`
    <tr>
      <td><code>${_(e)}</code></td>
      <td>
  ...
  `).join(``)}}window.loadSBCData=oa,window.saveSBCConfig=da,window.terminateSBCRelay=fa,window.addSBCBlacklist=pa,window.removeSBCBlacklist=ma,window.addSBCWhitelist=ha,window.removeSBCWhitelist=ga,window.detectSBCNat=_a,window.fetchWithTimeout=h,window.getAuthHeaders=m,window.getApiBaseUrl=p,window.DEFAULT_FETCH_TIMEOUT=We,window.store=g,window.showNotification=y,window.displayError=b,window.setSuppressErrorNotifications=rt,window.showTab=x,window.switchTab=x,window.initializeTabs=ut,window.escapeHtml=_,window.copyToClipboard=Ge,window.formatDate=Ke,window.truncate=qe,window.getDuration=Je,window.getStatusBadge=Ye,window.getHealthBadge=Xe,window.getPriorityBadge=Ze,window.getQualityClass=Qe,window.getScheduleDescription=$e,window.downloadLicense=et,window.executeBatched=dt,window.refreshAllData=ft;var W=`/admin/login.html`;async function ya(){if(debugLog(`Initializing user context...`),!localStorage.getItem(`pbx_token`)){debugLog(`No authentication token found, redirecting to login...`),window.location.replace(W);return}try{let e=await h(`${p()}/api/extensions`,{headers:m()},5e3);if(e.status===401||e.status===403){debugLog(`Authentication token is invalid, redirecting to login...`),localStorage.removeItem(`pbx_token`),localStorage.removeItem(`pbx_extension`),localStorage.removeItem(`pbx_is_admin`),localStorage.removeItem(`pbx_name`),window.location.replace(W);return}if(!e.ok)throw Error(`HTTP ${e.status}`)}catch(e){console.error(`Error verifying authentication:`,e),y(`Unable to verify authentication - server may be starting up`,`error`)}let e=localStorage.getItem(`pbx_extension`),t=localStorage.getItem(`pbx_is_admin`)===`true`,n=localStorage.getItem(`pbx_name`)||`User`;if(!e){debugLog(`No extension number found, redirecting to login...`),window.location.replace(W);return}g.set(`currentUser`,{number:e,is_admin:t,name:n}),debugLog(`User context initialized:`,{number:e,is_admin:t,name:n}),t?(debugLog(`Admin user - showing dashboard tab`),x(`dashboard`)):(debugLog(`Regular user - showing webrtc-phone tab`),x(`webrtc-phone`)),debugLog(`User context initialization complete`)}function ba(){let e=document.querySelectorAll(`form[data-ajax]`);for(let t of e)t.addEventListener(`submit`,e=>{e.preventDefault(),debugLog(`Ajax form submitted:`,t.id)})}function xa(){let e=document.getElementById(`logout-button`);e&&e.addEventListener(`click`,async()=>{let e=localStorage.getItem(`pbx_token`),t=m();localStorage.removeItem(`pbx_token`),localStorage.removeItem(`pbx_extension`),localStorage.removeItem(`pbx_is_admin`),localStorage.removeItem(`pbx_name`),localStorage.removeItem(`pbx_current_extension`);try{e&&await fetch(`${p()}/api/auth/logout`,{method:`POST`,headers:t})}catch(e){console.error(`Logout API error:`,e)}window.location.href=W})}async function Sa(){let e=document.getElementById(`connection-status`);if(!e){console.error(`Connection status badge element not found`);return}try{if((await h(`${p()}/api/status`,{headers:m()},5e3)).ok)e.textContent=`Connected`,e.classList.remove(`connecting`,`disconnected`),e.classList.add(`connected`);else throw Error(`Connection failed`)}catch(t){console.error(`Connection check failed:`,t),e.textContent=`Disconnected`,e.classList.remove(`connecting`,`connected`),e.classList.add(`disconnected`)}}document.addEventListener(`DOMContentLoaded`,async()=>{debugLog(`DOMContentLoaded event fired - starting initialization`),Sa(),setInterval(Sa,1e4),debugLog(`Initializing tabs, forms, and logout`),ut(),ba(),xa(),pt();try{await ya(),debugLog(`User context initialization complete`)}catch(e){console.error(`User context initialization failed:`,e)}debugLog(`Page initialization complete`)});async function Ca(){try{let e=await fetch(`${API_BASE}/api/auto-attendant/config`,{headers:pbxAuthHeaders()});if(e.ok){let t=await e.json();document.getElementById(`aa-enabled`).checked=t.enabled??!1,document.getElementById(`aa-extension`).value=t.extension??`0`,document.getElementById(`aa-timeout`).value=t.timeout??10,document.getElementById(`aa-max-retries`).value=t.max_retries??3}await Ia(),await G(),await wa()}catch(e){console.error(`Error loading auto attendant config:`,e),showNotification(`Failed to load auto attendant configuration`,`error`)}}async function wa(){try{let e=await fetch(`${API_BASE}/api/auto-attendant/prompts`,{headers:pbxAuthHeaders()});if(!e.ok){debugWarn(`Failed to load prompts, using defaults`);return}let t=await e.json(),n=t.prompts??{},r=t.company_name??``,i=document.getElementById(`aa-company-name`);i&&(i.value=r),n.welcome&&(document.getElementById(`aa-prompt-welcome`).value=n.welcome),n.main_menu&&(document.getElementById(`aa-prompt-main-menu`).value=n.main_menu),n.invalid&&(document.getElementById(`aa-prompt-invalid`).value=n.invalid),n.timeout&&(document.getElementById(`aa-prompt-timeout`).value=n.timeout),n.transferring&&(document.getElementById(`aa-prompt-transferring`).value=n.transferring)}catch(e){console.error(`Error loading prompts:`,e)}}async function G(){try{let e=await fetch(`${API_BASE}/api/auto-attendant/menus/${q}/items`,{headers:pbxAuthHeaders()});if(e.status===404)return debugWarn(`Menu items endpoint returned 404 for menu '${q}', trying legacy API...`),await Ta();if(!e.ok){let t=await Fa(e);throw Error(`Failed to load menu options: ${e.status} - ${t.error||e.statusText}`)}let t=await e.json(),n=document.getElementById(`aa-menu-options-table-body`);if(Oa(),!t.items||t.items.length===0){n.innerHTML=`<tr><td colspan="5" class="no-data">No menu options configured. Click "Add Menu Option" to get started.</td></tr>`;return}let r=t.items.sort((e,t)=>e.digit===t.digit?0:e.digit===`*`?1:t.digit===`*`?-1:e.digit===`#`?1:t.digit===`#`?-1:e.digit.localeCompare(t.digit));n.innerHTML=``;for(let e of r){let t=document.createElement(`tr`),r=document.createElement(`td`);r.innerHTML=`<strong>${K(e.digit)}</strong>`,t.appendChild(r);let i=document.createElement(`td`);i.innerHTML=`${Ea(e.destination_type)} ${e.destination_type}`,t.appendChild(i);let a=document.createElement(`td`);e.destination_type===`submenu`?a.innerHTML=`<span style="color: #4CAF50; font-weight: bold;">${K(e.destination_value)}</span>`:a.textContent=e.destination_value,t.appendChild(a);let o=document.createElement(`td`);o.textContent=e.description,t.appendChild(o);let s=document.createElement(`td`);if(e.destination_type===`submenu`){let t=document.createElement(`button`);t.className=`btn btn-secondary`,t.textContent=`➡️ Open`,t.onclick=()=>Da(e.destination_value),s.appendChild(t)}let c=document.createElement(`button`);c.className=`btn btn-primary`,c.textContent=`✏️ Edit`,c.onclick=()=>ja(e.digit,e.destination_type,e.destination_value,e.description),s.appendChild(c);let l=document.createElement(`button`);l.className=`btn btn-danger`,l.textContent=`🗑️ Delete`,l.onclick=()=>Na(e.digit),s.appendChild(l),t.appendChild(s),n.appendChild(t)}}catch(e){console.error(`Error loading menu options:`,e),showNotification(`Failed to load menu options`,`error`)}}async function Ta(){try{let e=await fetch(`${API_BASE}/api/auto-attendant/menu-options`,{headers:pbxAuthHeaders()});if(!e.ok)throw Error(`Failed to load menu options`);let t=await e.json(),n=document.getElementById(`aa-menu-options-table-body`);if(!t.menu_options||t.menu_options.length===0){n.innerHTML=`<tr><td colspan="5" class="no-data">No menu options configured.</td></tr>`;return}let r=t.menu_options.sort((e,t)=>e.digit===t.digit?0:e.digit===`*`?1:t.digit===`*`?-1:e.digit===`#`?1:t.digit===`#`?-1:e.digit.localeCompare(t.digit));n.innerHTML=``;for(let e of r){let t=document.createElement(`tr`),r=document.createElement(`td`);r.innerHTML=`<strong>${K(e.digit)}</strong>`,t.appendChild(r);let i=document.createElement(`td`);i.innerHTML=`📞 extension`,t.appendChild(i);let a=document.createElement(`td`);a.textContent=e.destination,t.appendChild(a);let o=document.createElement(`td`);o.textContent=e.description,t.appendChild(o);let s=document.createElement(`td`),c=document.createElement(`button`);c.className=`btn btn-primary`,c.textContent=`✏️ Edit`,c.onclick=()=>ja(e.digit,`extension`,e.destination,e.description),s.appendChild(c);let l=document.createElement(`button`);l.className=`btn btn-danger`,l.textContent=`🗑️ Delete`,l.onclick=()=>Na(e.digit),s.appendChild(l),t.appendChild(s),n.appendChild(t)}}catch(e){throw console.error(`Error loading legacy menu options:`,e),e}}function Ea(e){return{extension:`📞`,submenu:`📁`,queue:`👥`,voicemail:`📧`,operator:`🎧`}[e]??`❓`}async function Da(e){q=e,await G()}async function Oa(){try{let e=await fetch(`${API_BASE}/api/auto-attendant/menus/${q}`,{headers:pbxAuthHeaders()});if(e.ok){let t=await e.json(),n=document.getElementById(`breadcrumb-path`);if(n){let e=K(t.menu.menu_name);q!==`main`&&(e=`<button class="btn btn-secondary" onclick="navigateToMenu('main')" style="margin-right: 10px;">⬅️ Back to Main</button> ${e}`),n.innerHTML=e}}}catch(e){console.error(`Error updating breadcrumb:`,e)}}document.addEventListener(`DOMContentLoaded`,function(){let e=document.getElementById(`auto-attendant-config-form`);e&&e.addEventListener(`submit`,async function(e){e.preventDefault();let t={enabled:document.getElementById(`aa-enabled`).checked,extension:document.getElementById(`aa-extension`).value,timeout:parseInt(document.getElementById(`aa-timeout`).value,10),max_retries:parseInt(document.getElementById(`aa-max-retries`).value,10)};try{let e=await fetch(`${API_BASE}/api/auto-attendant/config`,{method:`PUT`,headers:pbxAuthHeaders(),body:JSON.stringify(t)});if(e.ok)showNotification(`Auto attendant configuration saved successfully`,`success`);else{let t=await e.json();showNotification(t.error||`Failed to save configuration`,`error`)}}catch(e){console.error(`Error saving auto attendant config:`,e),showNotification(`Failed to save configuration`,`error`)}})});function ka(){document.getElementById(`add-menu-option-modal`).classList.add(`active`),document.getElementById(`add-menu-option-form`).reset()}function Aa(){document.getElementById(`add-menu-option-modal`).classList.remove(`active`)}function ja(e,t,n,r){document.getElementById(`edit-menu-digit`).value=e,document.getElementById(`edit-menu-digit-display`).textContent=e,document.getElementById(`edit-menu-dest-type`).value=t??`extension`,document.getElementById(`edit-menu-description`).value=r,Ra(`edit`),t===`submenu`?document.getElementById(`edit-menu-submenu`).value=n:document.getElementById(`edit-menu-destination`).value=n,document.getElementById(`edit-menu-option-modal`).classList.add(`active`)}function Ma(){document.getElementById(`edit-menu-option-modal`).classList.remove(`active`)}async function Na(e){if(confirm(`Are you sure you want to delete menu option for digit "${e}"?`))try{let t=await fetch(`${API_BASE}/api/auto-attendant/menus/${q}/items/${e}`,{method:`DELETE`,headers:pbxAuthHeaders()});if(t.ok)showNotification(`Menu option deleted successfully`,`success`),G();else{let e=await t.json();showNotification(e.error||`Failed to delete menu option`,`error`)}}catch(e){console.error(`Error deleting menu option:`,e),showNotification(`Failed to delete menu option`,`error`)}}document.addEventListener(`DOMContentLoaded`,function(){let e=document.getElementById(`add-menu-option-form`);e&&e.addEventListener(`submit`,async function(e){e.preventDefault();let t=document.getElementById(`new-menu-dest-type`).value,n;n=t===`submenu`?document.getElementById(`new-menu-submenu`).value:document.getElementById(`new-menu-destination`).value;let r={digit:document.getElementById(`new-menu-digit`).value,destination_type:t,destination_value:n,description:document.getElementById(`new-menu-description`).value};try{let e=await fetch(`${API_BASE}/api/auto-attendant/menus/${q}/items`,{method:`POST`,headers:pbxAuthHeaders(),body:JSON.stringify(r)});if(e.ok)showNotification(`Menu option added successfully`,`success`),Aa(),G();else{let t=await e.json();showNotification(t.error||`Failed to add menu option`,`error`)}}catch(e){console.error(`Error adding menu option:`,e),showNotification(`Failed to add menu option`,`error`)}})}),document.addEventListener(`DOMContentLoaded`,function(){let e=document.getElementById(`edit-menu-option-form`);e&&e.addEventListener(`submit`,async function(e){e.preventDefault();let t=document.getElementById(`edit-menu-digit`).value,n=document.getElementById(`edit-menu-dest-type`).value,r;r=n===`submenu`?document.getElementById(`edit-menu-submenu`).value:document.getElementById(`edit-menu-destination`).value;let i={destination_type:n,destination_value:r,description:document.getElementById(`edit-menu-description`).value};try{let e=await fetch(`${API_BASE}/api/auto-attendant/menus/${q}/items/${t}`,{method:`PUT`,headers:pbxAuthHeaders(),body:JSON.stringify(i)});if(e.ok)showNotification(`Menu option updated successfully`,`success`),Ma(),G();else{let t=await e.json();showNotification(t.error||`Failed to update menu option`,`error`)}}catch(e){console.error(`Error updating menu option:`,e),showNotification(`Failed to update menu option`,`error`)}})}),document.addEventListener(`DOMContentLoaded`,function(){let e=document.getElementById(`auto-attendant-prompts-form`);e&&e.addEventListener(`submit`,async function(e){e.preventDefault();let t=document.getElementById(`voice-generation-status`),n=document.getElementById(`voice-generation-message`);t&&(t.style.display=`block`,n.textContent=`⏳ Saving prompts and regenerating voices using gTTS...`);let r={company_name:document.getElementById(`aa-company-name`).value,prompts:{welcome:document.getElementById(`aa-prompt-welcome`).value,main_menu:document.getElementById(`aa-prompt-main-menu`).value,invalid:document.getElementById(`aa-prompt-invalid`).value,timeout:document.getElementById(`aa-prompt-timeout`).value,transferring:document.getElementById(`aa-prompt-transferring`).value}};try{let e=await fetch(`${API_BASE}/api/auto-attendant/prompts`,{method:`PUT`,headers:pbxAuthHeaders(),body:JSON.stringify(r)});if(e.ok)showNotification(`Prompts saved and voices regenerated successfully!`,`success`),t&&(n.textContent=`✅ Voice prompts regenerated successfully using gTTS!`,setTimeout(()=>{t.style.display=`none`},3e3));else{let r=await e.json();showNotification(r.error||`Failed to save prompts`,`error`),t&&(n.textContent=`❌ Failed to regenerate voices`,setTimeout(()=>{t.style.display=`none`},3e3))}}catch(e){console.error(`Error saving prompts:`,e),showNotification(`Failed to save prompts`,`error`),t&&(n.textContent=`❌ Error: `+e.message,setTimeout(()=>{t.style.display=`none`},3e3))}})});function K(e){let t=document.createElement(`div`);return t.textContent=e,t.innerHTML}var q=`main`,Pa=[];async function Fa(e){try{return await e.json()}catch(e){let t=e?.message||String(e);return e instanceof SyntaxError||/JSON|Unexpected token/i.test(t)?{error:`Server returned an invalid response format`}:e instanceof TypeError||/network|NetworkError|fetch/i.test(t)?{error:`Network error while reading server response`}:(debugWarn(`Unexpected error parsing response:`,e),{error:`Unable to read server response`})}}async function Ia(){try{let e=await fetch(`${API_BASE}/api/auto-attendant/menus`,{headers:pbxAuthHeaders()});if(e.ok)return Pa=(await e.json()).menus??[],debugLog(`Loaded ${Pa.length} menu(s) for submenu selection`),Pa;{let t=await Fa(e);console.error(`Failed to load menus: ${e.status} ${e.statusText}`,t);let n=t.error||e.statusText;e.status===404?n+=`. API endpoint not found - check server version.`:e.status>=500&&(n+=`. Server error occurred.`),showNotification(`Failed to load menus: ${n}`,`error`)}}catch(e){console.error(`Error loading menus:`,e),showNotification(`Unable to connect to API: ${e.message}. Please check your connection.`,`error`)}return[]}async function La(){let e=await Ia(),t=document.getElementById(`new-menu-submenu`);if(t)if(t.innerHTML=`<option value="">Select a submenu</option>`,e.length===0){let e=document.createElement(`option`);e.value=``,e.textContent=`No menus available - create one first`,e.disabled=!0,t.appendChild(e),t.disabled=!0}else{for(let n of e)if(n.menu_id!==q){let e=document.createElement(`option`);e.value=n.menu_id,e.textContent=`${n.menu_name} (${n.menu_id})`,t.appendChild(e)}t.disabled=!1}let n=document.getElementById(`edit-menu-submenu`);if(n)if(n.innerHTML=`<option value="">Select a submenu</option>`,e.length===0){let e=document.createElement(`option`);e.value=``,e.textContent=`No menus available - create one first`,e.disabled=!0,n.appendChild(e),n.disabled=!0}else{for(let t of e)if(t.menu_id!==q){let e=document.createElement(`option`);e.value=t.menu_id,e.textContent=`${t.menu_name} (${t.menu_id})`,n.appendChild(e)}n.disabled=!1}let r=document.getElementById(`submenu-parent`);if(r)if(r.innerHTML=``,e.length===0){let e=document.createElement(`option`);e.value=``,e.textContent=`No parent menus available - check API connection or server status`,e.disabled=!0,e.selected=!0,r.appendChild(e),debugWarn(`No menus loaded for parent dropdown`)}else{for(let t of e){let e=document.createElement(`option`);e.value=t.menu_id,e.textContent=`${t.menu_name} (${t.menu_id})`,t.menu_id===`main`&&(e.selected=!0),r.appendChild(e)}debugLog(`Populated parent menu dropdown with ${e.length} menu(s)`)}}function Ra(e){let t=document.getElementById(`${e}-menu-dest-type`),n=document.getElementById(`${e}-dest-extension-group`),r=document.getElementById(`${e}-dest-submenu-group`),i=document.getElementById(`${e}-menu-destination`),a=document.getElementById(`${e}-menu-submenu`),o=document.getElementById(`${e}-dest-label`),s=document.getElementById(`${e}-dest-help`);if(!t)return;let c=t.value;if(c===`submenu`)n&&(n.style.display=`none`),r&&(r.style.display=`block`),i&&(i.required=!1),a&&(a.required=!0),La();else if(n&&(n.style.display=`block`),r&&(r.style.display=`none`),i&&(i.required=!0),a&&(a.required=!1),o)switch(c){case`extension`:o.textContent=`Extension`,s&&(s.textContent=`Extension number to transfer to`);break;case`queue`:o.textContent=`Queue`,s&&(s.textContent=`Queue extension number`);break;case`voicemail`:o.textContent=`Voicemail Box`,s&&(s.textContent=`Voicemail box extension`);break;case`operator`:o.textContent=`Operator Extension`,s&&(s.textContent=`Operator extension number`);break}}function za(){La(),document.getElementById(`create-submenu-modal`).classList.add(`active`),document.getElementById(`create-submenu-form`).reset()}function Ba(){document.getElementById(`create-submenu-modal`).classList.remove(`active`)}async function Va(){let e=document.getElementById(`menu-tree-container`);e.style.display===`none`?(e.style.display=`block`,await Ha()):e.style.display=`none`}async function Ha(){try{let e=await fetch(`${API_BASE}/api/auto-attendant/menu-tree`,{headers:pbxAuthHeaders()});if(!e.ok){let t=await Fa(e);throw console.error(`Failed to load menu tree: ${e.status} ${e.statusText}`,t),e.status===404?Error(`Menu tree endpoint not found. The server may need to be restarted to load new API routes.`):e.status===500?Error(`Server error: ${t.error||`Internal server error`}`):Error(`Failed to load menu tree: ${t.error||e.statusText}`)}let t=await e.json(),n=document.getElementById(`menu-tree-view`);t.menu_tree?(n.innerHTML=Ua(t.menu_tree,0),debugLog(`Menu tree loaded successfully`)):(n.innerHTML=`<p style="color: #666;">No menu structure available</p>`,debugWarn(`Menu tree data is empty`))}catch(e){console.error(`Error loading menu tree:`,e),showNotification(`Failed to load menu tree: ${e.message}`,`error`);let t=document.getElementById(`menu-tree-view`);t&&(t.innerHTML=`<p class="error-message">
    ...
    `,!document.getElementById(`notification-animations`)){let e=document.createElement(`style`);e.id=`notification-animations`,e.textContent=`
        ...
        `,document.head.appendChild(e)}r.appendChild(i),n>0&&setTimeout(()=>{i.style.animation=`slideIn 0.3s ease-out reverse`,setTimeout(()=>i.remove(),300)},n)}async function Ka(){Xa(),$a(),no(),await X()}async function X(){try{let e=(await(await fetch(`/api/config`,{headers:pbxAuthHeaders()})).json()).integrations||{},t=e.jitsi?.enabled||!1,n=document.getElementById(`quick-jitsi-enabled`),r=document.getElementById(`jitsi-status-badge`);n&&(n.checked=t),r&&(r.style.display=t?`inline-block`:`none`,r.style.backgroundColor=`#4CAF50`,r.style.color=`white`,r.textContent=`● Enabled`);let i=e.matrix?.enabled||!1,a=document.getElementById(`quick-matrix-enabled`),o=document.getElementById(`matrix-status-badge`);a&&(a.checked=i),o&&(o.style.display=i?`inline-block`:`none`,o.style.backgroundColor=`#9C27B0`,o.style.color=`white`,o.textContent=`● Enabled`);let s=e.espocrm?.enabled||!1,c=document.getElementById(`quick-espocrm-enabled`),l=document.getElementById(`espocrm-status-badge`);c&&(c.checked=s),l&&(l.style.display=s?`inline-block`:`none`,l.style.backgroundColor=`#2196F3`,l.style.color=`white`,l.textContent=`● Enabled`)}catch(e){console.error(`Failed to load integration status:`,e)}}async function qa(e){document.getElementById(`quick-${e}-enabled`).checked?await Ja(e):await Ya(e)}async function Ja(e){let t={jitsi:{enabled:!0,server_url:`https://localhost`,auto_create_rooms:!0,app_id:``,app_secret:``},matrix:{enabled:!0,homeserver_url:`https://localhost:8008`,bot_username:``,bot_password:"${MATRIX_BOT_PASSWORD}",notification_room:``,voicemail_room:``,missed_call_notifications:!0},espocrm:{enabled:!0,api_url:`https://localhost/api/v1`,api_key:"${ESPOCRM_API_KEY}",auto_create_contacts:!0,auto_log_calls:!0,screen_pop:!0}}[e];if(!t){Y(`Unknown integration: ${e}`,`error`);return}try{if((await fetch(`/api/config/section`,{method:`PUT`,headers:pbxAuthHeaders(),body:JSON.stringify({section:`integrations`,data:{[e]:t}})})).ok){let t=document.getElementById(`quick-${e}-enabled`);t&&(t.checked=!0),X();let n=`✅ ${Ga[e]} enabled with default settings! The integration is now active.`;e===`matrix`?Y(`${n} Note: You need to set MATRIX_BOT_PASSWORD in your .env file for Matrix to work.`,`warning`,8e3):e===`espocrm`?Y(`${n} Note: You need to set ESPOCRM_API_KEY and api_url in the configuration tab.`,`warning`,8e3):Y(n,`success`),e===`jitsi`?Xa():e===`matrix`?$a():e===`espocrm`&&no()}else{Y(`Failed to enable ${Ga[e]}`,`error`);let t=document.getElementById(`quick-${e}-enabled`);t&&(t.checked=!1)}}catch(t){Y(`Error enabling integration: ${t.message}`,`error`);let n=document.getElementById(`quick-${e}-enabled`);n&&(n.checked=!1)}}async function Ya(e){try{let t=(await(await fetch(`/api/config`,{headers:pbxAuthHeaders()})).json()).integrations?.[e]??{};if(t.enabled=!1,(await fetch(`/api/config/section`,{method:`PUT`,headers:pbxAuthHeaders(),body:JSON.stringify({section:`integrations`,data:{[e]:t}})})).ok)X(),Y(`${Ga[e]} has been disabled.`,`info`),e===`jitsi`?Xa():e===`matrix`?$a():e===`espocrm`&&no();else{Y(`Failed to disable ${Ga[e]}`,`error`);let t=document.getElementById(`quick-${e}-enabled`);t&&(t.checked=!0)}}catch(t){Y(`Error disabling integration: ${t.message}`,`error`);let n=document.getElementById(`quick-${e}-enabled`);n&&(n.checked=!0)}}async function Xa(){try{let e=await fetch(`/api/config`,{headers:pbxAuthHeaders()});if(!e.ok){window.suppressErrorNotifications?debugLog(`Config endpoint returned error:`,e.status,`(feature may not be available or authentication required)`):console.error(`Failed to load Jitsi config:`,e.status);return}let t=(await e.json()).integrations?.jitsi??{};document.getElementById(`jitsi-enabled`).checked=t.enabled??!1,document.getElementById(`jitsi-server-url`).value=t.server_url??`https://localhost`,document.getElementById(`jitsi-auto-create-rooms`).checked=t.auto_create_rooms!==!1,document.getElementById(`jitsi-app-id`).value=t.app_id??``,document.getElementById(`jitsi-app-secret`).value=t.app_secret??``,Za()}catch(e){window.suppressErrorNotifications?debugLog(`Failed to load Jitsi config (expected if not authenticated):`,e.message):console.error(`Failed to load Jitsi config:`,e)}}function Za(){let e=document.getElementById(`jitsi-enabled`).checked;document.getElementById(`jitsi-settings`).style.display=e?`block`:`none`}document.getElementById(`jitsi-enabled`)?.addEventListener(`change`,Za),document.getElementById(`jitsi-config-form`)?.addEventListener(`submit`,async function(e){e.preventDefault();let t={enabled:document.getElementById(`jitsi-enabled`).checked,server_url:document.getElementById(`jitsi-server-url`).value,auto_create_rooms:document.getElementById(`jitsi-auto-create-rooms`).checked,app_id:document.getElementById(`jitsi-app-id`).value,app_secret:document.getElementById(`jitsi-app-secret`).value};try{(await fetch(`/api/config/section`,{method:`PUT`,headers:pbxAuthHeaders(),body:JSON.stringify({section:`integrations`,data:{jitsi:t}})})).ok?(Z(`Configuration saved successfully!`,`success`),X()):Z(`Failed to save configuration`,`error`)}catch(e){Z(`Error: ${J(e.message)}`,`error`)}});async function Qa(){let e=document.getElementById(`jitsi-server-url`).value;Z(`Testing connection to ${J(e)}...`,`info`);try{let t=`${e}/external_api.js`;await fetch(t,{mode:`no-cors`});let n=`${e}/test-pbx-${Date.now()}`;Z(`✅ Connection successful!<br>Test meeting URL: <a href="${J(n)}" target="_blank">${J(n)}</a>`,`success`)}catch(e){Z(`⚠️ Could not verify connection. Server may still be accessible.<br>Error: ${J(e.message)}`,`warning`)}}function Z(e,t){let n=[`success`,`error`,`warning`,`info`].includes(t)?t:`info`,r=document.getElementById(`jitsi-status`);r.innerHTML=`<div class="alert alert-${n}">${e}</div>`}async function $a(){try{let e=await fetch(`/api/config`,{headers:pbxAuthHeaders()});if(!e.ok){window.suppressErrorNotifications?debugLog(`Config endpoint returned error:`,e.status,`(feature may not be available or authentication required)`):console.error(`Failed to load Matrix config:`,e.status);return}let t=(await e.json()).integrations?.matrix??{};document.getElementById(`matrix-enabled`).checked=t.enabled??!1,document.getElementById(`matrix-homeserver-url`).value=t.homeserver_url??`https://localhost:8008`,document.getElementById(`matrix-bot-username`).value=t.bot_username??``,document.getElementById(`matrix-bot-password`).value=t.bot_password??``,document.getElementById(`matrix-notification-room`).value=t.notification_room??``,document.getElementById(`matrix-voicemail-room`).value=t.voicemail_room??``,document.getElementById(`matrix-missed-call-notifications`).checked=t.missed_call_notifications!==!1,eo()}catch(e){window.suppressErrorNotifications?debugLog(`Failed to load Matrix config (expected if not authenticated):`,e.message):console.error(`Failed to load Matrix config:`,e)}}function eo(){let e=document.getElementById(`matrix-enabled`).checked;document.getElementById(`matrix-settings`).style.display=e?`block`:`none`}document.getElementById(`matrix-enabled`)?.addEventListener(`change`,eo),document.getElementById(`matrix-config-form`)?.addEventListener(`submit`,async function(e){e.preventDefault();let t={enabled:document.getElementById(`matrix-enabled`).checked,homeserver_url:document.getElementById(`matrix-homeserver-url`).value,bot_username:document.getElementById(`matrix-bot-username`).value,bot_password:document.getElementById(`matrix-bot-password`).value,notification_room:document.getElementById(`matrix-notification-room`).value,voicemail_room:document.getElementById(`matrix-voicemail-room`).value,missed_call_notifications:document.getElementById(`matrix-missed-call-notifications`).checked};try{(await fetch(`/api/config/section`,{method:`PUT`,headers:pbxAuthHeaders(),body:JSON.stringify({section:`integrations`,data:{matrix:t}})})).ok?(Q(`Configuration saved successfully!`,`success`),X()):Q(`Failed to save configuration`,`error`)}catch(e){Q(`Error: ${J(e.message)}`,`error`)}});async function to(){let e=document.getElementById(`matrix-homeserver-url`).value,t=document.getElementById(`matrix-bot-username`).value,n=document.getElementById(`matrix-bot-password`).value;if(!t||!n){Q(`Please enter bot username and password`,`error`);return}Q(`Testing Matrix connection...`,`info`);try{let t=`${e}/_matrix/client/versions`,n=await fetch(t);if(!n.ok)throw Error(`Homeserver not accessible`);Q(`✅ Homeserver is accessible!<br>Supported versions: ${J((await n.json()).versions?.join(`, `)??`Unknown`)}<br><small>Note: Full authentication test requires server-side validation</small>`,`success`)}catch(e){Q(`❌ Connection failed: ${J(e.message)}`,`error`)}}function Q(e,t){let n=[`success`,`error`,`warning`,`info`].includes(t)?t:`info`,r=document.getElementById(`matrix-status`);r.innerHTML=`<div class="alert alert-${n}">${e}</div>`}async function no(){try{let e=await fetch(`/api/config`,{headers:pbxAuthHeaders()});if(!e.ok){window.suppressErrorNotifications?debugLog(`Config endpoint returned error:`,e.status,`(feature may not be available or authentication required)`):console.error(`Failed to load EspoCRM config:`,e.status);return}let t=(await e.json()).integrations?.espocrm??{};document.getElementById(`espocrm-enabled`).checked=t.enabled??!1,document.getElementById(`espocrm-api-url`).value=t.api_url??`https://localhost/api/v1`,document.getElementById(`espocrm-api-key`).value=t.api_key??``,document.getElementById(`espocrm-auto-create-contacts`).checked=t.auto_create_contacts!==!1,document.getElementById(`espocrm-auto-log-calls`).checked=t.auto_log_calls!==!1,document.getElementById(`espocrm-screen-pop`).checked=t.screen_pop!==!1,ro()}catch(e){window.suppressErrorNotifications?debugLog(`Failed to load EspoCRM config (expected if not authenticated):`,e.message):console.error(`Failed to load EspoCRM config:`,e)}}function ro(){let e=document.getElementById(`espocrm-enabled`).checked;document.getElementById(`espocrm-settings`).style.display=e?`block`:`none`}document.getElementById(`espocrm-enabled`)?.addEventListener(`change`,ro),document.getElementById(`espocrm-config-form`)?.addEventListener(`submit`,async function(e){e.preventDefault();let t={enabled:document.getElementById(`espocrm-enabled`).checked,api_url:document.getElementById(`espocrm-api-url`).value,api_key:document.getElementById(`espocrm-api-key`).value,auto_create_contacts:document.getElementById(`espocrm-auto-create-contacts`).checked,auto_log_calls:document.getElementById(`espocrm-auto-log-calls`).checked,screen_pop:document.getElementById(`espocrm-screen-pop`).checked};try{(await fetch(`/api/config/section`,{method:`PUT`,headers:pbxAuthHeaders(),body:JSON.stringify({section:`integrations`,data:{espocrm:t}})})).ok?($(`Configuration saved successfully!`,`success`),X()):$(`Failed to save configuration`,`error`)}catch(e){$(`Error: ${J(e.message)}`,`error`)}});async function io(){let e=document.getElementById(`espocrm-api-url`).value,t=document.getElementById(`espocrm-api-key`).value;if(!e||!t){$(`Please enter API URL and API Key`,`error`);return}$(`Testing EspoCRM connection...`,`info`);try{let n=`${e}/App/user`,r=await fetch(n,{headers:{"X-Api-Key":t,"Content-Type":`application/json`}});if(r.ok)$(`✅ Connection successful!<br>Connected as: ${J((await r.json()).userName??`Unknown`)}<br>EspoCRM is ready for integration.`,`success`);else throw Error(`API returned status ${r.status}`)}catch(e){$(`❌ Connection failed: ${J(e.message)}<br>Check API URL and API Key.`,`error`)}}function $(e,t){let n=[`success`,`error`,`warning`,`info`].includes(t)?t:`info`,r=document.getElementById(`espocrm-status`);r.innerHTML=`<div class="alert alert-${n}">${e}</div>`}async function ao(){let e=document.getElementById(`jitsi-instant-room`).value??``;try{let t=await fetch(`/api/integrations/jitsi/instant`,{method:`POST`,headers:pbxAuthHeaders(),body:JSON.stringify({room_name:e,extension:`admin`})});if(!t.ok){let e=await t.json();throw Error(e.error||`Failed to create meeting`)}so((await t.json()).meeting_url),Y(`Meeting created successfully!`,`success`)}catch(e){Y(`Failed to create meeting: ${e.message}`,`error`)}}async function oo(){let e=document.getElementById(`jitsi-schedule-subject`).value,t=document.getElementById(`jitsi-schedule-duration`).value;if(!e){Y(`Please enter a meeting subject`,`warning`);return}try{let n=await fetch(`/api/integrations/jitsi/meetings`,{method:`POST`,headers:pbxAuthHeaders(),body:JSON.stringify({subject:e,duration:parseInt(t),moderator_name:`Admin`})});if(!n.ok){let e=await n.json();throw Error(e.error||`Failed to schedule meeting`)}so((await n.json()).meeting_url),Y(`Meeting scheduled successfully!`,`success`)}catch(e){Y(`Failed to schedule meeting: ${e.message}`,`error`)}}function so(e){let t=document.getElementById(`jitsi-meeting-result`),n=document.getElementById(`jitsi-meeting-url`);n.value=e,t.style.display=`block`}async function co(){let e=document.getElementById(`jitsi-meeting-url`),t=e.value;try{navigator.clipboard&&navigator.clipboard.writeText?(await navigator.clipboard.writeText(t),Y(`Meeting URL copied to clipboard!`,`success`,3e3)):(e.select(),document.execCommand(`copy`),Y(`Meeting URL copied to clipboard!`,`success`,3e3))}catch{e.select(),document.execCommand(`copy`),Y(`Meeting URL copied to clipboard!`,`success`,3e3)}}function lo(){let e=document.getElementById(`jitsi-meeting-url`).value;e&&window.open(e,`_blank`)}document.addEventListener(`DOMContentLoaded`,function(){let e=document.getElementById(`matrix-room-select`);e&&e.addEventListener(`change`,function(){let e=document.getElementById(`matrix-custom-room`);this.value===`custom`?e.style.display=`block`:e.style.display=`none`})});async function uo(){let e=document.getElementById(`matrix-room-select`).value,t=document.getElementById(`matrix-custom-room-id`).value,n=document.getElementById(`matrix-message-text`).value;if(!n){Y(`Please enter a message`,`warning`);return}let r=null;if(e===`custom`){if(r=t,!r){Y(`Please enter a custom room ID`,`warning`);return}}else (e===`notification`||e===`voicemail`)&&(r=null);try{let e=await fetch(`/api/integrations/matrix/messages`,{method:`POST`,headers:pbxAuthHeaders(),body:JSON.stringify({room_id:r,message:n,msg_type:`m.text`})});if(!e.ok){let t=await e.json();throw Error(t.error||`Failed to send message`)}await e.json(),mo(`✅ Message sent successfully!`,`success`),document.getElementById(`matrix-message-text`).value=``}catch(e){mo(`❌ Failed to send message: ${e.message}`,`error`)}}async function fo(){try{let e=await fetch(`/api/integrations/matrix/notifications`,{method:`POST`,headers:pbxAuthHeaders(),body:JSON.stringify({message:`🧪 Test notification from PBX Admin Panel - ${new Date().toLocaleString()}`})});if(!e.ok){let t=await e.json();throw Error(t.error||`Failed to send notification`)}mo(`✅ Test notification sent successfully!`,`success`)}catch(e){mo(`❌ Failed to send notification: ${e.message}`,`error`)}}async function po(){let e=document.getElementById(`matrix-new-room-name`).value,t=document.getElementById(`matrix-new-room-topic`).value;if(!e){Y(`Please enter a room name`,`warning`);return}try{let n=await fetch(`/api/integrations/matrix/rooms`,{method:`POST`,headers:pbxAuthHeaders(),body:JSON.stringify({name:e,topic:t})});if(!n.ok){let e=await n.json();throw Error(e.error||`Failed to create room`)}ho((await n.json()).room_id),Y(`Room created successfully!`,`success`),document.getElementById(`matrix-new-room-name`).value=``,document.getElementById(`matrix-new-room-topic`).value=``}catch(e){Y(`Failed to create room: ${e.message}`,`error`)}}function mo(e,t){let n=document.getElementById(`matrix-message-result`),r=document.getElementById(`matrix-message-result-text`);r.textContent=e,n.style.display=`block`,n.querySelector(`.info-box`).style.backgroundColor=t===`success`?`#e8f5e9`:`#ffebee`,setTimeout(()=>{n.style.display=`none`},5e3)}function ho(e){let t=document.getElementById(`matrix-room-result`),n=document.getElementById(`matrix-new-room-id`);n.textContent=e,t.style.display=`block`}async function go(){let e=document.getElementById(`espocrm-search-type`).value,t=document.getElementById(`espocrm-search-term`).value;if(!t){Y(`Please enter a search term`,`warning`);return}try{let n=await fetch(`/api/integrations/espocrm/contacts/search?${e===`phone`?`phone`:e===`email`?`email`:`name`}=${encodeURIComponent(t)}`,{method:`GET`,headers:pbxAuthHeaders()});if(!n.ok){let e=await n.json();throw Error(e.error||`Failed to search contact`)}_o(await n.json())}catch(e){Y(`Failed to search contact: ${e.message}`,`error`)}}function _o(e){let t=document.getElementById(`espocrm-search-results`),n=document.getElementById(`espocrm-contact-details`);if(!e.success||!e.contact){n.innerHTML=`<div class="info-box" style="background-color: #fff3e0;">No contact found</div>`,t.style.display=`block`;return}let r=e.contact,i=`<div class="info-box" style="background-color: #e8f5e9;">`;i+=`<h4>✅ Contact Found</h4>`,i+=`<table style="width: 100%; margin-top: 10px;">`,r.name&&(i+=`<tr><td><strong>Name:</strong></td><td>${J(r.name)}</td></tr>`),r.email&&(i+=`<tr><td><strong>Email:</strong></td><td>${J(r.email)}</td></tr>`),r.phone&&(i+=`<tr><td><strong>Phone:</strong></td><td>${J(r.phone)}</td></tr>`),r.company&&(i+=`<tr><td><strong>Company:</strong></td><td>${J(r.company)}</td></tr>`),r.title&&(i+=`<tr><td><strong>Title:</strong></td><td>${J(r.title)}</td></tr>`),r.id&&(i+=`<tr><td><strong>CRM ID:</strong></td><td>${J(r.id)}</td></tr>`),i+=`</table></div>`,n.innerHTML=i,t.style.display=`block`}async function vo(){let e=document.getElementById(`espocrm-new-firstname`).value,t=document.getElementById(`espocrm-new-lastname`).value,n=document.getElementById(`espocrm-new-phone`).value,r=document.getElementById(`espocrm-new-email`).value,i=document.getElementById(`espocrm-new-company`).value,a=document.getElementById(`espocrm-new-title`).value;if(!e||!t){Y(`Please enter first and last name`,`warning`);return}if(!n&&!r){Y(`Please enter at least phone or email`,`warning`);return}try{let o=await fetch(`/api/integrations/espocrm/contacts`,{method:`POST`,headers:pbxAuthHeaders(),body:JSON.stringify({name:`${e} ${t}`,phone:n,email:r,company:i,title:a})});if(!o.ok){let e=await o.json();throw Error(e.error||`Failed to create contact`)}yo(`✅ Contact created successfully! CRM ID: ${(await o.json()).contact?.id??`unknown`}`),document.getElementById(`espocrm-new-firstname`).value=``,document.getElementById(`espocrm-new-lastname`).value=``,document.getElementById(`espocrm-new-phone`).value=``,document.getElementById(`espocrm-new-email`).value=``,document.getElementById(`espocrm-new-company`).value=``,document.getElementById(`espocrm-new-title`).value=``}catch(e){yo(`❌ Failed to create contact: ${e.message}`)}}function yo(e){let t=document.getElementById(`espocrm-create-result`),n=document.getElementById(`espocrm-create-result-text`);n.textContent=e,t.style.display=`block`,setTimeout(()=>{t.style.display=`none`},5e3)}window.loadOpenSourceIntegrations=Ka,window.loadJitsiConfig=Xa,window.loadMatrixConfig=$a,window.loadEspoCRMConfig=no,window.toggleJitsiSettings=Za,window.testJitsiConnection=Qa,window.toggleMatrixSettings=eo,window.testMatrixConnection=to,window.toggleEspoCRMSettings=ro,window.testEspoCRMConnection=io,window.quickToggleIntegration=qa,window.quickSetupIntegration=Ja,window.disableIntegration=Ya,window.createInstantJitsiMeeting=ao,window.scheduleJitsiMeeting=oo,window.copyJitsiMeetingUrl=co,window.openJitsiMeeting=lo,window.sendMatrixMessage=uo,window.sendMatrixTestNotification=fo,window.createMatrixRoom=po,window.searchEspoCRMContact=go,window.createEspoCRMContact=vo,setTimeout(function(){if(!window.currentUser&&!document.querySelector(`.tab-content.active`)){console.warn(`Page may not have loaded correctly. Checking for common issues...`);var e=document.querySelector(`.sidebar`);if(e){var t=window.getComputedStyle(e);if(t.width===`auto`||t.width===`0px`){console.error(`CSS may not be loaded correctly. Try clearing your browser cache:`),console.error(`  - Press Ctrl+Shift+R (Windows/Linux) or Cmd+Shift+R (Mac)`),console.error(`  - See BROWSER_CACHE_FIX.md for detailed instructions`);var n=document.createElement(`div`);n.className=`page-load-alert`,n.innerHTML=`<strong class="page-load-alert-title">⚠️ Page Loading Issue</strong><p>The admin panel may not be displaying correctly due to cached files.</p><p class="page-load-alert-action">Press <code>Ctrl+Shift+R</code> (or <code>Cmd+Shift+R</code> on Mac) to reload without cache</p>`;var r=document.createElement(`button`);r.className=`page-load-alert-dismiss`,r.textContent=`Dismiss`,r.addEventListener(`click`,function(){n.remove()}),n.appendChild(r),document.body.appendChild(n)}}}else console.log(`Page loaded successfully at`,new Date().toISOString())},3e3),(function(){var e=document.getElementById(`sidebar-toggle`),t=document.querySelector(`.sidebar`),n=document.getElementById(`sidebar-overlay`);if(!e||!t||!n)return;function r(){t.classList.add(`open`),n.classList.add(`active`),e.classList.add(`active`),e.setAttribute(`aria-expanded`,`true`)}function i(){t.classList.remove(`open`),n.classList.remove(`active`),e.classList.remove(`active`),e.setAttribute(`aria-expanded`,`false`)}e.addEventListener(`click`,function(){t.classList.contains(`open`)?i():r()}),n.addEventListener(`click`,i),t.addEventListener(`click`,function(e){e.target.closest(`.tab-button`)&&i()}),document.addEventListener(`keydown`,function(e){e.key===`Escape`&&t.classList.contains(`open`)&&i()})})();
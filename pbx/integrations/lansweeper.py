...
class LansweeperIntegration:
    ...
    def __init__(self, config: dict | None = None) -> None:
        ...
        if self.enabled and not REQUESTS_AVAILABLE:
            ...
            if self.api_url.startswith("http://"):
                self.logger.warning(
                    "  WARNING: Using HTTP (not HTTPS) - API credentials sent in plaintext!"
                )
    ...
    def link_phone_to_extension(self, extension: str, mac_address: str) -> bool:
        ...
        if not asset:
            self.logger.warning(
                f"Cannot link extension {extension}: asset not found for MAC {mac_address}"
            )
            return False
        ...
        self.logger.info(
            f"Linked extension {extension} to Lansweeper asset {asset.get('AssetName', 'Unknown')}"
        )
        return True
    ...
    def update_asset_custom_field(self, mac_address: str, field_name: str, value: str) -> bool:
        """
        ...
        data = {"mac_address": mac_normalized, "field_name": field_name, "value": value}
    ...
    def sync_phone_extension(self, extension: str, mac_address: str, phone_model: str) -> bool:
        """
    ...
    def get_building_phones(self, building: str) -> list[dict]:
        ...
        # Filter by building
        building_phones = [
            phone for phone in all_phones if phone.get("Building", "").lower() == building.lower()
        ]
    ...
    def generate_e911_report(self) -> dict:
        ...
        self.logger.info(
            f"Generated E911 report: {len(phones)} phones, {len(report['missing_location'])} missing location"
        )

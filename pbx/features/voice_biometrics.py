...
class VoiceBiometrics:
    ...
    def verify_speaker(self, user_id: str, audio_data: bytes) -> dict:
        ...
        if profile.status != BiometricStatus.ENROLLED:
            return {"verified": False, "confidence": 0.0, "error": "Profile not enrolled"}
        ...
        if verified:
            ...
            self.logger.info(
                f"Voice verification successful for user {user_id} (confidence: {confidence:.2f})"
            )
        else:
            profile.failed_verifications += 1
            self.failed_verifications += 1
            self.logger.warning(
                f"Voice verification failed for user {user_id} (confidence: {confidence:.2f})"
            )

        # Save to database
        if self.db:
            call_id = f"call-{datetime.now(UTC).timestamp()}"
    ...
    def detect_fraud(self, audio_data: bytes, caller_info: dict) -> dict:
        ...
        if audio_len > 0:
            ...
            if chunk_size > 0:
                ...
                for i in range(0, audio_len - chunk_size, chunk_size):
                    ...
                    chunk2 = (
                        audio_data[i + chunk_size : i + 2 * chunk_size]
                        if i + 2 * chunk_size <= audio_len
                        else b""
                    )
        ...
        # Save to database
        if self.db:
            call_id = caller_info.get("call_id", f"call-{datetime.now(UTC).timestamp()}")
    ...
    def _generate_session_id(self, user_id: str) -> str:
        ...
        data = f"{user_id}-{datetime.now(UTC).isoformat()}"
    ...
    def _create_voiceprint(self, user_id: str, audio_data: bytes) -> str:
        ...
        if user_id not in self.profiles:
            return hashlib.sha256(f"{user_id}-{datetime.now(UTC)}".encode()).hexdigest()
        ...
        if profile.enrollment_features:
            ...
            feature_string = "-".join(
                f"{k}:{v:.4f}" for k, v in sorted(aggregated_features.items())
            )
            ...
            if (
                SKLEARN_AVAILABLE
                and len(profile.enrollment_features) >= self.MIN_GMM_ENROLLMENT_SAMPLES
            ):
                try:
    ...
    def _calculate_match_score(self, profile: VoiceProfile, voice_features: dict) -> float:
        ...
        if (
            SKLEARN_AVAILABLE
            and hasattr(profile, "gmm_model")
            and profile.gmm_model is not None
            and voice_features
        ):
            try:
        ...
        try:
            ...
            if not features:
                ...
                if len(chunk_energies) > 1:
                    ...
                    variance = sum((e - mean_energy) ** 2 for e in chunk_energies) / len(
                        chunk_energies
                    )
    ...
    def get_statistics(self) -> dict:
        ...
        stats = {
            ...
            "enrolled_profiles": len(
                [p for p in self.profiles.values() if p.status == BiometricStatus.ENROLLED]
            ),
            "total_enrollments": self.total_enrollments,
            "total_verifications": self.total_verifications,
            "successful_verifications": self.successful_verifications,
            "failed_verifications": self.failed_verifications,
            "verification_success_rate": self.successful_verifications

...
def test_vip_caller_database() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert console.mark_vip_caller(
            "555-1234", priority_level=1, name="Important Client", notes="Key account"
        )
...
def test_voicemail_ivr() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        config._config = {
            "extensions": [{"number": "1001", "name": "Test User", "voicemail_pin": "1234"}]
        }
        config.get = lambda key, default=None: config._config.get(key, default)
        config.get_extension = lambda num: next(
            (e for e in config._config["extensions"] if e["number"] == num), None
        )
...
def test_ad_group_permissions_mapping() -> None:
    ...
    user_groups = [
        "CN=PBX_Admins,OU=Groups,DC=test,DC=local",
